...
public class MetricRecorderProvider30Metrics50Impl implements MetricRecorderProvider {
    ...
    private MetricRecorder createNewRecorder(Method method, RetryPolicy retryPolicy, CircuitBreakerPolicy circuitBreakerPolicy, TimeoutPolicy timeoutPolicy,
        ...
        if (isMetricsEnabled(method.getDeclaringClass())) {
            MetricRegistry registry = sharedRegistries.getOrCreate(MetricRegistry.BASE_SCOPE);
            return new MetricRecorder30Metrics50Impl(getMetricName(method), registry, retryPolicy, circuitBreakerPolicy, timeoutPolicy, bulkheadPolicy, fallbackPolicy, isAsync);
        } else {
