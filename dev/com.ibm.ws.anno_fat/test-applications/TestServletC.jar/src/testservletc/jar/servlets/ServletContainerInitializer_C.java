...
public class ServletContainerInitializer_C implements ServletContainerInitializer {

    @Override
    public void onStartup(Set<Class<?>> setOfClassesInterestedIn, ServletContext context) throws ServletException {
        
        System.out.println("--- Servlet C CONTAINER INITIALIZER! ---");
        
        //going to add a context attribute to show the set of classes that were passed in
        if (setOfClassesInterestedIn != null) {
            
            String sciName = "SCI C";
            context.setAttribute("Sci_C_RanMsg", sciName + " actually ran! ");   // Unique attribute for each SCI.  Intended to show that this SCI ran.
            ...
            context.setAttribute("sciOrder", (sciOrder == null ? "" : sciOrder)  + "C");
        ...
        System.out.println("--- Servlet C CONTAINER INITIALIZER! --- EXIT");
    }
