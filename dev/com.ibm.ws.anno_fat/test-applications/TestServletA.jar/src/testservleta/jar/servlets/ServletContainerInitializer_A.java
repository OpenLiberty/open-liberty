...
public class ServletContainerInitializer_A implements ServletContainerInitializer {

    @Override
    public void onStartup(Set<Class<?>> setOfClassesInterestedIn, ServletContext context) throws ServletException {
        
        System.out.println("--- Servlet A  CONTAINER INITIALIZER! ---");
        
        //going to add a context attribute to show the set of classes that were passed in
        if (setOfClassesInterestedIn != null) {
            
            String sciName = "SCI A";
            context.setAttribute("Sci_A_RanMsg", sciName + " actually ran! ");   // Unique attribute for each SCI.  Intended to show that this SCI ran.
            ...
            context.setAttribute("sciOrder", (sciOrder == null ? "" : sciOrder)  + "A");
        ...
        System.out.println("--- Servlet A CONTAINER INITIALIZER! --- EXIT");
    }
