package testservletb.jar.servlets;
...
public class ServletContainerInitializer_B implements ServletContainerInitializer {

    @Override
    public void onStartup(Set<Class<?>> setOfClassesInterestedIn, ServletContext context) throws ServletException {
        
        System.out.println("--- Servlet B CONTAINER INITIALIZER! ---");
        
        //going to add a context attribute to show the set of classes that were passed in
        if (setOfClassesInterestedIn != null) {
            
            String sciName = "SCI B";
            context.setAttribute("Sci_B_RanMsg", sciName + " actually ran! ");   // Unique attribute for each SCI.  Intended to show that this SCI ran.
            ...
            context.setAttribute("sciOrder", (sciOrder == null ? "" : sciOrder)  + "B");
        ...
        System.out.println("--- Servlet B CONTAINER INITIALIZER! --- EXIT");
    }
