...
import com.ibm.ws.anno.jandex.internal.LimitedAnnotation;
...
public class AnnotationTargetsImpl_JandexConverter {
    ...
    public void convertClassInfo(
        ...
        List<DotName> interfaceDotNames = Arrays.asList(jandexClassInfo.interfaceNames());
        ...
        for ( DotName jandexClassAnnotation : jandexClassInfo.classAnnotations() ) {
            jandex_recordAnnotation(
                scanPolicy, AnnotationCategory.CLASS,
                i_className, jandexClassAnnotation);
        }

        for ( LimitedAnnotation jandexFieldAnno : jandexClassInfo.fieldAnnotations() ) {
            jandex_recordAnnotation(scanPolicy, AnnotationCategory.FIELD,
                                     i_className, jandexFieldAnno.getName());
        }

        for( LimitedAnnotation jandexMethodAnno : jandexClassInfo.methodAnnotations() ) {
            jandex_recordAnnotation(
                    scanPolicy, AnnotationCategory.METHOD,
                    i_className, jandexMethodAnno.getName());
        }
    }
}
