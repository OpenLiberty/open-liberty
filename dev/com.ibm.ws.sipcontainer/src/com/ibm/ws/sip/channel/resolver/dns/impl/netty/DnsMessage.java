...
public class DnsMessage {
    ...
    public ByteBuf toBuffer() {
        ...
        // X CR 4564
        buffer.writeByte((byte) (_header.identification >>> 8) & 0x000000ff);
        buffer.writeByte((byte) (_header.identification & 0x000000ff));
        _header.bitSetToShort();
        buffer.writeShort(_header.flags);
        buffer.writeShort(_header.numQuestions);
        buffer.writeShort(_header.numAnswers);
        buffer.writeShort(_header.numAuthority);
        buffer.writeShort(_header.numAdditional);
        ...
        for (Enumeration e = _questions.elements(); e.hasMoreElements();) {
            ...
            buffer.writeShort(q.qtype);
            buffer.writeShort(q.qclass);
        }
    ...
    public DnsMessage(ByteBuf buffer) throws Exception {
    ...
    /*
     * public String toHexString() { WsByteBuffer buffer = this.toBuffer(); String s
     * = new String(); for (int i = 0; i < 512; i++){ byte b = buffer.get(); char c
     * = HEX[(b & 0xF0) >> 4]; s += c; char d = HEX[(b & 0x0F)]; s += d; } return s;
     * }
     */
    ...
    class Header {
        ...
        protected void parse(ByteBuf buffer) {
            ...
            try {
                identification = (int) buffer.readShort() & 0x0000ffff;
                flags = buffer.readShort();
                numQuestions = buffer.readShort();
                numAnswers = buffer.readShort();
                numAuthority = buffer.readShort();
                numAdditional = buffer.readShort();
        ...
        public String toString() {
            ...
            short sh = 0;
            if (bitSetFlags.get(1)) {
                sh |= 0x1000;
            }
            if (bitSetFlags.get(2)) {
                sh |= 0x0100;
            }
            if (bitSetFlags.get(3)) {
                sh |= 0x0010;
            }
            if (bitSetFlags.get(4)) {
                sh |= 0x0001;
            }
            ...
            sh = 0;
            if (bitSetFlags.get(9)) {
                sh |= 0x100;
            }
            if (bitSetFlags.get(10)) {
                sh |= 0x010;
            }
            if (bitSetFlags.get(11)) {
                sh |= 0x001;
            }
            ...
            s += "  num Questions : " + (new Short(numQuestions)).toString() + "\n" + "  num Answers   : "
                    + (new Short(numAnswers)).toString() + "\n" + "  num Authority : "
                    + (new Short(numAuthority)).toString() + "\n" + "  num Additional: "
                    + (new Short(numAdditional)).toString() + "\n";
    ...
    class Question {
        ...
        protected Question(ByteBuf buffer) {
            qname = new Name(buffer);
            qtype = buffer.readShort();
            qclass = buffer.readShort();
        }
