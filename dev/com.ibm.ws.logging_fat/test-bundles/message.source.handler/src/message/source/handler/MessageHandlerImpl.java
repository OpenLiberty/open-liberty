 ...
 *******************************************************************************/
package message.source.handler;
 ...
 *
 */
public class MessageHandlerImpl implements Handler {

    private static final TraceComponent tc = Tr.register(MessageHandlerImpl.class, "MessageSourceHandler",
                                                         "message.source.handler.MessageHandlerImplMessages");
    ...
    private final List<String> sourceIds = new ArrayList<String>() {
        {
            add("com.ibm.ws.logging.source.message|memory");
        }
    };
    ...
    private final Runnable handlerTask = new Runnable() {
        ...
        public void run() {
            ...
            while (loop > 0) {
                try {
                    Object event = bufferMgr.getNextEvent("MessageHandler");
                    if (event != null) {
                        if (TraceComponent.isAnyTracingEnabled() && tc.isDebugEnabled()) {
                            Tr.debug(tc, "Received message event: " + event, this);
                        }
                    }
                    loop--;
