 ...
 *******************************************************************************/
package accesslog.source.handler;
 ...
 *
 */
public class AccessLogHandlerImpl implements Handler {

    private static final TraceComponent tc = Tr.register(AccessLogHandlerImpl.class, "AccessLogSourceHandler",
                                                         "accesslog.source.handler.AccessLogHandlerImplMessages");
    ...
    private final List<String> sourceIds = new ArrayList<String>() {
        {
            add("com.ibm.ws.http.logging.source.accesslog|memory");
        }
    };
    ...
    private final Runnable handlerTask = new Runnable() {
        ...
        public void run() {
            ...
            while (count <= 3) {
                try {
                    Object event = bufferMgr.getNextEvent("AccessLogHandler");
                    if (TraceComponent.isAnyTracingEnabled() && tc.isDebugEnabled()) {
                        Tr.debug(tc, "Received AccessLog event: " + event, this);
                    }
                    count++;
