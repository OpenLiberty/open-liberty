...
public class AESKeyManager {
    ...
    public enum KeyVersion {
        ...
        AES_V0(CryptoUtils.PBKDF2_WITH_HMAC_SHA1, CryptoUtils.PBKDF2HMACSHA1_ITERATIONS, CryptoUtils.AES_128_KEY_LENGTH_BITS, CryptoUtils.AES_V0_SALT, PROPERTY_WLP_PASSWORD_ENCRYPTION_KEY, (byte) 0),
        AES_V1(CryptoUtils.PBKDF2_WITH_HMAC_SHA512, CryptoUtils.PBKDF2HMACSHA512_ITERATIONS, CryptoUtils.AES_256_KEY_LENGTH_BITS, CryptoUtils.AES_V1_SALT, PROPERTY_WLP_PASSWORD_ENCRYPTION_KEY, (byte) 1),
        // AES_v2 omits salt and iterations since it doesn't use PBKDF for key derivation, unlike V0 and V1.
        AES_V2(CryptoUtils.ENCRYPT_ALGORITHM_AES, 0, CryptoUtils.AES_256_KEY_LENGTH_BITS, null, PROPERTY_WLP_BASE64_AES_ENCRYPTION_KEY, (byte) 2);
        ...
        /** The wire-format version byte written/read at position 0 of every AES-encrypted payload. */
        public final byte wireByte;
        private final AtomicReference<SecretKeyResolver> secretKeyResolver;

        private KeyVersion(String alg, int iterations, int keyLength, byte[] salt, String resolverProperty, byte wireByte) {
            this.alg = alg;
        ...
        /**
         * Returns the {@link KeyVersion} whose {@link #wireByte} matches {@code b}.
         *
         * @param b the version byte read from the first byte of an AES-encrypted payload
         * @return the matching {@link KeyVersion}
         * @throws InvalidPasswordCipherException if no version has that wire byte (malformed ciphertext)
         */
        public static KeyVersion fromWireByte(byte b) throws InvalidPasswordCipherException {
            for (KeyVersion v : values()) {
                if (v.wireByte == b) {
                    return v;
                }
            }
            throw (InvalidPasswordCipherException) new InvalidPasswordCipherException(
                    "Unknown AES wire-format version byte: " + b).initCause(
                    new IllegalArgumentException("Unknown AES wire-format version byte: " + b));
        }
    ...
    /**
     * Returns the appropriate {@link SecretKeyResolver} and {@link KeyVersion} to use when
     * enciphering a password under the {@code {aes}} or {@code {aes-256}} algorithm tags.
     *
     * <p>Priority order:
     * <ol>
     *   <li>If {@code base64Key} is non-null, return an {@link KeyVersion#AES_V2} resolver that
     *       decodes the supplied base64 key string.</li>
     *   <li>Otherwise, if a custom (hardware) {@link SecretKeyResolver} is registered for
     *       {@link KeyVersion#AES_V2}, use that resolver.</li>
     *   <li>Otherwise, fall back to a {@link KeyVersion#AES_V1} resolver derived from
     *       {@code cryptoKey} (which may be {@code null} to use the configured property).</li>
     * </ol>
     *
     * @param base64Key a caller-supplied raw AES-256 key in base64, or {@code null}
     * @param cryptoKey a caller-supplied passphrase for PBKDF2 key derivation, or {@code null}
     * @return a two-element array {@code [KeyVersion, SecretKeyResolver]} — callers should
     *         capture as {@code Object[]}, or use the typed helper below
     */
