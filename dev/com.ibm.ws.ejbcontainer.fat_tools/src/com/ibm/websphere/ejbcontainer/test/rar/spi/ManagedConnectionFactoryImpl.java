...
//  05/29/03   jitang     LIDB2110.31 Provide J2C 1.5 resource adapter
//  09/19/03   swai       d177221     Since WAS has enabled security check,
//                                    so it's necessary to call java.security.doPrivilege
//                                    before calling getConnections. Else we will get
//                                    security permission exception when the test case
//                                    calls getConnections.
//  02/19/04   swai                   Not using doPrivileged call when get datasource to
//                                    see if it works. This works even without any special
//                                    security permission.
//  04/22/04   swai                   Set the credential object to null. If the Subject 
//                                    contains a null credential object, Test Resource 
//                                    Adapter should throw javax.resource.spi.SecurityException
//                                    according to JCA Spec, 9.1.8.2
//  05/14/04   alvinso                @alvinso.1 Add config properties for testing LI2110.86
//  06/04/04   swai                   Changed the debug message as it may throw NPE.
//  06/08/04   gburli     LIDB2110.97 Implemented ValidatingManagedConnectionFactory interface.
//                                    Modified the implementation of getInvalidConnections() for exception
//                                    scenario.
//  12/14/04   gburli     247285      Added Type 4 XA support on zOS
//  05/15/06   cjn        313344.1    Fix problems
//  07/14/06   djr		  377494	  Change logininTimeout from -1 to 0
//  ----------------------------------------------------------------
...
public class ManagedConnectionFactoryImpl implements ManagedConnectionFactory, Serializable, ResourceAdapterAssociation, ValidatingManagedConnectionFactory {
        ...
        try {
            ...
            if (is2Phase) {
                ...
                pconn = userName == null
                                ...
                                : ((XADataSource) dataSource).getXAConnection(userName, password);
            }
            else {
                if (userName == null)
                {
                ...
                pconn = userName == null
                                ? ((ConnectionPoolDataSource) dataSource).getPooledConnection()
                                : ((ConnectionPoolDataSource) dataSource).getPooledConnection(userName, password);
            }
    ...
    public PrintWriter getLogWriter() throws ResourceException {
        try {
            return dataSource == null ? null
                            : (is2Phase
                                            ? ((XADataSource) dataSource).getLogWriter()
                                            : ((ConnectionPoolDataSource) dataSource).getLogWriter());
        } catch (SQLException sqle) {
            throw new ResourceException(sqle.getMessage());
        }
    }
    ...
    public final int getLoginTimeout() throws ResourceException {
        if (loginTimeout != -1) {
            ...
            try {
                return is2Phase
                                ? ((XADataSource) dataSource).getLoginTimeout()
                                : ((ConnectionPoolDataSource) dataSource).getLoginTimeout();
            } catch (SQLException sqle) {
                throw new ResourceException(sqle.getMessage());
            }
    }
