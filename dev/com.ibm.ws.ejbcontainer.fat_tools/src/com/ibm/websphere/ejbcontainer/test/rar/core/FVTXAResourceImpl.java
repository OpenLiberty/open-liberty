...
//  12/18/03   swai       177221       Ken Lawrence requests to change the method
//                                     storeTransaction to no-op. The reason is
//                                     messageProvider.getTranLog has error as the
//                                     tranlog directory path was wrong, but somehow
//                                     when I fixed the TRA install problem when Java
//                                     2 security is on I fixed the invalid tranlog
//                                     path problem. But to play safe, storeTranscaction
...
import com.ibm.websphere.ejbcontainer.test.rar.message.MessageEndpointTestResults;
 ...
 * sendMessage(String...) in interfaces FVTMessageProvider or FVTBaseMessageProvider.
...
public class FVTXAResourceImpl implements XAResource {
    private final static String CLASSNAME = FVTXAResourceImpl.class.getName();
    ...
    /**
     * Constructor with an enumeration of Xids.
     */
    ...
    public void commit(Xid xid, boolean onePhase) throws XAException {
        svLogger.entering(CLASSNAME, "commit", new Object[] { xid, new Boolean(onePhase) });

        // Check whether the passed-in xid is in the vector or not. 
        // If not, throw an XAException
        if (!xids.contains(xid)) {
            svLogger.info("A non-existent XID is passed to the commit call");
        ...
        if (testResult != null) {
            svLogger.info("commit: setRaXaResourceCommit");
        ...
        if (sleepTimeInCommit > 0) {
            try {
                ...
                svLogger.info("commit - InterruptedException: " + ie);
            }
        }
    ...
    public void end(Xid xid, int flags) throws XAException {
        svLogger.entering(CLASSNAME, "end", new Object[] { xid, new Integer(flags) });
        ...
        // If not, throw an XAException
        if (!xids.contains(xid)) {
            svLogger.info("A non-existent XID is passed to the end call");
        ...
        if (sleepTimeInEnd > 0) {
            try {
                ...
                svLogger.info("end - InterruptedException: " + ie);
            }
        }
    ...
    public void forget(Xid xid) throws XAException {
        ...
        if (sleepTimeInForget > 0) {
            try {
                ...
                svLogger.info("forget - InterruptedException: " + ie);
            }
        }
     ...
     * by the target object is the same as the resource manager instance represented by
    ...
    public boolean isSameRM(XAResource xares) throws XAException {
        svLogger.entering(CLASSNAME, "isSameRM", new Object[] { this, xares });
        if (this == xares) {
            return true;
        }
            ...
            // todo: I should do some checking here. but I blindly return false here.
            return false;
        }
    }
    ...
    public int prepare(Xid xid) throws XAException {
        ...
        // If not, throw an XAException
        if (!xids.contains(xid)) {
            svLogger.info("A non-existent XID is passed to the prepare call");
        ...
        if (sleepTimeInPrepare > 0) {
            try {
                ...
                svLogger.info("prepare - InterruptedException: " + ie);
            }
        }
    ...
    public Xid[] recover(int arg0) throws XAException {
        svLogger.entering(CLASSNAME, "recover", new Integer(arg0));
        ...
        if (sleepTimeInRecover > 0) {
            try {
                ...
                svLogger.info("recover - InterruptedException: " + ie);
            }
        }
    ...
    public void rollback(Xid xid) throws XAException {
        svLogger.entering(CLASSNAME, "rollback", xid);
        ...
        // If not, throw an XAException
        if (!xids.contains(xid)) {
            svLogger.info("A non-existent XID is passed to the rollback call");
        ...
        if (testResult != null) {
            svLogger.info("rollback: setRaXaResourceRollback");
        ...
        if (sleepTimeInRollback > 0) {
            try {
                ...
                svLogger.info("rollback - InterruptedException: " + ie);
            }
        }
    ...
    public void start(Xid xid, int flags) throws XAException {
        svLogger.entering(CLASSNAME, "start", new Object[] { xid, new Integer(flags) });
        ...
        if (testResult != null) {
            svLogger.info("start: setRaXaResourceEnlisted");
        ...
        if (sleepTimeInStart > 0) {
            try {
                ...
                svLogger.info("start - InterruptedException: " + ie);
            }
        }
    ...
    public void setEndpoint(MessageEndpointWrapper endpoint) {
        svLogger.entering(CLASSNAME, "setEndpoint", endpoint);
