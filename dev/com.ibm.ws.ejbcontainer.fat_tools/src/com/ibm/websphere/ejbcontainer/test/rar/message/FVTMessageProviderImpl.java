...
//  08/02/03    swai       d174256      add createWork(FVTMessage, String) to provide
//                                      a key to the hash map of Work instances. This
//                                      is for getTestResults().
//  08/19/03    swai       d174592      Rename releaseTestResultsWork to releaseWork
...
//                                      called.
//  09/18/03    swai       d177221      Add getEISStatus to get the status of the EIS.
//                                      Add a instance variable, EISStatus to indicate
//                                      the status of the EIS (messageProvider).
//                                      The status is either STATUS_OK or STATUS_FAIL.
//  11/17/03    swai                    Add 2 sendMessageWaitNestedWork methods for nested
//                                      work test cases
//                                      When test cases called setResourceAdapter, a new
//                                      workDispatcher instance is created. This creates a
//                                      new EISTimer instance every time. Since the provider
//                                      instance will exist unless RA is shutdown. So, it is
//                                      not necessary to create a new workDispatcher everytime
//                                      test case calls setResourceAdapter. Only need to create
//                                      workDispatcher once.
//  11/20/03    swai                    Modified executeConcurrentWork methods such that they
//                                      take state and waitTime parameters. They are used to
//                                      to implement the return of executeConcurrentWork only
//                                      after all works have reached the desire state.
//  12/01/03    swai                    signalEISFailure and signalEISRecover will be notified
//                                      by the EISTimer directly via syncEISTimerObj rather than
//                                      putting the thread asleep. Also added syncEISTimerObj
//                                      for synchronization. This object has to be serializeable
//                                      because messageProvider, as an Admin Object, has to be
//                                      serializable. So syncEISTimerObj has to be a serializable
//                                      class, such as Integer.
//  12/16/03    swai                    TRA doesn't have access to call System.getProperty when 
//                                      Java 2 security is on. The static variable tranLog used 
//                                      to call System.getProperty("WAS_HOME"). This won't work
//                                      when Java 2 security is on. In addition, "WAS_HOME" 
//                                      is not one of the PropertyNames from System. So the new
//                                      way to construct tranLog is through was_install_root and
//                                      pathSeparator.
//                                      As a result, tranLog is constructed when getTranLog is called
//                                      and if tranLog is null. This means the default value of tranLog
//                                      is <was_install_root>/tranlog.
...
//
//  05/19/04    alvinso                 @alvinso.2 Return null in getXAResources
//  --------------------------------------------------------------------
...
import com.ibm.websphere.ejbcontainer.test.rar.core.FVTAdapterHelper;
import com.ibm.websphere.ejbcontainer.test.rar.core.FVTAdapterImpl;
import com.ibm.websphere.ejbcontainer.test.rar.work.FVTComplexWorkImpl;
import com.ibm.websphere.ejbcontainer.test.rar.work.FVTGeneralWorkImpl;
import com.ibm.websphere.ejbcontainer.test.rar.work.FVTWorkDispatcher;
import com.ibm.websphere.ejbcontainer.test.rar.work.FVTWorkImpl;
