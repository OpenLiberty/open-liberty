...
//  08/11/03    jitang      d173989     FVT: Timeout exception thrown from sendMessageWait
//  08/13/03    swai        d174256     Modify the workXXX so that deliverComplexMessage will throw
//                                      WorkException
//  08/18/03    jitang      d174592     Eat exception from endpoint.release
//  08/20/03    jtiang      d174742     Close the timing window b/w scheduleWork and waiting for status
//  09/21/03    swai        d177221     Add a thread for the timer object for each associated EIS 
//                                      (MessageProvider) so that the WorkDispatcher (RA) checks the 
//                                      status of the messageProvider.
//                                      Check the status of the messageProvider before delivering
//                                      messages. If the status is STATUS_FAIL, do not send any
//                                      messages.
//                                      Need to modify deliverXXXMessage to add the status check at the
//                                      beginning.
//  09/22/03    swai        d177221     Also, need to reset the timer when the dispatcher receives calls
//                                      from messageProvider. deliverXXXMessage is modified.
//  09/23/03    swai        d177221     Need to create 2 list of imported trans:
//                                      one contains all active trans and one contains all indoubt trans.
//                                      When deliverXXXMessage is called, a importeTrans object is created
//                                      and add this into the active trans list. When xaTermWrapper.prepare()
//                                      is called, then the importeTrans object will be removed from the 
//                                      active trans list and put into indoubt trans list.
//  09/24/03    swai        d177221     Add rollbackAllActiveTrans when the timer detects EIS failure and when
//                                      EIS calls rollback.
//  09/26/03    swai        d177221     Need to reset rollback status. Reason is a new test case after the 
//                                      rollback is unsuccessful due to EIS failure. So reset it.
//	10/29/03    rruvinsk                Because we deal with two Xid implementations, XidImpl and WebSphere's,
//                                      matching up the contents of IndoubtTransSet (XidImpl's) and the indoubt
//                                      transactions returned by WAS necessitated creating setContainsTrans, which
//                                      goes through the contents of a set and compares each Xid in it to the Xid
//                                      question using methods specified by the Xid interface.
//  11/16/03    swai        d177221     Modify executeConcurrentWork: Need to add transactions to activeTransSet.                                        
//  11/17/03    swai                    Add deliveryNestedWorkMessage and addWork for nested Work test cases.
//  11/19/03    swai                    If thread is not null and if it's not timed out, that means the
//                                      syn block above is unlocked by the work has reached the specified
//                                      work state (received the workXXX event) rathern than by the timer.
//                                      If we let the timer continues to run then ultimately the timedout
//                                      variable will be set to true and this will make a false impression
//                                      that the work submitted is timed out. So it is necessary to destroy
//                                      the timer thread. To be safe, set the timedout to false.
//  11/20/03    swai                    We will use a local variable for syncObj instead of instance variable.
//                                      In this case we can ensure the sync block is unlocked by the appropriate
//                                      timer or the workEvent.
//  11/20/03    swai                    Modified the constructor of the timer object so that it takes in a work
//                                      instance. This allows the timer object to check if the work has reached
//                                      the desired state.  If this is not true and when the timer awaked from the
//                                      sleep, this means we have to set timedout to true in the timer object.
...
//                                      works reached the desired state (refer to FVTConcurrentWorkImpl), or if 
//                                      the timer expires.        
//  11/26/03    swai                    No need to check if the nestedWork is in completed state or not. Coz
//                                      user may want nestedWork to return in state other than workCompleted state.
//  12/03/03    swai                    Add the xid to the ActiveTransSet before the work is submitted. It is because
//                                      the transaction is active even the work is failed to submit. If the work is failed
//                                      to submit, it's still necessary to rollback the active trans. Add the xid to
//                                      to the set will allow the XATerminatorWrapper to rollback the active trans.
//                                      Also, set xid to executionContext object no matter what xid is.
//  12/04/03    swai                    Due to M3_TransactionInflow_4.testSubmitAfterPreparedCMTRequired, it is possible
...
//  12/17/03    swai                    EJBContainer has some test cases which use MDB to rollback a transaction. Since
//                                      When the message is sent the xid is added to active Trans set. However, TRA doesn't
//                                      know that the transaction is rollbacked. Therefore when the xid with the same
//                                      global tran id is used later, duplicate xid occurs when message is sent. This failed
//                                      Sean's test cases (MsgEndpoint_TxTest.test3a and MsgEndpoint_CDTest.test2b_2 and 4a)
//                                      The solution is TRA eats the exception but shows it on the trace.
...
//  02/19/04    swai                    Since the pass-in xid may be from recover() call, the format Id and the 
...
import com.ibm.websphere.ejbcontainer.test.rar.core.EISTimer;
import com.ibm.websphere.ejbcontainer.test.rar.core.FVTAdapterImpl;
import com.ibm.websphere.ejbcontainer.test.rar.core.FVTXAResourceImpl;
import com.ibm.websphere.ejbcontainer.test.rar.core.XATerminatorWrapper;
import com.ibm.websphere.ejbcontainer.test.rar.core.XidImpl;
import com.ibm.websphere.ejbcontainer.test.rar.message.FVTMessage;
import com.ibm.websphere.ejbcontainer.test.rar.message.FVTMessageProvider;
import com.ibm.websphere.ejbcontainer.test.rar.message.FVTMessageProviderImpl;
...
import com.ibm.websphere.ejbcontainer.test.rar.message.MessageWaitTimeoutException;
import com.ibm.websphere.ejbcontainer.test.rar.message.TextMessageImpl;
