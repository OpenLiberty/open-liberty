...
public abstract class FVTWorkImpl implements Work, ResourceAdapterAssociation {
    ...
    /**
     * notfication status. When the work reaches the notification status, a notification
    ...
    public void setState(int state) {
        ...
        if (state == notificationState) {
            ...
            if (tc.isDebugEnabled())
                Tr.debug(tc, "setState", "work: " + this + " has reached desired state, " + state);
        }
    }
    ...
    public boolean hasWorkBeenInState(int state) {
        if (tc.isEntryEnabled())
            Tr.entry(tc, "hasWorkBeenInState", new Object[] {
                                                              this, AdapterUtil.getWorkStatusString(state) });
