...
public class TestUtilImpl implements TestUtil {
    ...
    public MergeProcessor getMergeProcessor() {
        ...
        try {
            ...
            f.set(mergeProcessor, new OpenAPIModelWalkerImpl());
        } catch (Exception e) {
    ...
    public String serialize(OpenAPI model, Format format) throws IOException {
        return OpenApiSerializer.serialize(model, getSmallryeFormat(format));
    }

    @Override
    public OpenAPI parse(InputStream is, Format format) throws IOException {
        return OpenApiParser.parse(is, getSmallryeFormat(format));
    }
