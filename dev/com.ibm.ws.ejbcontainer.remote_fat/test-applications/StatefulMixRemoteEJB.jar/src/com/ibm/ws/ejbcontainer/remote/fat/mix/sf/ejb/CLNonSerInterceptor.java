...
public class CLNonSerInterceptor {
    private final static String CLASSNAME = CLNonSerInterceptor.class.getName();
    ...
    private void postActivate(InvocationContext inv) {
        PassivationTracker.addMessage("CLNonSerInterceptor.postActivate:" + strValue);
        svLogger.info("CLNonSerInterceptor.postActivate: " + strValue);
        try {
            inv.proceed();
        } catch (Exception e) {
    ...
    private void prePassivate(InvocationContext inv) {
        PassivationTracker.addMessage("CLNonSerInterceptor.prePassivate:" + strValue);
        svLogger.info("CLNonSerInterceptor.prePassivate: " + strValue);
        try {
            inv.proceed();
        } catch (Exception e) {
    ...
    private Object aroundInvoke(InvocationContext inv) throws Exception {
        ...
        svLogger.info("CLNonSerInterceptor.aroundInvoke: " + m.getName());
        if (m.getName().equals("interceptorStart")) {
            Object[] parms = inv.getParameters();
            if (parms.length > 0) {
                strValue = (String) parms[0];
                PassivationTracker.addMessage("CLNonSerInterceptor.aroundInvoke:" + strValue);
            }
