...
public class ElementImpl implements SOAPElement, SOAPBodyElement {
    ...
    protected static final Logger log = Logger.getLogger(LogDomainConstants.SOAP_IMPL_DOMAIN,
    ...
    public ElementImpl(
        ...
        elementQName = new QName(uri, getLocalPart(qualifiedName), getPrefix(qualifiedName));
        soapDocument.register(this);
    }
    ...
    public SOAPElement addChildElement(String localName) throws SOAPException {
        ...
        Name name = (nsUri == null || nsUri.isEmpty()) ? NameImpl.createFromUnqualifiedName(localName) : NameImpl.createFromQualifiedName(localName, nsUri);
            ...
            while (currentAncestor != null &&
                ...
                if (currentAncestor instanceof ElementImpl) {
                    ...
                    if (((Element) currentAncestor).hasAttributeNS(
                        ...
                        String uri = ((Element) currentAncestor).getAttributeNS(
                                                                                XMLNS_URI, "xmlns");
                        if ("".equals(uri))
                            return null;
                        else {
...
//            return uri;
    ...
    public SOAPElement addChildElement(SOAPElement element) throws SOAPException {
        ...
        if ((SOAPConstants.URI_NS_SOAP_ENVELOPE).equals(elementURI)
            ...
            if ("Envelope".equalsIgnoreCase(localName) ||
                ...
                throw new SOAPExceptionImpl("Cannot add fragments which contain elements "
    ...
    protected SOAPElement createElement(Name name) {
        ...
        if (isNamespaceQualified(name)) {
            return (SOAPElement) getSoapDocument().createElementNS(
                                                                   name.getURI(),
                                                                   name.getQualifiedName());
        } else {
            return (SOAPElement) getSoapDocument().createElement(name.getQualifiedName());
        }
    }
    ...
    protected SOAPElement createElement(QName name) {
        ...
        if (isNamespaceQualified(name)) {
            ...
            return (SOAPElement) getSoapDocument().createElement(getQualifiedName(name));
        }
    }
    ...
    protected SOAPElement addCDATA(String text) throws SOAPException {
        org.w3c.dom.Text cdata = getOwnerDocument().createCDATASection(text);
        addNode(cdata);
        return this;
    }

    protected SOAPElement addText(String text) throws SOAPException {
        org.w3c.dom.Text textNode = getOwnerDocument().createTextNode(text);
        addNode(textNode);
        return this;
    }
    ...
    protected Iterator<String> doGetNamespacePrefixes(final boolean deep) {
        return new Iterator<String>() {
            String next = null;
            String last = null;
            NamespaceContextIterator eachNamespace = getNamespaceContextNodes(deep);

            void findNext() {
                while (next == null && eachNamespace.hasNext()) {
                    String attributeKey = eachNamespace.nextNamespaceAttr().getNodeName();
    ...
    private boolean removeAttribute(String uri, String localName) {
        ...
        org.w3c.dom.Attr attribute = getAttributeNodeNS(nonzeroLengthUri, localName);
        if (attribute == null) {
            return false;
        }
...
//                (ElementImpl) createElement(NameImpl.copyElementName(element)));
            ...
            public Node next() {
                if (hasNext()) {
                    ...
                    if ((soapElement instanceof ElementImpl)
                        ...
                        last = ((ElementImpl) soapElement).convertToSoapElement(
                                                                                (Element) last);
    ...
    protected void setEncodingStyleNamespace(
        ...
        Name encodingStyleAttributeName = NameImpl.create(
                                                          "encodingStyle",
                                                          soapNamespacePrefix,
                                                          soapNamespace);
        ...
        return new Iterator<Name>() {
            ...
            public Name next() {
                ...
                if (prefix.length() == 0) {
                    ...
                    Name attributeName = NameImpl.createFromQualifiedName(
                                                                          currentName,
