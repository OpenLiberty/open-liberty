 ...
 * @module resources/stats/_mbeanUtils
 * 
...
define(
    ...
    function __isLibertyFeatureActive(server, libertyFeatureShortName) {
      ...
      // Two code paths are needed here to support the Administrator and Reader role. The Reader role is only
      // able to do GET requests (and not POST requests) so there are two execution flows:
      // 1. Execution on the Admin Center host server (via a GET request to query the local server's features)
      //    (this is the execution flow needed by all users, both Administrator and Reader roles)
      // 2. Execution on a remote target, which relies on the HTTP POST operation and the MBean routing rules.
      // First, check to see if we have already looked up the feature. If so get the cached status and skip query.
      // Otherwise, query the feature services (either Admin Center API or the OSGi service) to see if the feature
        ...
        if (server && server.type && server.type === 'server' && !server.isAdminCenterServer) {
          this.getMBeans(server, 'osgi.core:type=serviceState,framework=org.eclipse.osgi', true).then(lang.hitch(this, function(response) {
            if (response.length > 0) {
              ...
              // add resolve call, otherwise the deferred will not be fulfilled
              outerDeferred.resolve(false, true);
            }
          }));
        } else {
            request.get('/ibm/api/adminCenter/v1/utils/feature/'+libertyFeatureShortName, {handleAs : 'json', sync: true}).then(
              function(data) {
                var libertyFeatureFound = data.provisioned;
