...
public class ZipCachingIntrospectorOutput {
    ...
    private String introspectorDescription, entryCacheSettings, zipReaperSettings, handleIntrospection, zipEntryCache,
            zipReaperValues;
    ...
    public ZipCachingIntrospectorOutput(InputStream in) throws Exception {
        try ( BufferedReader zipCachingReader = new BufferedReader(new InputStreamReader(in)) ) {
            ...
            // handleintrospections + zipEntryCache =
            // activeAndCachedZipFileHandles
            ...
            // check if DISABLED if so then set the rest of the variables to be
            // null and return
            {
            ...
            if ( currentLine.equals("Zip File Data [ completed ]") ) {
                ...
                // should not get to the EOF without reaching one of the two
                // lines above
                throw new RuntimeException("Reached EOF unexpectedly for Introspector Output");
            } else {
                // should not get to else since output should have 1 of the
                // first 2 conditionals
    ...
    public List<String> getZipHandleArchiveNames() {
        if ( getActiveAndCachedZipFileHandles() == null )
            ...
            // pattern will capture the file name at the end of the path
            // including comma and directory seperator
            ...
            for ( String line : getActiveAndCachedZipFileHandles().split("\n") ) {
                if ( line.contains("ZipFileHandle@0x") && hasAValidGroup(line, p) ) {
                    // match for the archive name and remove the comma and
                    // directory seperator
    ...
    private static void addZipFileDataIntrospection(List<String> aggregateList, String rawIntrospection) {
        ...
        if ( rawIntrospection != null ) {
            ...
            for ( String introspectLine : rawIntrospection.split("\n") ) {
                // pattern for the first line of the ZipFileData.introspect()
                // output
