...
public class FATSuite {
	 ...
	 */
	/*
	 * always add servlet-5.0 to enable EE9 in the op which had no feature versions
	 * to swap out to enable EE9
	 */
	@ClassRule
	public static RepeatTests repeat = RepeatTests.with(FeatureReplacementAction.NO_REPLACEMENT().liteFATOnly())
			...
			.andWith(new SecurityTestFeatureEE9RepeatAction().notOnWindows().removeFeatures(REMOVE).addFeatures(INSERT)
					.alwaysAddFeature("servlet-5.0").fullFATOnly())
			.andWith(new SecurityTestFeatureEE10RepeatAction().notOnWindows().removeFeatures(REMOVE).addFeatures(INSERT)
					.alwaysAddFeature("servlet-6.0").fullFATOnly())
			.andWith(new SecurityTestFeatureEE11RepeatAction().notOnWindows().removeFeatures(REMOVE).addFeatures(INSERT)
					.alwaysAddFeature("servlet-6.1").fullFATOnly());
