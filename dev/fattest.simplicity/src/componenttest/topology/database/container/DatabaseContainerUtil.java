...
/**
 * This is a generic utility class to provide methods to help in the use of
 * test containers when enlisting a fat bucket in database rotation.
 */
public class DatabaseContainerUtil {
    ...
    private static void modifyDataSourceProps(DataSource ds, ServerConfiguration cloneConfig, LibertyServer serv,
        ...
        try {
            serverName = cont.getContainerIpAddress();
        } catch (UnsupportedOperationException e) {
        }

        try {
            port = Integer.toString(cont.getFirstMappedPort());
        } catch (UnsupportedOperationException e) {
        }

        try {
            databaseName = cont.getDatabaseName();
        } catch (UnsupportedOperationException e) {
        }
