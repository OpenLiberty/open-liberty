...
public class AuthFilterElementTest extends CommonTest {
    ...
    public void testAuthFilterRequestUrlMatchTypeToNotContain() throws Exception {
        ...
        testHelper.reconfigureServer("malformedrequestURLmatchType.xml", name.getMethodName(), startMsgs, AuthFilterConstants.RESTART_SERVER);
    ...
    public void testNoAuthFilterElementSpecified() throws Exception {
        testHelper.reconfigureServer("authFilterRefNoElementSpecified.xml", name.getMethodName(), AuthFilterConstants.NO_MSGS, AuthFilterConstants.RESTART_SERVER);
        Map<String, String> headers = getCommonHeadersWithSSOCookie(ssoCookie);
        successfulLtpaServletCall(headers);
     ...
     *
     * Expected results:
     * - Should have a validation message for authFilterRef and all will use SSO LTPA.
     */
    ...
    public void testAuthFilterRefNonExistent() throws Exception {
        ...
        testHelper.reconfigureServer("authFilterRefNonExistent.xml", name.getMethodName(), startMsgs, AuthFilterConstants.RESTART_SERVER);
     ...
     *
     * Expected results:
     * - Successfully access a protected resource.
     */

    @Test
    public void testAuthFilterUsingRequestUrl_Host_UserAgent_Valid() throws Exception {
        testHelper.reconfigureServer("authFilterUsingRequestUrl_Host_UserAgent_Valid.xml", name.getMethodName(), AuthFilterConstants.NO_MSGS,
                                     AuthFilterConstants.RESTART_SERVER);
        Map<String, String> headers = getCommonHeadersWithSSOCookie(ssoCookie);
    ...
    public void testRequestUrlNotContain_HostContains_UserAgentNotContain() throws Exception {
        ...
        testHelper.reconfigureServer("requestUrlNotContain_HostContains_UserAgentNotContain.xml", name.getMethodName(), AuthFilterConstants.NO_MSGS,
                                     AuthFilterConstants.RESTART_SERVER);
        Map<String, String> headers = getCommonHeadersWithSSOCookie(ssoCookie);
        successfulLtpaServletCall(headers);
        testHelper.checkForMessages(true, AUTHENTICATION_FILTER_PROCESSED_CWWKS4358I);
     ...
     * - Restart the server and make a servlet call.
     ...
     *
     * Expected results:
     * - We should fail to get access a protected resource (FirFox sent expecting IE).
     */
     ...
     *
     * Expected results:
     * - We should fail to get access a protected resource on null token
    ...
    public void testSkipForUnprotectedURI_False_SendNegotiateHeader() throws Exception {
        ...
        testHelper.reconfigureServer("skipForUnprotectedURIFalse.xml", name.getMethodName(), AuthFilterConstants.NO_MSGS, AuthFilterConstants.RESTART_SERVER);
        Map<String, String> headers = testHelper.setTestHeaders("Negotiate " + null, AuthFilterConstants.IE, TARGET_SERVER, null);
        String response = unsuccessfulServletCall("/UnprotectedSimpleServlet", headers, AuthFilterConstants.DONT_IGNORE_ERROR_CONTENT, expectedStatusCode);
     ...
     *
     * Expected results:
     * - Successfully access protected resource.
     */

    @Test
    public void testUseOfWebAppAttribute_Contains() throws Exception {
        testHelper.reconfigureServer("webAppContainsBasicAuth_webAppNotContainFormLogin.xml", name.getMethodName(), AuthFilterConstants.NO_MSGS,
                                     AuthFilterConstants.RESTART_SERVER);
    ...
    public void testUseOfWebAppAttribute_Equals() throws Exception {
        testHelper.reconfigureServer("webAppEquals.xml", name.getMethodName(), AuthFilterConstants.NO_MSGS, AuthFilterConstants.RESTART_SERVER);
        commonSuccessfulLtpaServletCall(ssoCookie);
        testHelper.checkForMessages(true, AUTHENTICATION_FILTER_PROCESSED_CWWKS4358I);
    }
     ...
     * - Restart the server and check log results for validation of configuration.
     * - We will also invoke servlet with BasicAuth credentials.
     *
     * Expected results:
     * - We shouldn't be able to access resource.
     */

    @Test
    public void testUseOfWebAppAttribute_NotContain() throws Exception {
        testHelper.reconfigureServer("webAppNotEquals.xml", name.getMethodName(), AuthFilterConstants.NO_MSGS, AuthFilterConstants.RESTART_SERVER);
     ...
     *
     * Expected results:
     * - Successfully access protected resource, for all in contain list
     ...
     * - Make a servlet call, by default "basicauth" web app is used.
     * - Create a url which contains "basicauth" web app.
     ...
     * - Authentication to "basicauth" should fail, resulting in a 401.
     */
    ...
    public void testAuthFiltersMultipleElements_For_RemoteAddress_Host_WebApp() throws Exception {
        ...
        testHelper.reconfigureServer("authFiltersMultipleElements_RemoteAddress_Host_WebApputhFilterMultipleRequestUrl.xml", name.getMethodName(),
                                     AuthFilterConstants.NO_MSGS, AuthFilterConstants.RESTART_SERVER);

        commonSuccessfulLtpaServletCall(ssoCookie);

        // Access a protected resource, it is expected to be unsuccessful since the web app must not contain 'basicauth'
        String url = "http://" + myServer.getHostname() + ":" + myServer.getHttpDefaultPort() + "/basicauth" + AuthFilterConstants.SIMPLE_SERVLET;
        myClient.accessWithHeaders(url, 401, createCommonHeaders(), AuthFilterConstants.IGNORE_ERROR_CONTENT, AuthFilterConstants.DONT_HANDLE_SSO_COOKIE);
    ...
    public void testRemoteAddressIPUsingWildcard() throws Exception {
        ...
        testHelper.reconfigureServer("remoteAddress_Ip_Wildcard.xml", name.getMethodName(), AuthFilterConstants.NO_MSGS, AuthFilterConstants.RESTART_SERVER);
    ...
    public void testRemoteAddressIPRange() throws Exception {
        ...
        testHelper.reconfigureServer("remoteAddress_Ip_GreaterThan_Valid.xml", name.getMethodName(), AuthFilterConstants.NO_MSGS, AuthFilterConstants.RESTART_SERVER);
        // It is expected a remote address ip greater than the specified in the server.xml file,
        // since our local ip 127.0.0.1 is greater than 127.0.0.0 the servlet call must be successful
        commonSuccessfulLtpaServletCall(ssoCookie);

        testHelper.reconfigureServer("remoteAddress_Ip_GreaterThan_Invalid.xml", name.getMethodName(), AuthFilterConstants.NO_MSGS, AuthFilterConstants.RESTART_SERVER);
        // It is expected a remote address ip greater than the specified in the server.xml file,
        // since our local ip 127.0.0.1 is less than 127.0.0.2 the servlet call must be unsuccessful
        commonUnsuccessfulLtpaServletCall(ssoCookie);

        testHelper.reconfigureServer("remoteAddress_Ip_LessThan_Valid.xml", name.getMethodName(), AuthFilterConstants.NO_MSGS, AuthFilterConstants.RESTART_SERVER);
        // It is expected a remote address ip less than the specified in the server.xml file,
        // since our local ip 127.0.0.1 is less than 127.0.0.2 the servlet call must be successful
        commonSuccessfulLtpaServletCall(ssoCookie);

        testHelper.reconfigureServer("remoteAddress_Ip_LessThan_Invalid.xml", name.getMethodName(), AuthFilterConstants.NO_MSGS, AuthFilterConstants.RESTART_SERVER);
        // It is expected a remote address ip less than the specified in the server.xml file,
        // since our local ip 127.0.0.1 is greater than 127.0.0.0 the servlet call must be unsuccessful
        commonUnsuccessfulLtpaServletCall(ssoCookie);
    }
     ...
     * - Restart the server and check log results for validation of configuration.
     * - Try to access a servlet with valid header which includes email.
     *
     * Expected results:
     * - Successfully access protected resource.
     */
     ...
     * - Restart the server and check log results for validation of configuration.
     * - Try to access a servlet with valid header. The header will have a valid email.
     *
     * Expected results:
     * - Successfully access protected resource.
     */
     ...
     * - Set server xml file to have a filter with an email element configured to equals.
     * - Restart the server and check log results for validation of configuration.
     * - Try to access a servlet with an valid header. The header will have an invalid email.
     *
     * Expected results:
     * - Unable to access protected resource.
     */
