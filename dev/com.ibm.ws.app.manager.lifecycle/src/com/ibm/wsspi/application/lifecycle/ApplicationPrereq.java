 ...
 *     <pre>
 * package com.acme;
 * {@code @Component}(configurationPolicy = REQUIRE)
 ...
 *       </li>
 *       <li>
 *         The OCD must include an AD with a unique application prereq id:
 *         <pre>
 * {@code <AD id="application.prereq.id"}
 *      name="internal"
 *      description="internal use only"
 *      ibm:final="true"
 *      type="String"
 *      default="${id}"
 *      ibm:unique="application.prereq.id"
 * {@code />}
 *         </pre>
 *         Defaulting this to <code>"${id}"</code> will make it assume the value of the id attribute,
 *         provided one is defined. The <code>ibm:unique</code> tells the configuration processing in
 *         Liberty to check that the value is unique across the server configuration.
 *         This should use the index name specified in {@link #APP_PREREQ_ID}.
 *
 *       </li>
 *       <li>
...
public interface ApplicationPrereq {
    /**
     * The string used by config processing for uniqueness checking.
     * Config elements with metatype ADs with <code>ibm:unique="application.prereq.id"</code>
     * will be checked for uniqueness against each other.
     */
    String APP_PREREQ_ID = "application.prereq.id";
}
