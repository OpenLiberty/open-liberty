...
class JAXBContextInitializer extends ServiceModelVisitor {
    ...
    JAXBContextInitializer(ServiceInfo serviceInfo,
        ...
        //                        Map<String, Object> unmarshallerProperties) { Liberty change: removed parameter
        super(serviceInfo);
        this.classes = classes;
        this.typeReferences = typeReferences;
        //this.unmarshallerProperties = unmarshallerProperties;  Liberty change: removed assignment of the parameter removed
    }
    ...
    private void checkForAdapter(Class<?> clazz, Annotation[] anns) {
        if (anns != null) {
            for (Annotation a : anns) {
                if (XmlJavaTypeAdapter.class.isAssignableFrom(a.annotationType())) {
                    inspectTypeAdapter(((XmlJavaTypeAdapter)a).value()); // Liberty change: added this lines instead below lines
                    //Liberty change: removed 4 lines below
        ...
        if (xjta != null) {
            inspectTypeAdapter(xjta.value()); // Liberty change: added this lines instead below lines
            //Liberty change: removed 4 lines below
        ...
        //Liberty change: removed lines below
    ...
    private void addType(Type cls, boolean allowArray) {
        if (cls instanceof Class) {
            //Liberty change: removed 3 lines below
            ...
            //if (!parameterizedType.getRawType().equals(Enum.class)) { Liberty change: removed if clause
            for (Type t2 : parameterizedType.getActualTypeArguments()) {
                if (shouldTypeBeAdded(t2, parameterizedType)) {
                    addType(t2);
                }
            }
            //} Liberty change: removed if clause
    ...
    void addClass(Class<?> cls) {
        ...
        // Liberty change: removed else if clause below
        // else if (cls.getName().startsWith("java.")
        //     || cls.getName().startsWith("javax.")) {
        //     return;
        } else {
            cls = JAXBUtils.getValidClass(cls); // Liberty change: removed class type definition Class<?> from Class<?> cls
            // Liberty change: removed else if clause below
            // if (cls == null
            //     && ReflectionUtil.getDeclaredConstructors(cls).length > 0
            //     && !Modifier.isAbstract(cls.getModifiers())) {
            //     if (LOG.isLoggable(Level.INFO)) {
            //         LOG.info("Class " + cls.getName() + " does not have a default constructor which JAXB requires.");
            //     }
            //     //there is no init(), but other constructors
            //     Object factory = createFactory(cls, ReflectionUtil.getDeclaredConstructors(cls)[0]);
    ...
    // Liberty change: added inspectTypeAdapter method below
    ...
    private void walkReferences(Class<?> cls) {
        ...
        //XmlAccessType accessType = Utils.getXmlAccessType(cls);  Liberty change: removed line
        // Liberty change: added 5 lines below
        ...
        if (accessType != XmlAccessType.PROPERTY) {   // only look for fields if we are instructed to
            ...
            for (Field f : fields) {
                if (isFieldAccepted(f, accessType)) {
                    // Liberty change: removed 8 lines below
    ...
    static boolean isFieldAccepted(Field field, XmlAccessType accessType) {
        ...
        // Liberty change: Modifier.isStatic(field.getModifiers()) || is added to below if statement
    ...
    static boolean isMethodAccepted(Method method, XmlAccessType accessType) {
        ...
        if (method.getReturnType() == Void.class  // Liberty change:  || method.getReturnType() == Void.TYPE is removed from if clause
            || method.getParameterTypes().length != 0
            || (method.getDeclaringClass().equals(Throwable.class)
            && !("getMessage".equals(method.getName())))) { // Liberty change: || !(method.getName().startsWith("get") || method.getName().startsWith("is")) are removed from if clause
            return false;
        }
        // Liberty change: added 5 lines below
        ...
        if ((setter != null)  // Liberty change: if (setter.isAnnotationPresent(XmlTransient.class) is merged with parent if clause
              && (setter.isAnnotationPresent(XmlTransient.class)
              || !Modifier.isPublic(setter.getModifiers()))) {
                return false;
            //} Liberty change: end of merged if clause
        }
        // Liberty change: removed else if clause below
     ...
     */
    private static boolean checkJaxbAnnotation(Annotation[] annotations) { // Liberty change: private is added to the method instead of default
