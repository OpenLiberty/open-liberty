...
//------------------------------------------------------------------------------
/** Data container for SMF interval data related to a WebApplication. */
public class WebApplicationIntervalSection extends SmfEntity {
    ...
    /** Servlet interval sections loaded with this WebApplication. */
    public ServletIntervalSection[] m_servletIntervalSections;
     ...
     */
    public WebApplicationIntervalSection(SmfStream aSmfStream, int aRequestedVersion) throws UnsupportedVersionException, UnsupportedEncodingException {

        super(aRequestedVersion);

        m_name = aSmfStream.getString(256, SmfUtil.UNICODE);

        m_loadedServletN = aSmfStream.getInteger(4);
        ...
        m_servletIntervalSections = new ServletIntervalSection[m_servletN];
        for (int sX = 0; sX < m_servletN; ++sX) {
            m_servletIntervalSections[sX] = new ServletIntervalSection(aSmfStream, aRequestedVersion);
        }

    } // WebApplicationIntervalSection(...)
     ...
     *                           where this instance of WebApplicationIntervalSection originates from.
     */
    public void dump(SmfPrintStream aPrintStream, int aTripletNumber) {

        aPrintStream.println("");
        aPrintStream.printKeyValue("Triplet #", aTripletNumber);
        aPrintStream.printlnKeyValue("Type", "WebApplicationIntervalSection");
        ...
        aPrintStream.println("");
        aPrintStream.printlnKeyValue("# Servlets loaded", m_loadedServletN);
        ...
        for (int sX = 0; sX < m_servletN; ++sX) {
            m_servletIntervalSections[sX].dump(aPrintStream, aTripletNumber, sX + 1); // @L2C
        } // for
