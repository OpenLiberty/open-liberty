...
public class TestZosRequestLoggingSmfUnMappedUserid implements SMFFilter {

    public TestZosRequestLoggingSmfUnMappedUserid() {
    ...
    public void processRecord(SmfRecord record) {
        ...
        if (rec.m_libertyRequestInfoSection.m_userid.equals("RSTUSR2")) {
            if (rec.m_libertyRequestInfoSection.m_mappedUserid.equals("")) {
                if (basicRc == 0) {
                    if (rec.m_libertyRequestInfoSection.m_lengthRequestUri == 48) {
                        if (rec.m_libertyRequestInfoSection.m_requestUri.equals("/mappedCredentialsSync/unMappedMvsUseridSyncTest")) {
                            looksGood = 1;
                        } else {
                            smf_printstream.println("TestZosRequestLoggingSmfUnauth request URI is not /mappedCredentialsSync/unMappedMvsUseridSyncTest. Request URI "
                                                    + rec.m_libertyRequestInfoSection.m_requestUri);
                        }
                    } else {
                        smf_printstream.println("TestZosRequestLoggingSmfUnauth request URI length is not 48. Request URI length "
                                                + rec.m_libertyRequestInfoSection.m_lengthRequestUri);
                    }
                } else {
                    smf_printstream.println("TestZosRequestLoggingSmfUnMappedUserid basic validation failed. rc=" + basicRc);
                }
            } else {
                smf_printstream.println("TestZosRequestLoggingSmfUnMappedUserid mapped user id has a value when it should not. " + rec.m_libertyRequestInfoSection.m_mappedUserid
                                        + " end");
            }

        } else {
            smf_printstream.println("TestZosRequestLoggingSmfUnMappedUserid user id not RSTUSR2. " + rec.m_libertyRequestInfoSection.m_userid + " end");
        }
    ...
    public void processingComplete() {
        if (looksGood == 1)
            smf_printstream.println("TestZosRequestLoggingSmfUnMappedUserid looks good.");
    }
