...
public class TestBatchSMFLoggingPartitioned implements SMFFilter {

    public TestBatchSMFLoggingPartitioned() {
    ...
    public void processRecord(SmfRecord record) {
        ...
        if (!libertyBatchIdentificationSection.m_jobName.equals("BonusPayoutJob.partitioned")) {

            smf_printstream.println("ERROR Batch Identification Section job name is incorrect. Expecting BonusPayoutJob.partitioned but got "
                                    + libertyBatchIdentificationSection.m_jobName);
            rc = 1;
        }
    ...
    public void processingComplete() {
        if (looksGood == 1)
            smf_printstream.println("TestBatchSMFLoggingPartitioned looks good.");
    }
