...
public class TestZosRequestLoggingSmfUserDataString implements SMFFilter {

    public TestZosRequestLoggingSmfUserDataString() {
                ...
                if (uds.m_dataLength == userDataBytes.length()) {
                    ...
                    try {
                        ...
                        if (stringData.equals(userDataBytes)) {
                            ...
                            smf_printstream.println("TestZosRequestLoggingSmfUserDataString userdata is wrong. expected data=" + userDataBytes + "But data=" + stringData);
                        }
                    } catch (UnsupportedEncodingException e) {
                        smf_printstream.println("TestZosRequestLoggingSmfUserDataString formatting error." + e.getStackTrace());
                    }
                } else {
                    smf_printstream.println("TestZosRequestLoggingSmfUserDataString userdata length is wrong. expected length=" + userDataBytes.length() + "But length="
                                            + uds.m_dataLength);
                }
            } else {
                smf_printstream.println("TestZosRequestLoggingSmfUserDataString userdata type is not 1. User data type=" + uds.m_dataType);
            }
        } else {
            smf_printstream.println("TestZosRequestLoggingSmfUserDataString userdata count is not 1. User data count=" + userDataCount);
        }
    }
    ...
    public void processingComplete() {
        if (looksGood == 1)
            smf_printstream.println("TestZosRequestLoggingSmfUserDataString looks good.");
    }
