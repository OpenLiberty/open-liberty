...
public class TestZosRequestLoggingSmfUserDataBytesServletForward implements SMFFilter {

    public TestZosRequestLoggingSmfUserDataBytesServletForward() {
            ...
            if (uds.m_dataType == 2) {
                if (uds.m_dataLength == 8) {
                    ...
                    if ((firstInt == 1) && (secondInt == 9)) {
                        ...
                        smf_printstream.println("TestZosRequestLoggingSmfUserDataBytesServletForward userdata is wrong. expected first int to be 1 and second int to be 9 but first int="
                                                + firstInt + " and second int=" + secondInt);
                    }
                } else {
                    smf_printstream.println("TestZosRequestLoggingSmfUserDataBytesServletForward userdata length is wrong. expected length=8 But length=" + uds.m_dataLength);
                }
            } else {
                smf_printstream.println("TestZosRequestLoggingSmfUserDataBytesServletForward userdata type is not 2. User data type=" + uds.m_dataType);
            }

            if (looksGood == 1) {
                looksGood = 0;
                UserDataSection uds1 = UDS[1];
                if (uds1.m_dataType == 3) {
                    if (uds1.m_dataLength == 8) {
                        SmfStream data = new SmfStream(uds1.m_data);
                        int firstInt = data.getInteger(4);
                        int secondInt = data.getInteger(4);
                        if ((firstInt == 3) && (secondInt == 9)) {
                            if (rc == 0) {
                                looksGood = 1;
                            }
                        } else {
                            smf_printstream.println("TestZosRequestLoggingSmfUserDataBytesServletForward 2nd userdata is wrong. expected first int to be 3 and second int to be 9 but first int="
                                                    + firstInt + " and second int=" + secondInt);
                        }
                    } else {
                        smf_printstream.println("TestZosRequestLoggingSmfUserDataBytesServletForward 2nd userdata length is wrong. expected length=8 But length="
                                                + uds1.m_dataLength);
                    }
                } else {
                    smf_printstream.println("TestZosRequestLoggingSmfUserDataBytesServletForward 2nd userdata type is not 3. User data type=" + uds1.m_dataType);
                }
            }
        } else {
            smf_printstream.println("TestZosRequestLoggingSmfUserDataBytesServletForward userdata count is not 2. User data count=" + userDataCount);
        }
    }
    ...
    public void processingComplete() {
        if (looksGood == 1)
            smf_printstream.println("TestZosRequestLoggingSmfUserDataBytesServletForward looks good.");
    }
