...
public class AsymmetricBindingHandler extends AbstractBindingBuilder {
    ...
    private void doSignBeforeEncrypt() {
        try {
            ...
            if (isRequestor() && initiatorWrapper != null) {
                doSignature(initiatorWrapper, sigs, attached); //~Failure here
                doEndorse();
            } else if (!isRequestor()) {
                //confirm sig
        ...
        if (!abinding.isSignatureProtection()) {
            ...
            if (encryptionToken.isDerivedKeys() && !secondEncrParts.isEmpty()
                ...
                try {
                    ...
                    encryptForRef((WSSecEncrypt) encrBase, secondRefList, secondEncrParts);
        ...
        if (recToken != null && recToken.getToken() != null && encrParts.size() > 0) {
            ...
            if (encrToken.isDerivedKeys()) {
                ...
                try {
                    ...
                    //Encrypt, get hold of the ref list and add it
                    if (externalRef) {
                        Element refList = encryptForRef(encr, null, encrParts);
                        insertBeforeBottomUp(refList);
                    } else {
                        Element refList = encryptForRef(encr, null, encrParts);
                    ...
                    policyNotAsserted(recToken, pae.getException().getMessage());
                }
            }
        }
        return null;
    ...
    private Element encryptForRef(WSSecEncrypt encr, Element secondRefList, List<WSEncryptionPart> encrParts) throws PrivilegedActionException {
        return AccessController.doPrivileged(new PrivilegedExceptionAction<Element>() {
            @Override
            public Element run() throws WSSecurityException {
                return encr.encryptForRef(secondRefList, encrParts);
            }
        });
    }
    ...
    private void doSignature(TokenWrapper wrapper, List<WSEncryptionPart> sigParts, boolean attached) throws WSSecurityException, SOAPException {
        ...
        if (sigToken.isDerivedKeys()) {
            ...
            dkSign.setDerivedKeyLength(abinding.getAlgorithmSuite().getSignatureDerivedKeyLength() / 8);
            ...
            try {
                ...
                if (abinding.isTokenProtection()) {
                    if (bstElement != null) {
                        WSEncryptionPart bstPart = new WSEncryptionPart(bstElement.getAttributeNS(WSConstants.WSU_NS, "Id"));
