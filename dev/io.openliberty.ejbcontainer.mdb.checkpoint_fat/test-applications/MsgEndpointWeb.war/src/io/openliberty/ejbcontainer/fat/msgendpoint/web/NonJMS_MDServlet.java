...
public class NonJMS_MDServlet extends FATServlet {
    ...
    public void testAuthDataUpdatesDuringRestoreAAS() throws Exception {
        ...
        // construct a FVTMessage
        FVTMessage message = new FVTMessage();
        message.addTestResult("EndpointRestoreAuthDataNonJMS", 51);

        // Add a FVTXAResourceImpl for this delivery.
        message.addXAResource("EndpointRestoreAuthDataNonJMS", 51, new FVTXAResourceImpl());

        // Add a option B transacted delivery to another instance.
        // Endpoint is Non-JMS, CMT+required
        Method m = javax.jms.MessageListener.class.getMethod("onMessage", new Class[] { javax.jms.Message.class });
        message.addDelivery("EndpointRestoreAuthDataNonJMS", FVTMessage.BEFORE_DELIVERY, m, 51);
        message.add("EndpointRestoreAuthDataNonJMS", "message1", m, 51);
        message.addDelivery("EndpointRestoreAuthDataNonJMS", FVTMessage.AFTER_DELIVERY, null, 51);
        System.out.println(message.toString());
        baseProvider.sendDirectMessage(deliveryID, message);
