...
public class JMS_MDServlet extends FATServlet {
    ...
    public void testJMSAuthDataUpdatesDuringRestoreAAS() throws Exception {
        String deliveryID = "MD_test7a";

        prepareTRA();

        // construct a FVTMessage
        FVTMessage message = new FVTMessage();
        message.addTestResult("EndpointRestoreAuthDataJMS", 61);

        // Add a FVTXAResourceImpl for this delivery.
        message.addXAResource("EndpointRestoreAuthDataJMS", 61, new FVTXAResourceImpl());

        // Add a option A
        message.add("EndpointRestoreAuthDataJMS", "message1", 61);
        // message.addRelease("BMTJMS", 61);
