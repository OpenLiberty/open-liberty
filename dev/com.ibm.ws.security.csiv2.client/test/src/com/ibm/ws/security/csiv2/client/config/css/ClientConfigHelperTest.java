...
public class ClientConfigHelperTest {
    ...
    private ClientContainerConfigHelper clientConfigHelper;
    ...
    public void setUp() throws Exception {
        ...
        clientConfigHelper = new ClientContainerConfigHelper(DEFAULT_ALIAS);
    }

    private Map<String, Object> createDefaultInstances() {
        Map<String, Object> properties = new HashMap<String, Object>();
        properties.put("clientPolicy.0.config.referenceType", "com.ibm.ws.security.csiv2.clientContainer.clientPolicyCSIV2");
        ...
        properties.put("clientPolicy.0.layers.0.authenticationLayer.0.config.referenceType", "com.ibm.ws.security.csiv2.clientContainer.clientPolicyAuthenticationLayer");
        properties.put("clientPolicy.0.layers.0.authenticationLayer.0.mechanisms", GSSUP);
        properties.put("clientPolicy.0.layers.0.authenticationLayer.0.establishTrustInClient", "Supported");

        properties.put("clientPolicy.0.layers.0.transportLayer.0.config.referenceType", "com.ibm.ws.security.csiv2.clientContainer.transportLayer");
...
//        properties.put("clientPolicy.0.layers.0.transportLayer.0.sslRef", DEFAULT_ALIAS);
    ...
    private void addConfigFromServerXMLWithEmptyLayersElement(Map<String, Object> properties) {
        // CSIv2
        properties.put("clientPolicy.1.config.referenceType", "com.ibm.ws.security.csiv2.clientContainer.clientPolicyCSIV2");
        properties.put("clientPolicy.1.stateful", false);
        // Layers
        properties.put("clientPolicy.1.layers.0.config.referenceType", "com.ibm.ws.security.csiv2.clientContainer.clientPolicyLayers");
    }
    ...
    private void assertDefaultConfig(CSSCompoundSecMechListConfig compoundSecMechList) {
        assertEquals("There must be only one compound sec mechs in the list.", 1, compoundSecMechList.size());
    ...
    private void putTransportLayerProperties() {
        properties.put("clientPolicy.1.layers.0.transportLayer.0.config.referenceType", "com.ibm.ws.security.csiv2.clientContainer.transportLayer");
        properties.put("clientPolicy.1.layers.0.transportLayer.0.sslEnabled", true);
    }

    /**
     * Test authentication layer configured with EstablishTrustInClient: Required and mechanism: GSSUP
     * Verifying required value is set to EstablishTrustInClient.value
    ...
    public void testAuthenticationLayerGSSUP() throws Exception {
        ...
        properties.put("clientPolicy.1.layers.0.authenticationLayer.0.establishTrustInClient", "Required");
        properties.put("clientPolicy.1.layers.0.authenticationLayer.0.establishTrustInClient", "Required");
        properties.put("clientPolicy.1.layers.0.authenticationLayer.0.user", USER);
        properties.put("clientPolicy.1.layers.0.authenticationLayer.0.password", new SerializableProtectedString("abcde".toCharArray()));
        properties.put("clientPolicy.1.layers.0.authenticationLayer.0.realm", REALM);
        ...
        assertTrue("The authentication layer must be enabled with type CSSGSSUPMechConfigDynamic.",
                   cssConfig.getMechList().mechAt(0).getAs_mech() instanceof CSSGSSUPMechConfigDynamic);
        /* Validate user id, realm name are set.(password cannot be tested) */
        assertTrue("The user name must be set.", cssConfig.getMechList().mechAt(0).getAs_mech().toString().contains(USER));
        assertTrue("The realm name must be set.", cssConfig.getMechList().mechAt(0).getAs_mech().toString().contains(REALM));

    }

    /**
     * Test authentication layer configured with EstablishTrustInClient: Required and mechanism: LTPA
     * Verifying required value is set to EstablishTrustInClient.value
     * Verifying failure since it's not supported.
     *
     * @throws Exception
     */
    @Test
    public void testAuthenticationLayerLTPA() throws Exception {
        ...
        /* Validate EstablishTrustInClient: required. */
        assertEquals("The authentication layer must not require EstablishTrustInClient due to incompatible as mech.",
                     0, cssConfig.getMechList().mechAt(0).getAs_mech().getRequires());
        ...
        assertTrue("Expected message was not logged", outputMgr.checkForMessages("CWWKS9700E:"));
    }
    ...
    public void testAuthenticationLayerNullMechanism() throws Exception {
        ...
        assertTrue("The authentication layer must be enabled with type CSSNULLASMechConfig.",
                   cssConfig.getMechList().mechAt(0).getAs_mech() instanceof CSSNULLASMechConfig);
        assertTrue("Expected message was not logged", outputMgr.checkForMessages("CWWKS9701W:"));
    }
    ...
    private void putAuthLayerProperties(String[] mechanism) {
        properties.put("clientPolicy.1.layers.0.authenticationLayer.0.config.referenceType", "com.ibm.ws.security.csiv2.clientContainer.clientPolicyAuthenticationLayer");
     ...
     * @throws Exception
     */
    //TODO duplicate is not eliminated, count is 2, there is a bug?  defect 172667
