...
public class ResourceMethodRegistry implements Registry
{
   ...
   protected void processMethod(ResourceFactory rf, String base, ResourceLocator method)
   {
      ...
      if (method instanceof ResourceMethod)
      {
         ...
         // Liberty Change Start: Debug logging for invoker creation
         System.out.println("      Created ResourceMethodInvoker");
         System.out.println("      Invocation method: " + invoker.getMethod().getName());
         System.out.println("      Invocation method class: " + invoker.getMethod().getDeclaringClass().getName());
         System.out.println("      HTTP methods from ResourceMethod: " + ((ResourceMethod)method).getHttpMethods());
         System.out.println("      Registering with path: " + fullpath);
         
         try {
            if (widerMatching)
               rootNode.addInvoker(fullpath, invoker);
            else {
               System.out.println("      Calling root.addInvoker(classExpression=" + classExpression + ", fullpath=" + fullpath + ")");
               root.addInvoker(classExpression, fullpath, invoker);
               System.out.println("      root.addInvoker() completed successfully");
            }
            statisticsController.register(invoker);
            System.out.println("      Registration complete for: " + invoker.getMethod().getName());
         } catch (Exception e) {
            System.out.println("      ERROR during registration: " + e.getClass().getName() + ": " + e.getMessage());
            e.printStackTrace();
            throw e;
         }
         // Liberty Change End
      ...
      else
      {
         ...
         // Liberty Change Start: Debug logging for locator creation
         System.out.println("      Created ResourceLocatorInvoker");
         System.out.println("      Registering with path: " + fullpath);
         // Liberty Change End
   ...
   public ResourceInvoker getResourceInvoker(HttpRequest request)
   {
      ...
      // Liberty Change Start - Debug logging for request matching
      String path = request.getUri().getPath();
      String method = request.getHttpMethod();
      System.out.println("=== getResourceInvoker DEBUG ===");
      System.out.println("  Request path: " + path);
      System.out.println("  Request HTTP method: " + method);
      System.out.println("  widerMatching: " + widerMatching);
      // Liberty Change End
      
      try {
         ResourceInvoker invoker;
         if (widerMatching) {
            invoker = rootNode.match(request, 0);
         } else {
            invoker = root.match(request, 0);
         }
         
         // Liberty Change Start - Debug logging for match result
         if (invoker != null) {
            System.out.println("  Match found!");
            System.out.println("  Invoker class: " + invoker.getClass().getName());
            if (invoker instanceof ResourceMethodInvoker) {
               ResourceMethodInvoker rmi = (ResourceMethodInvoker) invoker;
               System.out.println("  Invoker method: " + rmi.getMethod().getName());
               System.out.println("  Invoker method class: " + rmi.getMethod().getDeclaringClass().getName());
            }
         } else {
         ...
         System.out.println("  Exception during matching: " + e.getMessage()); // Liberty Change
         throw e;
      }
      finally {
