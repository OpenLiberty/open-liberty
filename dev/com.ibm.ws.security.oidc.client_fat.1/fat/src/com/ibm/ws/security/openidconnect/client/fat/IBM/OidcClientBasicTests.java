...
public class OidcClientBasicTests extends GenericOidcClientTests {
    ...
    public void OidcClientTestUseOAuth() throws Exception {
        ...
        updatedTestSettings.setTestURL(updatedTestSettings.getTestURL().replace(Constants.DEFAULT_SERVLET, "simple/oauthClient"));
    ...
    public void OidcClientTestUseOAuthOpenidInScope() throws Exception {
        ...
        updatedTestSettings.setTestURL(updatedTestSettings.getTestURL().replace(Constants.DEFAULT_SERVLET, "simple/openidInScopeOauthClient"));

        List<validationData> expectations = validationTools.add401Responses(Constants.LOGIN_USER);
        expectations = validationTools.addMessageExpectation(testRPServer, expectations, Constants.LOGIN_USER, Constants.MESSAGES_LOG, Constants.STRING_CONTAINS, "Client messages.log should contain a message indicating that the ID Token is missing in the response", "CWWKS1712E.*client01.*ID token");
    ...
    public void OidcClientTestBadTokenEndpoint() throws Exception {
        ...
        TestSettings updatedTestSettings = testSettings.copyTestSettings();
        updatedTestSettings.setTestURL(updatedTestSettings.getTestURL().replace(Constants.DEFAULT_SERVLET, "simple/badTokenEndpoint"));
        ...
        expectations = validationTools.addMessageExpectation(testOPServer, expectations, Constants.LOGIN_USER, Constants.MESSAGES_LOG, Constants.STRING_CONTAINS, "Server messages.log should contain a message indicating that the OP received a request that was not valid.", Constants.MSG_RP_REQUEST_INVALID);
        expectations = validationTools.addMessageExpectation(testRPServer, expectations, Constants.LOGIN_USER, Constants.MESSAGES_LOG, Constants.STRING_CONTAINS, "Server messages.log should contain a message indicating that the RP is unable to contact the OP for client01", Constants.MSG_UNABLE_TO_CONTACT_OP);
    ...
    public void OidcClientTestBadAuthorizationEndpointOAuthError() throws Exception {
        ...
        TestSettings updatedTestSettings = testSettings.copyTestSettings();
        updatedTestSettings.setTestURL(updatedTestSettings.getTestURL().replace(Constants.DEFAULT_SERVLET, "simple/badAuthEndpoint"));
        ...
        expectations = validationTools.addMessageExpectation(testOPServer, expectations, expected404Step, Constants.MESSAGES_LOG, Constants.STRING_CONTAINS, "Did not get a message indicating bad endpoint in OP messages.log", Constants.MSG_RP_REQUEST_INVALID + ".*bad/OidcConfigSample");
    ...
    public void OidcClientBadAuthorizeEndpointWebContainerError() throws Exception {
        ...
        TestSettings updatedTestSettings = testSettings.copyTestSettings();
        updatedTestSettings.setTestURL(updatedTestSettings.getTestURL().replace(Constants.DEFAULT_SERVLET, "simple/webContainerBadAuthEndpoint"));

        String expected404Step = Constants.GET_LOGIN_PAGE;
        List<validationData> expectations = validationTools.getDefault404VDataExpectations(expected404Step);
        expectations = validationTools.addMessageExpectation(testOPServer, expectations, expected404Step, Constants.MESSAGES_LOG, Constants.STRING_CONTAINS, "Did not get a message indicating bad endpoint in OP messages.log", Constants.MSG_FILE_NOT_FOUND + ".*v10/endpoint/bad/");
    ...
    public void OidcClientTestBadIssuerIdentifierOPConfig() throws Exception {
        ...
        List<validationData> expectations = validationTools.add401Responses(Constants.LOGIN_USER);
        expectations = vData.addExpectation(expectations, Constants.GET_LOGIN_PAGE, Constants.RESPONSE_FULL, Constants.STRING_CONTAINS, "Did Not get the OpenID Connect login page.", null, Constants.LOGIN_PROMPT);
        expectations = validationTools.addMessageExpectation(testRPServer, expectations, Constants.LOGIN_USER, Constants.MESSAGES_LOG, Constants.STRING_CONTAINS, "Did not get a message in RP messages.log indicating the validation failed because the issuer specified did not match the provider.", "CWWKS1751E:.*/endpoint/bad/OidcBadIssuerConfigSample");
    ...
    public void OidcClientSSLTest_NoRPTrustStore() throws Exception {
        ...
        // Can't remove the reconfigServer in this case since we're changing the server wide ssl config
        // NOTE:  The rp reconfig will take longer since we're disabling ssl and there is a check in the reconfig logic that will wait for SSL to
        // restart if we detect that SSL has been stopped. - the new config will not enable SSL
        testRPServer.reconfigServer("rp_server_basic_ssl_no_truststore.xml", _testName, false, null);
    ...
    public void OidcClientSSLTest_BadRPTrustStore() throws Exception {
        ...
        // Reconfigure RP server with SSL settings
        // Can't remove the reconfigServer in this case since we're changing the server wide ssl config
        testRPServer.reconfigServer("rp_server_basic_ssl_bad_truststore.xml", _testName, false, null);
    ...
    public void OidcClientSSLTest_BadOPTrustStoreWithClientAuth() throws Exception {
        ...
        // Can't remove the reconfigServer in this case since we're changing the server wide ssl config
        testOPServer.reconfigServer("op_server_ssl_bad_truststore_clientAuth.xml", _testName, false, null);
    ...
    public void OidcClientTestUseJvmProps() throws Exception {
        ...
        expectations = validationTools.addMessageExpectation(testRPServer, expectations, Constants.LOGIN_USER, Constants.MESSAGES_LOG, Constants.STRING_CONTAINS, "Client messages.log should contain a message indicating that use of the proxy was attempted", MessageConstants.CWWKS1708E_CLIENT_FAILED_TO_CONTACT_PROVIDER);
