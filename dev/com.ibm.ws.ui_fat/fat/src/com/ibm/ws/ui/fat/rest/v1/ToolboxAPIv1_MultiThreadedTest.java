...
public class ToolboxAPIv1_MultiThreadedTest extends CommonMultiThreadedTest implements APIConstants {
    ...
    public ToolboxAPIv1_MultiThreadedTest() {
        super(c);
        url = API_V1_TOOLBOX;
    }
     ...
     */
    class AsyncGetToolbox extends LatchedRunnable {
        final int max;
        ...
        public void run() {
            for (int i = 0; (i < max) && (inThreadThrowable == null); i++) {
                try {
                    ...
                    Bookmark newTool = new Bookmark(name, baseTool.getURL(), baseTool.getIcon());
    ...
    /**
     * Attempts to add many tools concurrently. This is not a normally
     * anticipated flow, but we have to support it!
     */
    @Test
    public void modifyToolboxConcurrentAccess() throws Exception {
        // A set of tools that look similar. Name and version will be overridden
        Bookmark t1Tool = new Bookmark("willOverrideWithThreadId", "http://concurrencytest.com", "https://concurrencytest.com/favicon.png");
        Bookmark t2Tool = new Bookmark("willOverrideWithThreadId", "http://concurrencytest.com", "https://concurrencytest.com/favicon.png");
        Bookmark t3Tool = new Bookmark("willOverrideWithThreadId", "http://concurrencytest.com", "https://concurrencytest.com/favicon.png");
        Bookmark t4Tool = new Bookmark("willOverrideWithThreadId", "http://concurrencytest.com", "https://concurrencytest.com/favicon.png");
        ...
        // While things are changing, get the toolbox
        addThreads.add(new AsyncGetToolbox(THREAD_LOOP_COUNT));
        ...
        // While things are changing, get the toolbox
        deleteThreads.add(new AsyncGetToolbox(THREAD_LOOP_COUNT));
