 ...
 */
public class ApiAuthnEvent extends AuditEvent {
    static AtomicServiceReference<SecurityService> securityServiceRef = null;

    private static final TraceComponent tc = Tr.register(ApiAuthnEvent.class);

    @SuppressWarnings("unchecked")
    public ApiAuthnEvent() {
        set(AuditEvent.EVENTNAME, AuditConstants.SECURITY_API_AUTHN);
    ...
    public ApiAuthnEvent(HttpServletRequest req, AuthenticationResult authResult, Integer statusCode) {
        this();
        try {
            // add initiator
            ...
            if (TraceComponent.isAnyTracingEnabled() && tc.isDebugEnabled()) {
                Tr.debug(tc, "Internal error creating SessionLoginEvent", e);
            }
        }
    }
