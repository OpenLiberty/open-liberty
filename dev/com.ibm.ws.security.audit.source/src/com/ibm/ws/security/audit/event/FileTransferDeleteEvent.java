 ...
 */
public class FileTransferDeleteEvent extends AuditEvent {

    private static final TraceComponent tc = Tr.register(FileTransferDeleteEvent.class);
    private static AuditManager auditManager = new AuditManager();

    @SuppressWarnings("unchecked")
    public FileTransferDeleteEvent() {
        ...
        set(AuditEvent.EVENTNAME, AuditConstants.FILE_TRANSFER_DELETE);

    }

    public FileTransferDeleteEvent(Object request, Object resp) {
        this();
        try {
...
/*
 * String originalFileContents = auditManager.getOriginalFileContents();
 ...
 */
            ...
            if (statusCode == HttpServletResponse.SC_OK || statusCode == 204) {
                setOutcome("success");
                set(AuditEvent.REASON_CODE, HttpServletResponse.SC_OK);
            } else {
                setOutcome("failure");
            ...
            if (TraceComponent.isAnyTracingEnabled() && tc.isDebugEnabled()) {
                Tr.debug(tc, "Internal error creating ServerConfigEvent", e);
            }
        }
    }
