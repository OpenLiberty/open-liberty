 ...
 */
public class AuthenticationTerminateEvent extends AuditEvent {

    private static final TraceComponent tc = Tr.register(AuthenticationTerminateEvent.class);

    @SuppressWarnings("unchecked")
    public AuthenticationTerminateEvent() {
        set(AuditEvent.EVENTNAME, AuditConstants.SECURITY_AUTHN_TERMINATE);
    ...
    public AuthenticationTerminateEvent(HttpServletRequest req, AuthenticationResult authResult, Integer statusCode) {
        this();
        try {
            // add initiator
            ...
            if (AuditUtils.getJ2EEComponentName() != null) {
            ...
            else if (authResult.getAuditLogoutSubject() != null)
                set(AuditEvent.TARGET_CREDENTIAL_TOKEN, authResult.getAuditLogoutSubject().getPrincipals().iterator().next().getName());
            ...
            if (TraceComponent.isAnyTracingEnabled() && tc.isDebugEnabled()) {
                Tr.debug(tc, "Internal error creating AuthenticationTerminateEvent", e);
            }
        }
    }
