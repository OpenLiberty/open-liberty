...
public class AuditUtils {
        ...
        try {
            ...
            if ((e.getException()) instanceof com.ibm.websphere.servlet.session.UnauthorizedSessionRequestException) {
                if (!req.isRequestedSessionIdFromCookie()) {
                    sessionID = getPrivilegedSessionID(f_req, isGenerateNewSession());
                } else {
                    sessionID = getPrivilegedRequestedSessionID(f_req);
                }
            }
    ...
    /**
     * Get the isGenerateNewSession configuration value from the audit service
     *
     * @return true if new session generation is enabled, false otherwise (default is false if service is unavailable)
     */
    public static boolean isGenerateNewSession() {
        AuditService service = auditServiceRef != null ? auditServiceRef.getService() : null;
        return (service != null) ? service.isGenerateNewSession() : false;
    }
