 ...
 *******************************************************************************/
package com.ibm.ws.openapi31.privateapi;
...
public class PrivateOpenAPIHandlerServlet extends HttpServlet {

    private static final long serialVersionUID = 1L;
    
    private static final TraceComponent tc = Tr.register(PrivateOpenAPIHandlerServlet.class);
    ...
    protected void doGet(HttpServletRequest request,
        ...
        if ("GET".equals(request.getMethod())) {
            ...
            boolean isSuccess = getOASProviderAggregator(false).getDocumentation(request, compact,yaml,response);
            if(!isSuccess) {
                //OASProviderAggregator was deactivated, grab the new OASProviderAggregator (if available) and try again
                if (TraceComponent.isAnyTracingEnabled() && tc.isDebugEnabled()) {
                    Tr.debug(this, tc, "Couldn't get public and private API documentation. Retry after resetting the aggregator.");
                }
                getOASProviderAggregator(true).getDocumentation(request, compact,yaml,response);
            }
        }
    }
    ...
    private void setOASProviderAggregator() throws ServletException {
        ...
        if (TraceComponent.isAnyTracingEnabled() && tc.isDebugEnabled()) {
            Tr.debug(this, tc, "Found aggregator, oasProviderAggregator=" + oasProviderAggregator);
        }
