...
public class CodepageUtils {
    ...
    /**
     * @param bytes - A null-terminated string of ebcdic chars
     * 
     * @return The bytes converted to String format.
     */
    public static String ebcdicBytesToString(byte[] ebcdicBytes) {
        if (ebcdicBytes == null) {
            return "";
        }
        
        try {
            return new String(ebcdicBytes, 0, firstNull(ebcdicBytes), EBCDIC);
        } catch (UnsupportedEncodingException uee) {
            throw new RuntimeException(uee);
        }
    }
    ...
    /**
     * @return the index of the first null byte in the given byte array,
     *         or bytes.length if there are no null bytes.  If bytes is null,
     *         then returns 0.
     */
    protected static int firstNull(byte[] bytes) {
        if (bytes == null) {
            return 0;
        }
