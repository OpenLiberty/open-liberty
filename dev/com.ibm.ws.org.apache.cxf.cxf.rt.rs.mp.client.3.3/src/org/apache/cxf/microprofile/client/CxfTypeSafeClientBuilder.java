 ...
 * under the License.
 *
 * Liberty Change: This file comes from CXF 3.4.3 and the following is a snapshot of this file at that time:
 * https://github.com/apache/cxf/blob/c7a6024d9b66dd601acd5780d8551487ff0e4c78/rt/rs/microprofile-client/src/main/java/org/apache/cxf/microprofile/client/CxfTypeSafeClientBuilder.java
 */
...
public class CxfTypeSafeClientBuilder implements RestClientBuilder, Configurable<RestClientBuilder> {
    ...
    @Override
    public RestClientBuilder property(String key, @Sensitive Object value) {
        // Liberty Change Start - need to convert proxy password to ProtectedString
        if (JAXRSClientConstants.PROXY_PASSWORD.equals(key) && value != null &&
            !(value instanceof ProtectedString)) {
            configImpl.property(key, new ProtectedString(value.toString().toCharArray()));
        } else {
    ...
    public RestClientBuilder proxyAddress(String proxyHost, int proxyPort) {
        ...
        configImpl.property(JAXRSClientConstants.PROXY_HOST, proxyHost);
        configImpl.property(JAXRSClientConstants.PROXY_PORT, proxyPort);
        // Liberty Change End
        return this;
    }
