...
public class JMSContextServlet extends HttpServlet {
	...
	public void testcreateContextwithUser_TCP_SecOn(HttpServletRequest request,
		...
		if (exceptionFlag)
			throw new WrongException(
					"testcreateContextwithUser_TCP_SecOn failed");
	...
	public void testCreateObjectMessage_B_SecOn(HttpServletRequest request,
		...
		String serlzedObj = new StockObject().toString("TEST STOCK", 5400);
	...
	public void testCreateTextMessageStr_TCP_SecOn(HttpServletRequest request,
		...
		if (exceptionFlag)
			throw new WrongException(
					"testCreateTextMessageStr_TCP_SecOn failed");
		jmsContext.close();
	...
	public void testJMSCorrelationIDAsBytes_B_SecOn(HttpServletRequest request,
		...
		if (exceptionFlag)
			throw new WrongException(
					"testJMSCorrelationIDAsBytes_B_SecOn failed");
		jmsContext.close();
	...
	public void testJMSCorrelationIDAsBytes_TCP_SecOn(
		...
		if (exceptionFlag)
			throw new WrongException(
					"testJMSCorrelationIDAsBytes_TCP_SecOn failed");
		jmsContext.close();
	...
	public void testcreateBrowser_MessageSelector_B_SecOn(
		...
		if (exceptionFlag)
			throw new WrongException(
					"testcreateBrowser_MessageSelector_B_SecOn failed");
		jmsConsumer.close();
		jmsContext.close();
	}
	...
	public void testcreateBrowser_MessageSelector_TCP_SecOn(
		...
		if (exceptionFlag)
			throw new WrongException(
					"testcreateBrowser_MessageSelector_TCP_SecOn failed");
		jmsConsumer.close();
		jmsContext.close();
	}
	...
	public void testJMSProducerSendMessage_B_SecOn(HttpServletRequest request,
		...
		JMSProducer jmsProducer = jmsContext.createProducer()
				.setJMSCorrelationID("TestCorrelID").setJMSType("NewTestType")
				.send(queue2, msg);
		...
		jmsConsumer.receive(30000);
		if (!(numMsgs == 1
				&& jmsProducer.getJMSCorrelationID().equals("TestCorrelID") && jmsProducer
				.getJMSType().equals("NewTestType")))

			exceptionFlag = true;
		if (exceptionFlag)
			throw new WrongException(
					"testJMSProducerSendMessage_B_SecOn failed ");
		jmsConsumer.close();
		jmsContext.close();
	...
	public void testJMSProducerSendMessage_TCP_SecOn(
		...
		if (exceptionFlag)
			throw new WrongException(
					"testJMSProducerSendMessage_TCP_SecOn failed ");
		jmsConsumer.close();
		jmsContext.close();
	...
	public void testJMSProducerSendMessage_Topic_B_SecOn(
		...
		if (exceptionFlag)
			throw new WrongException(
					"testJMSProducerSendMessage_Topic_B_SecOn failed");
		jmsConsumer.close();
		jmsContext.close();
	...
	public void testJMSProducerSendMessage_Topic_TCP_SecOn(
		...
		if (exceptionFlag)
			throw new WrongException(
					"testJMSProducerSendMessage_Topic_TCP_SecOn failed ");
		jmsConsumer.close();
		jmsContext.close();
	...
	public void testJMSProducerSendTextMessage_B_SecOn(
		...
		JMSProducer jmsProducer = jmsContext.createProducer()
				.setJMSCorrelationID("TestCorrelID").setJMSType("NewTestType")
				.send(queue2, "This is the messageBody");
	...
	public void testJMSProducerSendTextMessage_InvalidDestinationTopic_B_SecOn(
		...
		if (!exceptionFlag)
			throw new WrongException(
					"testJMSProducerSendTextMessage_InvalidDestinationTopic_B_SecOn failed");
		jmsContext.close();
	...
	public void testJMSProducerSendTextMessage_InvalidDestinationTopic_TCP_SecOn(
		...
		if (!exceptionFlag)
			throw new WrongException(
					"testJMSProducerSendTextMessage_InvalidDestinationTopic_TCP_SecOn failed ");
		jmsContext.close();
	...
	public void testJMSProducerSendMapMessage_Topic_B_SecOn(
		...
		if (exceptionFlag)
			throw new WrongException(
					"testJMSProducerSendMapMessage_Topic_B_SecOn failed");
		jmsConsumer.close();
		jmsContext.close();
	...
	public void testJMSProducerSendMapMessage_Topic_TCP_SecOn(
		...
		boolean correctMapBody = jmsConsumer.receive(10000)
				.getBody(java.util.Map.class).containsValue(val);
		if (!(correctMapBody == true
				&& jmsProducer.getJMSCorrelationID().equals("TestCorrelID") && jmsProducer
				.getJMSType().equals("NewTestType")))
		...
		if (exceptionFlag)
			throw new WrongException(
					"testJMSProducerSendMapMessage_Topic_TCP_SecOn failed");
		jmsConsumer.close();
		jmsContext.close();
	}
	...
	public void testJMSProducerSendByteMessage_B_SecOn(
		...
		jmsProducer.setJMSCorrelationID("TestCorrelID")
				.setJMSType("NewTestType").send(queue2, content);
		...
		if (exceptionFlag)
			throw new WrongException(
					"testJMSProducerSendByteMessage_B_SecOn failed");
		jmsConsumer.close();
		jmsContext.close();
	...
	public void testJMSProducerSendByteMessage_TCP_SecOn(
		...
		if (exceptionFlag)
			throw new WrongException(
					"testJMSProducerSendByteMessage_TCP_SecOn failed ");
		jmsConsumer.close();
		jmsContext.close();
	}
	...
	public void testJMSProducerSendObjectMessage_Topic_B_SecOn(
		...
		producer.setJMSCorrelationID("TestCorrelID").setJMSType("NewTestType")
		...
		if (!(msgRecvd.equals(objBody)
				&& producer.getJMSCorrelationID().equals("TestCorrelID") && producer
				.getJMSType().equals("NewTestType")))
			exceptionFlag = true;

		if (exceptionFlag)
			throw new WrongException(
					"testJMSProducerSendObjectMessage_Topic_B_SecOn failed");
		jmsConsumer.close();
		jmsContext.close();
	...
	public void testJMSProducerSendObjectMessage_Topic_TCP_SecOn(
		...
		if (exceptionFlag)
			throw new WrongException(
					"testJMSProducerSendObjectMessage_Topic_TCP_SecOn failed ");
		jmsConsumer.close();
		jmsContext.close();
	}
	...
	public void testSetDisableMessageID_TCP_SecOn(HttpServletRequest request,
		...
		if (!exceptionFlag)
			throw new WrongException(
					"testSetDisableMessageID_TCP_SecOn failed ");
		jmsConsumer.close();
		jmsContext.close();
	...
	public void testSetDisableMessageTimestamp_B_SecOn(
		...
		if (exceptionFlag)
			throw new WrongException(
					"testSetDisableMessageTimestamp_B_SecOn failed ");
		jmsConsumer.close();
		jmsContext.close();
	...
	public void testSetDisableMessageTimestamp_TCP_SecOn(
		...
		if (exceptionFlag)
			throw new WrongException(
					"testSetDisableMessageTimestamp_TCP_SecOn failed ");
		jmsConsumer.close();
		jmsContext.close();
	}
	...
	public void testSetGetJMSReplyTo_B_SecOn(HttpServletRequest request,
		...
		TextMessage msg1 = jmsContRec
				.createTextMessage("testSetGetJMSReplyTo: Reply Msg");
	...
	public void testQueueConsumer_B_SecOn(HttpServletRequest request,
		...
		JMSConsumer jmsConsumer = jmsContext.createConsumer(queue2);
		TextMessage msg = jmsContext
				.createTextMessage("Expected text - testCreateJmsProducerAndSend_B_SecOn");

		jmsProducer.send(queue2, msg);

		TextMessage message = (TextMessage) jmsConsumer.receive(30000);
		if (!(message.getText()
				.equals("Expected text - testCreateJmsProducerAndSend_B_SecOn")))
			exceptionFlag = true;
		if (exceptionFlag)
			throw new WrongException(
					"testCreateJmsProducerAndSend_B_SecOn failed ");
		jmsConsumer.close();
		jmsContext.close();
	}
	...
	public void testQueueConsumer_TCP_SecOn(HttpServletRequest request,
		...
		JMSConsumer jmsConsumer = jmsContext.createConsumer(queue2);
		TextMessage msg = jmsContext
				.createTextMessage("testCreateJmsProducerAndSend_TCP_SecOn");

		jmsProducer.send(queue2, msg);

		TextMessage message = (TextMessage) jmsConsumer.receive(30000);
		if (!(message.getText()
				.equals("testCreateJmsProducerAndSend_TCP_SecOn")))
			exceptionFlag = true;
		if (exceptionFlag)
			throw new WrongException(
					"testCreateJmsProducerAndSend_TCP_SecOn failed ");
		jmsConsumer.close();
		jmsContext.close();
	...
	public void testCreateConsumerWithMsgSelectorTopic_B_SecOn(
		...
		TextMessage msg = jmsContext
				.createTextMessage("testCreateConsumerWithMsgSelectorTopic_B_SecOn");
		...
		if (exceptionFlag)
			throw new WrongException(
					"testCreateConsumerWithMsgSelectorTopic_B_SecOn failed ");
		jmsConsumer.close();
		jmsContext.close();
	...
	public void testCreateConsumerWithMsgSelectorTopic_TCP_SecOn(
		...
		TextMessage msg = jmsContext
				.createTextMessage("testCreateConsumerWithMsgSelectorTopic_TCP_SecOn");
		System.out
				.println("--testCreateConsumerWithMsgSelectorTopic_TCP_SecOn --msg1 --"
						+ msg);
		msg.setStringProperty("Month", "March");
		System.out
				.println("--testCreateConsumerWithMsgSelectorTopic_TCP_SecOn --msg2 --"
						+ msg);

		jmsProducer.send(topic, msg);
		TextMessage recmsg = (TextMessage) jmsConsumer.receive(10000);
		System.out
				.println("--testCreateConsumerWithMsgSelectorTopic_TCP_SecOn --msg3 --"
						+ recmsg);

		System.out
				.println("--testCreateConsumerWithMsgSelectorTopic_TCP_SecOn --msg4 --"
						+ recmsg.getStringProperty("Month").equals("March"));
		if (!recmsg.getStringProperty("Month").equals("March"))
			exceptionFlag = true;
		if (exceptionFlag)
			throw new WrongException(
					"testCreateConsumerWithMsgSelectorTopic_TCP_SecOn failed ");
		jmsConsumer.close();
		jmsContext.close();
	}
