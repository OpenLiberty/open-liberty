...
public class TestJaxWsTransportSecurityServlet extends HttpServlet {
	...
	private static final QName POJO_PORT_QNAME = new QName("http://ibm.com/ws/jaxws/transport/security/",
			"SayHelloPojoPort");
	private static final QName STATELESS_PORT_QNAME = new QName("http://ibm.com/ws/jaxws/transport/security/",
			"SayHelloStatelessPort");
	private static final QName SINGLETON_PORT_QNAME = new QName("http://ibm.com/ws/jaxws/transport/security/",
			"SayHelloSingletonPort");
	...
	private <T> T getAndConfigClient(HttpServletRequest req, Class<T> clazz) {
		...
		StringBuilder sBuilder = new StringBuilder(schema).append("://").append(host).append(":").append(port)
				.append(PROVIDER_CONTEXT_ROOT).append(requestPath);
	...
	private String createMessage(String userName) {
		StringBuilder sBuilder = new StringBuilder(
				"<soap:Envelope xmlns:soap=\"http://schemas.xmlsoap.org/soap/envelope/\">").append("<soap:Body>")
				.append("<ns2:sayHello xmlns:ns2=\"http://ibm.com/ws/jaxws/transport/security/\">").append("<arg0>")
				.append(userName).append("</arg0>").append("</ns2:sayHello>").append("</soap:Body>")
	...
	private String getThrowableMessage(Throwable origThrowable) {
		...
		Throwable tmp = null;
		do {
			twBuilder.append(origThrowable.getMessage()).append("\n");
			tmp = origThrowable;
