...
public class ZipFileReaper {
        ...
        public void run() {
            ...
            try {
                ...
                while ( true ) {
                    ...
                    // First make sure we are not over the max cache size
                    reaper.trimPending();
    ...
    /**
     * Reap pending closes if we have reached max cache size for either pending storage
     */
    void trimPending() {
        int maxCacheSize = getMaxCache();
        if (maxCacheSize != -1) {
            long closeAt = SystemUtils.getNanoTime();
            synchronized (pendingStorageLock) {
                while (pendingQuickStorage.size() > maxCacheSize) {
                    ZipFileData ripest = pendingQuickStorage.removeFirst();
                    fullyClose(ripest, closeAt, IS_NOT_SHUTDOWN_REAP);
                }
                while (pendingSlowStorage.size() > maxCacheSize) {
                    ZipFileData ripest = pendingSlowStorage.removeFirst();
                    fullyClose(ripest, closeAt, IS_NOT_SHUTDOWN_REAP);
                }
            }
        }
                        ...
                        if ( expireQuickly && (quickPendMin == 0) ) {
                            ...
                            synchronized (pendingStorageLock) {
                                ...
                                if ( expireQuickly ) {
                                    ...
                                    pendingQuickStorage.addLast(data);
                                    if (pendingQuickStorage.size() > getMaxCache()) {
                                        wakeReason = "Quick has reached max cache size";
                                        skipWakeReason = null;
                                    }
                                    // pendingQuickStorage.display();
                                } else {
                                    // pendingSlowStorage.display();
                                    pendingSlowStorage.addLast(data);
                                    if (pendingSlowStorage.size() > getMaxCache()) {
                                        wakeReason = "Slow has reached max cache size";
                                        skipWakeReason = null;
                                    }
                                    // pendingSlowStorage.display();
                                }
