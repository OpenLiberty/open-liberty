...
public class DecimalMaxValidatorForNumber implements ConstraintValidator<DecimalMax, Number> {
    ...
    public boolean isValid(Number value, ConstraintValidatorContext context) {
        ...
        if (value instanceof BigDecimal) {
            return ((BigDecimal) value).compareTo(maxValue) < comparator;
        }
        if (value instanceof BigInteger) {
            return (new BigDecimal((BigInteger) value)).compareTo(maxValue) < comparator;
        }
        return (new BigDecimal(value.doubleValue()).compareTo(maxValue)) < comparator;
    }
}
