...
import com.ibm.ws.jpa.pxml32.Persistence;
 ...
 * <persistence> stanza in a 3.2 version persistence.xml.
 * <p>
 *
 * This implementation wraps the JAXB generated class that represent a <persistence> stanza in a 3.2
 ...
 * <p>
 **/
public class JaxbPersistence32 extends JaxbPersistence {
    protected static final String SCHEMA_VERSION = "3.2";
    private static final String XSD_NAME = "persistence_3_2.xsd";

    private static final TraceComponent tc = Tr.register(JaxbPersistence32.class, JPA_TRACE_GROUP, JPA_RESOURCE_BUNDLE_NAME);

    /** The <persistence-unit>s found in this persistence.xml **/
    private List<JaxbPUnit> ivPUnits;

    public JaxbPersistence32(JPAPXml pxml) {
        super(pxml, "com.ibm.ws.jpa.pxml32", XSD_NAME);
    }
    ...
    void setResult(Object result) {
        ...
        if (TraceComponent.isAnyTracingEnabled() && tc.isDebugEnabled())
            Tr.debug(tc,
                     "JaxbPersistence32.setResult : persistence units : " + ((pUnits == null) ? "null" : pUnits.size()));

        if (pUnits != null) {
            ivPUnits = new ArrayList<JaxbPUnit>(pUnits.size());
            for (PersistenceUnit pUnit : pUnits) {
                ivPUnits.add(new JaxbPUnit32(pUnit));
            }
        } else {
