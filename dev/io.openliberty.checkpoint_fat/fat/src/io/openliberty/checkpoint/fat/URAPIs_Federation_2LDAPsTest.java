...
public class URAPIs_Federation_2LDAPsTest {
    ...
    public static void setUp() throws Exception {
        ...
        server.setCheckpoint(new CheckpointInfo(CheckpointPhase.AFTER_APP_START, false, (s) -> {
            ...
            try {
                // gathering info from bootstrap.properties first
                LDAPUtils.addLDAPVariables(server);
                Properties bootstrapProps = server.getBootstrapProperties();
                Map<String, String> envMap = new HashMap<>();
                // capture the bootstrap.properties values into the server.env for ldap settings
                for (Entry<Object, Object> entry : bootstrapProps.entrySet()) {
                    String bootStrapKey = (String) entry.getKey();
                    if (bootStrapKey.startsWith("ldap.server.")) {
                        String bootStrapValue = (String) entry.getValue();
                        // must use '_' instead of '.' for env keys
                        bootStrapKey = bootStrapKey.replace('.', '_');
                        envMap.put(bootStrapKey, bootStrapValue);
                    }
                }
    ...
    public void checkPasswordWithDuplicateLogOnId() throws Exception {
        ...
        assertNotNull("Two users with same userid should cause DuplicateLogonIdException", server.waitForStringInLog("CWIML4538E"));
        passwordChecker.checkForPasswordInAnyFormat(password);
    }
