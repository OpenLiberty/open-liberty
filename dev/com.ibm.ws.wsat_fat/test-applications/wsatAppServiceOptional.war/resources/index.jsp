<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%@ page import="com.ibm.ws.wsatAppServiceOptional.utils.CommonUtils"%>
...
<body>
<h1>Welcome to WS-AT Test Application from <%=request.getLocalAddr() %>:<%=request.getLocalPort() %></h1><h3> Made by Jordan 2015-12-16</h3> <br>
<h2><font color="green">URL Example: </font></h2>
<a href="http://localhost:9080/<%=CommonUtils.appName %>/ClientServlet" target="_blank">http://localhost:9080/<%=CommonUtils.appName %>/ClientServlet</a>?server1p=commit:http://localhost:9081&server2l=commit:http://localhost:9082
 <br> <br>
|---------------------------------------- README ----------------------------------------| <br>
| server1p=commit:http://localhost:9081<br>
| server2l=rollback:http://localhost:9082<br>
| client=commit<br>
| Server URL example: [server1p=commit:http://localhost:9081, won't request if null]<br>
| Server Operation type: [d: DIspatch, l:local wsdl, p:proxy, q:proxy with UOW local transaction, r:proxy and operation without Assertion or other: client proxy]
...
&nbsp;&nbsp;&nbsp;&nbsp;	<br>
&nbsp;&nbsp;&nbsp;&nbsp;    &lt;webApplication id=&quot;<%=CommonUtils.appName %>&quot; location=&quot;<%=CommonUtils.appName %>.war&quot; name=&quot;<%=CommonUtils.appName %>&quot;/&gt;<br>
...
<a href="http://localhost:9081/<%=CommonUtils.appName %>/ResultServlet?server=server1&method=init" target"_blank">http://localhost:9081/<%=CommonUtils.appName %>/ResultServlet?server=server1&method=init</a><br><br>
<b>Commit all transaction:</b> <br>
<a href="http://localhost:9080/<%=CommonUtils.appName %>/ClientServlet?server1=commit:http://localhost:9081&server2=commit:http://localhost:9082" target"_blank">http://localhost:9080/<%=CommonUtils.appName %>/ClientServlet?server1p=commit:http://localhost:9081&server2p=commit:http://localhost:9082</a><br><br>
<b>Other transaction:</b> <br>
<a href="http://localhost:9080/<%=CommonUtils.appName %>/ClientServlet?server1=rollback:http://localhost:9081&server2=exception:http://localhost:9082" target"_blank">http://localhost:9080/<%=CommonUtils.appName %>/ClientServlet?server1p=rollback:http://localhost:9081&server2p=exception:http://localhost:9082</a><br><br>
<b>Without client transaction:</b> <br>
<a href="http://localhost:9080/<%=CommonUtils.appName %>/ClientServlet?server1=countdb:http://localhost:9081&server2=countdb:http://localhost:9082&withouttrans=true" target"_blank">http://localhost:9080/<%=CommonUtils.appName %>/ClientServlet?server1=countdb:http://localhost:9081&server2=countdb:http://localhost:9082&withouttrans=true</a><br><br>
<b>Get db count:</b> <br>
<a href="http://localhost:9081/<%=CommonUtils.appName %>/ResultServlet?server=server1" target"_blank">http://localhost:9081/<%=CommonUtils.appName %>/ResultServlet?server=server1</a><br>
<br><br>
