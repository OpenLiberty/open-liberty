...
public class Entity implements com.ibm.ws.jpa.diagnostics.ormparser.entitymapping.IEntity {
    ...
    public Set<String> _getConverters() {
        ...
        final List<Convert> converters = this.getConvert();
        for (Convert c : converters) {
            retList.add(c.getConverter());
        }
    ...
    public Set<String> _getNamedEntityGraphClasses() {
        ...
        final List<NamedEntityGraph> negList = getNamedEntityGraph();
        if (negList.size() > 0) {
            for (NamedEntityGraph neg : negList) {
                final List<NamedSubgraph> subclassSubgraph = neg.getSubclassSubgraph();
                final List<NamedSubgraph> subgraph = neg.getSubgraph();

                for (NamedSubgraph ns : subclassSubgraph) {
                    retList.add(ns.getClazz());
                }

                for (NamedSubgraph ns : subgraph) {
                    retList.add(ns.getClazz());
                }
            }
        }
