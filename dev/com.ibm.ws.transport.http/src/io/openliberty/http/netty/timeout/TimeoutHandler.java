...
public class TimeoutHandler extends ChannelDuplexHandler {
    ...
    private void onTimeout(ChannelHandlerContext context) {
        switch (phase) {
            ...
            case READ:
                ...
                if (firstRequest) {
                    context.fireExceptionCaught(new SocketTimeoutException("The connection closed due to idle timeout"));
                } else {
    ...
    private static void send408AndClose(ChannelHandlerContext context) {
        if (TraceComponent.isAnyTracingEnabled() && tc.isDebugEnabled()) {
            Tr.debug(tc, "The connection closed due to idle timeout");
        }

        DefaultFullHttpResponse response = new DefaultFullHttpResponse(HttpVersion.HTTP_1_1, 
                    HttpResponseStatus.REQUEST_TIMEOUT, Unpooled.EMPTY_BUFFER);
