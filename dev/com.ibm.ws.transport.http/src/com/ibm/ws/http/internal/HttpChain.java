 ...
 */
public class HttpChain extends AbstractHttpChain implements ChainEventListener {
                ...
                if (validOldConfig) {
                    if (sameConfig) {
                        ...
                        if (state == ChainState.STARTED.val) {
                            ...
                            sameConfig = validateActivePort(oldConfig);
                            if (sameConfig) {
                                if (TraceComponent.isAnyTracingEnabled() && tc.isDebugEnabled()) {
    ...
    public synchronized void chainStarted(ChainData chainData) {
        ...
        final int port = getActivePort(cfg);
     ...
     */
    private boolean validateActivePort(ActiveConfiguration cfg) {
        try {
            return cfg.activePort == cfw.getListeningPort(chainName);
    ...
    private int getActivePort(ActiveConfiguration cfg) {
        if (cfg.configPort < 0)
            return -1;

        if (cfg.activePort == -1) {
            try {
                cfg.activePort = cfw.getListeningPort(chainName);
            } catch (ChainException ce) {
                cfg.activePort = -1;
            }
        }
        return cfg.activePort;
    }
