...
public class CustomLoginModule implements LoginModule {
    ...
    public boolean login() throws LoginException {
        System.out.println("Adding custom principals and credentials to subject.");

        return AccessController.doPrivileged(new PrivilegedAction<Boolean>() {
            @Override
            public Boolean run() {
                _subject.getPrincipals().add(new CustomPrincipal());
                _subject.getPublicCredentials().add(new CustomPublicCredential());
                _subject.getPrivateCredentials().add(new CustomPrivateCredential());
                return true;
            }
        });
    }
