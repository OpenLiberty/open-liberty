...
public class Social_BasicTests extends SocialCommonTest {
     ...
     * Expected Results:
     * <OL>
     * <LI>Should get the login page from Social. After entering a valid id/pw, we should
     ...
     * accessible through the UserProfile API. Since we invoked the protected resource, we should already be authenticated. This
     * calls a jsp that invokes the UserProfile.getUserInfo() API to check the userinfo.
     */
    ...
    public void Social_BasicTests_MainPath_withJwtSsoFeature() throws Exception {
        ...
        expectations = vData.addExpectation(expectations, lastStep, SocialConstants.RESPONSE_FULL, SocialConstants.STRING_MATCHES, "Did not find the expected JWT principal in the app response but should have.", null, jwtUserPrincipal);
     ...
     * Expected Results:
     * <OL>
     * <LI>Should get the login page from Social. After entering a valie id/pw, we should
     * receive access to the helloworld app
     * </OL>
     */
    @Test
    public void Social_BasicTests_InvalidUserPassword() throws Exception {
        ...
        //  Invoke the protected app, get the login page, put a bad pw in the login page and expect to get the login page again
        List<validationData> expectations = vData.addSuccessStatusCodesForActions(inovke_social_login_actions);
        expectations = vData.addExpectation(expectations, SocialConstants.INVOKE_SOCIAL_RESOURCE, Constants.RESPONSE_FULL, Constants.STRING_MATCHES, "Did not get to the Login page", null, updatedSocialTestSettings.getLoginPage());
        if (provider.equals(SocialConstants.TWITTER_PROVIDER)) {
            // Twitter will redirect to a different login page
            expectations = vData.addExpectation(expectations, perform_social_login, Constants.RESPONSE_FULL, Constants.STRING_CONTAINS, "Did NOT get to the Twitter Login only page", null, SocialConstants.TWITTER_LOGIN_TITLE);
        } else {
            if (provider.equals(SocialConstants.LINKEDIN_PROVIDER)) {
                expectations = vData.addExpectation(expectations, perform_social_login, Constants.RESPONSE_FULL, Constants.STRING_CONTAINS, "Did NOT get to the LinkedIn Sign-up page", null, SocialConstants.LINKEDIN_FAILED_LOGIN_TITLE);
            } else {
                expectations = vData.addExpectation(expectations, perform_social_login, Constants.RESPONSE_FULL, Constants.STRING_MATCHES, "Did not get to the Login page", null, updatedSocialTestSettings.getLoginPage());
            }
        }
        if (provider.equals(SocialConstants.LIBERTYOP_PROVIDER)) {
            expectations = validationTools.addMessageExpectation(testOPServer, expectations, perform_social_login, Constants.MESSAGES_LOG, Constants.STRING_CONTAINS, "The OP should have received a login error", SocialMessageConstants.CWWKS1100A_LOGIN_FAILED);
            expectations = validationTools.addMessageExpectation(testOPServer, expectations, perform_social_login, Constants.MESSAGES_LOG, Constants.STRING_CONTAINS, "The OP should have received a login error from WIM ", SocialMessageConstants.CWIML4537E_LOGIN_FAILED);
        }
        HtmlPage response = (HtmlPage) genericSocial(_testName, webClient, inovke_social_login_actions, updatedSocialTestSettings, expectations);

        /***
         * OK, I give... we'll just make sure that we didn't get to the app - make sure we got some form of a login or error page
         * and stop at that
         * LinkedIn, Twitter and now Facebook are giving one of various pages when we give a bad password - it's hard to code what
         * to expect, when
         * they keep changing what they'll reply with.
         */
        //        // login again, but this time use the correct password - show that subsequent good logins will work.
        //        List<validationData> expectations2 = setGoodSocialExpectations(socialSettings, doNotAddJWTTokenValidation, invoke_social_just_login_actions[invoke_social_just_login_actions.length - 1]);
        //        // Twitter has 2 panels to process after entering the wrong password
        //        //        if (provider.equals(SocialConstants.TWITTER_PROVIDER)) {
        //        //            expectations2 = vData.addExpectation(expectations2, SocialConstants.TWITTER_LOGIN, Constants.RESPONSE_FULL, Constants.STRING_CONTAINS, "Did NOT get to the Authorize only page", null, SocialConstants.TWITTER_AUTHORIZE_TITLE);
        //        //        }
        //
        //        if (provider.equalsIgnoreCase(SocialConstants.LINKEDIN_PROVIDER)) {
        //            genericSocial(_testName, webClient, response, SocialConstants.LINKEDIN_SIGN_IN_AND_LOGIN_ACTIONS, socialSettings, expectations2);
        //        } else {
        //            genericSocial(_testName, webClient, response, invoke_social_just_login_actions, socialSettings, expectations2);
        //        }
    }
