...
public class CommonUtils {
    ...
    //	private static final Class<?> thisClass = CommonUtils.class ;
    ...
    public List<validationData> getGoodExpectationsForJaxrsGet(String flowType, SAMLTestSettings testSettings, boolean checkRealm) throws Exception {
        ...
        if (checkRealm) {
            ...
            expectations = vData.addExpectation(expectations, SAMLConstants.INVOKE_JAXRS_GET, SAMLConstants.RESPONSE_FULL, SAMLConstants.STRING_CONTAINS, "Did not receive the expected realm in the public credentials.", null, "realmName=https:,");
            expectations = vData.addExpectation(expectations, SAMLConstants.INVOKE_JAXRS_GET, SAMLConstants.RESPONSE_FULL, SAMLConstants.STRING_CONTAINS, "Did not receive the expected realm in the private credentials.", null, "com.ibm.wsspi.security.cred.realm=https://" + chosenIdp);
        }
    ...
    public List<validationData> getGoodExpectationsForJaxrsAIPTests(String flowType, SAMLTestSettings testSettings, boolean checkRealm) throws Exception {
        ...
        if (checkRealm) {
            // Check the realm in the credentials
            String chosenIdp = testSettings.getSelectedIDPServerName();
            expectations = vData.addExpectation(expectations, SAMLConstants.INVOKE_ACS_WITH_SAML_RESPONSE, SAMLConstants.RESPONSE_FULL, SAMLConstants.STRING_CONTAINS, "Did not receive the expected realm in the public credentials.", null, "realmName=https:,");
            expectations = vData.addExpectation(expectations, SAMLConstants.INVOKE_ACS_WITH_SAML_RESPONSE, SAMLConstants.RESPONSE_FULL, SAMLConstants.STRING_CONTAINS, "Did not receive the expected realm in the private credentials.", null, "com.ibm.wsspi.security.cred.realm=https://" + chosenIdp);
        }
