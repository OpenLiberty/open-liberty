...
public class StatelessTimedBean implements StatelessTimedLocal {
    ...
    private static Object SetSessionContextResults;
    ...
    public void setSessionContext(SessionContext sc) {
        ...
        if (SetSessionContextResults != null) {
            return;
        }
    ...
    public void ejbCreate() throws CreateException {
        ...
        try {
            ...
            // -----------------------------------------------------------------------
            logger.info("ejbCreate: Calling getTimerService()");
            ts = context.getTimerService();
            timerService = ts;
            Assert.assertNotNull("1 ---> Got TimerService", ts);

            if (EjbCreateResults != null) {
                return;
            }
            ...
            // -----------------------------------------------------------------------
            try {
                logger.info("ejbCreate: Calling TimerService.createTimer()");
                TimerConfig timerConfig = new TimerConfig("PostConstruct", false);
            ...
            // -----------------------------------------------------------------------
            try {
                logger.info("ejbCreate: Calling getTimers()");
            ...
            // -----------------------------------------------------------------------
            try {
                logger.info("ejbCreate: Calling getAllTimers()");
     ...
     * the Container creates a Stateless bean, the results will be stored
     * in an instance variable. The test may then grab any instance (all
     * the same) and extract the results ({@link #getPreDestroyResults}). <p>
     *
     * This test will confirm the following :
     * <ol>
     * <li> getTimerService() returns a valid TimerService
    ...
    public void verifyPreDestroyResults() {
        verifyResults(EjbRemoveResults);
    }
    ...
    /**
     * Utility method that may be used to create a non-persistent Timer when
     * a Timer is required to perform a test, but cannot be created directly
     * by the bean performing the test. For example, if the bean performing
    ...
    public void recursiveCall(int depth) {
        ...
        StatelessTimedLocal bean = context.getBusinessObject(StatelessTimedLocal.class);
