...
public class MessageEndpointHandler implements MessageEndpoint, InvocationHandler {
    ...
    /**
     * The following static variables are used to indicate to the checkState which
     * Method this handler was invoked with.
     */
    ...
    /**
     * The following static variables are used to set the ivState instance variable
     * used to track what state this object is currently in.
     */
    ...
    /**
     * Must be set by using one of the above static variables for tracking the state
     * of this object.
     */
    private short ivState = RELEASED_STATE;

    /**
     * MessageEndpointFactory object. This object should be returned to when release
     * method is called. This is an immutable attribute of this object (set when
     * object is created and does not change for the life of this object).
     */
    private final BaseMessageEndpointFactory ivMessageEndpointFactory;
    ...
    /**
     * Recovery ID to use whenever this object enlists a XAResource object with the
     * transaction service. This is an immutable attribute of this object (set when
     * object is created and does not change for the life of this object).
     */
    ...
    /**
     * Proxy associated with this InvocationHandler. Must be set when this object is
     * created and remain set to the proxy instance until the discard method is
     * called. The discard instance is called whenever this object is discarded by
     * the PoolManager. At that time, discard method should null out ivProxy.
     */
    Object ivProxy = null;
    ...
    /**
     * The JCA major version of the resource adapter that is using this object. This
     * allows this code to continue the old behavior of a given version if a newer
     * JCA version requires a difference in behavior from the prior JCA version
     * (e.g. beforeDeliver/afterDelivery behavior change).
     */
    private int majorJCAVersion;

    /**
     * The JCA minor version of the resource adapter that is using this object. This
     * allows this code to continue the old behavior of a given version if a newer
     * JCA version requires a difference in behavior from the prior JCA version
     * (e.g. beforeDeliver/afterDelivery behavior change).
     */
    ...
    /**
     * Set to true if and only if beforeDelivery is called with an imported global
     * TX. afterDelivery must reset to false.
     */
     ...
     */
    private final EmbeddableWebSphereTransactionManager ivTransactionManager;

    /**
     * CTOR
     * 
     * @param factory          the MessageEndpointFactory that creates this handler.
     * @param recoveryId
     ...
     * 
     * @throws Throwable - the exception to throw from the method invocation on the
     *                   proxy instance. The exception's type must be assignable
     *                   either to any of the exception types declared in the throws
     *                   clause of the interface method or to the unchecked
     ...
     *                   declared in the throws clause of the interface method, then
     *                   an UndeclaredThrowableException containing the exception
     ...
     * @param args           the arguments to pass on the method call
     */
    protected void invokeJMSMethod(Method methodToInvoke, Object[] args) throws IllegalArgumentException, IllegalAccessException, InvocationTargetException {
        ...
        BatchJmsEndpointListener batchEndpointListener = new BatchJmsEndpointListener(ivMessageEndpointFactory.getConnectionFactory(),
                ivMessageEndpointFactory.getBatchOperationGroup(),
                ivMessageEndpointFactory.getWSJobRepository());
    ...
    /**
     * Process invocation of a method inherited from java.lang.Object. Note, only
     * the non final methods of java.lang.Object need to be implemented. The clone
     * method does not need to be implemented since we are not implementing the
     * Clonable interface.
     * 
     * @throws Exception
     */
     ...
     * detected, an IllegalStateException is thrown. Here is a list of assertions
     * that are checked by this method:
     * <dl>
     * <dt>Assertion 1
     * <dd>Section 12.5 - Any attempted use of the proxy endpoint (after its release
     * method is called) must result in a java.lang.IllegalStateException.
     * <dt>Assertion 2
     * <dd>Section 12.5.1.1 - A resource adapter must not attempt to deliver
     * messages concurrently to a single endpoint instance.
     * <dt>Assertion 3
     * <dd>Section 12.5.1.1 - The application server must reject concurrent usage of
     * an endpoint instance.
     * <dt>Assertion 4
     * <dd>Section 12.5.6 - Not explicitly stated, but the implication of this
     ...
     * <dt>Assertion 5
     * <dd>Section 12.5.6 - For each message delivery to an endpoint instance, the
     ...
     * releases the state of the proxy instance and makes it available for reuse. If
     * the release method is called while a message delivery is in-progress, the
     * application server must throw a java.lang.IllegalStateException, since
     * concurrent calls on a proxy endpoint instance is disallowed. In the case of
     * option B, if the release method is called inbetween beforeDelivery and
     * afterDelivery method calls, any transaction started during the corresponding
     * beforeDelivery method call must be aborted by the application server. Since
     * this section explicitly states the there must be a corresponding
     * afterDelivery call for each beforeDelivery call and multiple threads are not
     * allowed to use same MessageEndpoint instance concurrently, the assumption
     * here is IllegalStateException must still occur when release is called
     * inbetween beforeDelivery and afterDelivery and abortion of transaction should
     * only occur when called from the correct thread.
     * <dt>Assertion 6
     * <dd>Section 12.5.6 - There must not be more than one message delivery per
     * pair of beforeDelivery/afterDelivery invocation.
     * <dt>Assertion 7
     * <dd>Section 12.5.6 - For a single message delivery, the beforeDelivery, MDB
     * method invocation, and afterDelivery method calls must all occur from a
     * single thread of control.
     * </dl>
     * 
     * @param method   is the Method object passed to the invoke method of this
     *                 object.
     * 
     * @param args     is the array of arguments passed to the invoke method of this
     *                 object.
     * 
     * @param methodId is one of the constants defined in this class that indicates
     ...
     * MessageEndpoinFactory that created this instance must call this method during
     * its createEndpoint processing to make the Proxy and XAResource object known
     * to this object.
     * <p>
     * <dl>
     * <dt><b>pre-condition</b>
     * <dd>Must be called immediately after creating a new instance or after this
     * object is obtained from the Pool for reuse.
     * <dd>Caller must ensure ivProxy is set to the MessageEndpoint proxy object
     * whenever it creates a new instance of a MessageEndpointHandler object.
     * <dt><b>post-condition</b>
     * <dd>Ready for option A message delivery.
     * </dl>
     * 
     * @param xaResource            is the XAResource object to enlist in the
     *                              transaction when message delivery occurs. If a
     *                              null reference is passed, then no XAResource
     *                              object is enlisted.
     * 
     * @param recoverableXAResource must be set to true if and only if xaResource is
     *                              an instanceof RecoverableXAResource.
     * 
     * @param jcaVersion            is the JCA version of the RA that is using the
     ...
     * 
     * @param msg A message which explains why the IllegalStateException is being
     *            thrown (likely due to a JCA 1.5 spec violation
     */
    ...
    /**
     * This method is used for extending the transaction enlistment logic for the
     * lightweight server. It can be overridden to return a native RRS XAResource
     * 
     * @param coord - The UOWCoordinator.
