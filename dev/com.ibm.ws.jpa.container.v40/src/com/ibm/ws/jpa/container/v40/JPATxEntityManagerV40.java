...
public class JPATxEntityManagerV40 extends JPATxEntityManager {
    private static final TraceComponent tc = Tr.register(JPATxEntityManagerV40.class,
                                                         JPA_TRACE_GROUP,
                                                         JPA_RESOURCE_BUNDLE_NAME);

    public JPATxEntityManagerV40(JPAPuId puRefId, JPAPUnitInfo puInfo, J2EEName j2eeName, String refName, Map<?, ?> properties, boolean isUnsynchronized,
    ...
    protected JPATxEmInvocation createJPATxEmInvocation(UOWCoordinator uowCoord, EntityManager em) {
        return new JPATxEmInvocationV40(uowCoord, em, this, isTxUnsynchronized());
    }

    @Override
    protected JPANoTxEmInvocation createJPANoTxEmInvocation(UOWCoordinator uowCoord, EntityManager em) {
        return new JPANoTxEmInvocationV40(uowCoord, em, this, ivUnsynchronized);
    }
    ...
    public Statement createQuery(jakarta.persistence.criteria.CriteriaUpdate<?> updateQuery) {
        return getEMInvocationInfo(false).createQuery(updateQuery);
    }

    public Statement createQuery(jakarta.persistence.criteria.CriteriaDelete<?> deleteQuery) {
        return getEMInvocationInfo(false).createQuery(deleteQuery);
    }
    ...
    public <T> T find(EntityGraph<T> entityGraph, Object primaryKey, LockModeType lockMode, FindOption... options) {
        return getEMInvocationInfo(true).find(entityGraph, primaryKey, lockMode, options);
    }
    ...
    public void refresh(Object entity, LockModeType lockMode, RefreshOption... options) {
        getEMInvocationInfo(true).refresh(entity, lockMode, options);
    }
