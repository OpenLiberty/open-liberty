...
@Mode(TestMode.FULL)
public class URAPIs_CustomLDAPTest {
    private static LibertyServer server = LibertyServerFactory.getLibertyServer("com.ibm.ws.security.wim.adapter.ldap.fat.custom");
    private static final Class<?> c = URAPIs_CustomLDAPTest.class;
    ...
    public void getUsersForGroupWithInvalidGroup() throws Exception {
        ...
        try {
            ...
            assertTrue("An invalid user should cause EntryNotFoundException", errorMessage.contains("CWIML4001E"));
        }
    }
    ...
    /**
     * Hit the test servlet to see if getGroups works when passed in a valid group pattern
     * and a limit of 5; should only expect to find more entries but as limit 2, we should only get 2 groups
     * This verifies the various required bundles got installed and are working.
     */
    @Test
    ...
    public void getUniqueGroupIds() throws Exception {
        ...
        assertEquals("The number of entries did not match.", 5, list.size());
    }
