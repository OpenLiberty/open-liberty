...
@Mode(TestMode.FULL)
public class URAPIs_ADLDAP_NoIdTest {
    private static LibertyServer server = LibertyServerFactory.getLibertyServer("com.ibm.ws.security.wim.adapter.ldap.fat.ad.noId");
    private static final Class<?> c = URAPIs_ADLDAP_NoIdTest.class;
    ...
    public void getRealm() throws Exception {
        ...
        assertEquals("ADNoIdRealm", servlet.getRealm());
    }
    ...
    public void checkPasswordWithBadCredentials() throws Exception {
        ...
        assertNotNull("Should find CWIML4529E message.", server.waitForStringInLog("CWIML4529E"));
        passwordChecker.checkForPasswordInAnyFormat(password);
    }
    ...
    public void getGroupsForUserInNestedGroup() throws Exception {
        ...
        Log.info(c, "getGroupsForUserInNestedGroup", "Checking with a valid user.");
        List<String> list = servlet.getGroupsForUser(user);
        assertTrue(list.contains("embedded_group1") && list.contains("nested_g1"));
    ...
    public void getUniqueGroupIdsForUserInNestedGroup() throws Exception {
        String user = "CN=ng_user1,cn=users,dc=secfvt2,dc=austin,dc=ibm,dc=com";
        Log.info(c, "getUniqueGroupIdsForUserInNestedGroup", "Checking with a valid user.");
        List<String> list = servlet.getUniqueGroupIdsForUser(user);
        //assertTrue(list.contains("CN=embedded_group1,cn=users,dc=secfvt2,dc=austin,dc=ibm,dc=com") && list.contains("CN=nested_g1,cn=users,dc=secfvt2,dc=austin,dc=ibm,dc=com"));
        assertEquals("There should be two entries", 2, list.size());
    }
    ...
    public void getUniqueGroupIdsForTopNestedGroup() throws Exception {
        String user = "CN=topng_user1,cn=users,dc=secfvt2,dc=austin,dc=ibm,dc=com";
        Log.info(c, "getUniqueGroupIdsForTopNestedGroup", "Checking with a valid user.");
        List<String> list = servlet.getUniqueGroupIdsForUser(user);
        //assertTrue(list.contains("CN=nested_g1,cn=users,dc=secfvt2,dc=austin,dc=ibm,dc=com"));
