 ...
 *******************************************************************************/
package com.ibm.ws.ejbcontainer.mdb.jms.interceptor.xml.web;
...
import com.ibm.ws.ejbcontainer.mdb.jms.interceptor.xml.ejb.InterceptorMDB01Bean;
import com.ibm.ws.ejbcontainer.mdb.jms.interceptor.xml.ejb.InterceptorMDB02Bean;
import com.ibm.ws.ejbcontainer.mdb.jms.interceptor.xml.ejb.InterceptorMDB03Bean;
import com.ibm.ws.ejbcontainer.mdb.jms.interceptor.xml.ejb.InterceptorMDB04Bean;

import componenttest.app.FATServlet;

/**
 * <dl>
 * <dt><b>Test Name:</b> MDB30IntXMLTest
 *
...
@WebServlet("/MDB30IntXMLServlet")
public class MDB30IntXMLServlet extends FATServlet {
    private static final String LOGGER_CLASS_NAME = MDB30IntXMLServlet.class.getName();
    ...
    String testResultQueueName = "java:comp/env/jms/TestResultQueue";
    String InterceptorMDBReqQueueName = "java:comp/env/jms/MDBReqQueue";
    String InterceptorMDB02ReqQueueName = "java:comp/env/jms/MDB02ReqQueue";
    String InterceptorMDB03ReqQueueName = "java:comp/env/jms/MDB03ReqQueue";
    String InterceptorMDB04ReqQueueName = "java:comp/env/jms/MDB04ReqQueue";
    ...
    /**
     * Expected EXPECTED_testMDB01Bean_CALLREMOTEINJEJB string for testMDB01Bean.
     */
    private static final String EXPECTED_testMDB01Bean_CALLREMOTEINJEJB = "Just completed " + "lookup of ejb/SLEnvInjectTest_remote_biz." + newline + "Successfully "
                                                                          + "looked up the injected EJB and received expected message: success from the " + "method call.";
    ...
    private static final String EXPECTED_testMDB02Bean_CALLINJEJB = "Successfully invoked " + "the getString() on the injected bean and it returned: success from the "
                                                                    + "method call." + newline
                                                                    + "Just completed lookup of com.ibm.ws.ejbcontainer.mdb.jms.interceptor.xml.ejb"
    ...
    /**
     * Expected CALLREMOTEINJEJB string for testMDB02Bean.
     */
    private static final String EXPECTED_testMDB02Bean_CALLREMOTEINJEJB = "Successfully invoked " + "the getString() on the injected bean and it returned: success from the "
                                                                          + "method call." + newline
                                                                          + "Just completed lookup of com.ibm.ws.ejbcontainer.mdb.jms.interceptor.xml.ejb"
                                                                          + ".InterceptorMDB02Bean/injectedRef2." + newline + "Successfully looked "
                                                                          + "up the injected EJB via the default ENC created by injecting the EJB at the "
                                                                          + "field level and received expected message: success from the method call.";
    ...
    private static final String EXPECTED_testMDB03Bean_CALLINJEJB = "Successfully invoked " + "the getString() on the injected bean and it returned: success from the method call."
                                                                    + newline
                                                                    + "Just completed lookup of com.ibm.ws.ejbcontainer.mdb.jms.interceptor.xml.ejb.InterceptorMDB03Bean/simpleSL."
                                                                    + newline
     ...
     */
    private static final String[] EXPECTED_testMDB04Bean_AROUND_INVOKE = new String[] { "Interceptor03.aroundInvoke", "Interceptor02.aroundInvoke", "Interceptor01.aroundInvoke",
                                                                                        "InterceptorMDB04Bean.aroundInvoke", "InterceptorMDB04Bean.onMessage" };
    ...
    public void testXMLMDB01Bean() throws Exception {
        ...
        FATMDBHelper.putQueueMessage("CallRemoteInjEJB", qcfName, InterceptorMDBReqQueueName);
        actualInvSeq = (String) FATMDBHelper.getQueueMessage(qcfName, testResultQueueName);
        expectedInvSeq = EXPECTED_testMDB01Bean_CALLREMOTEINJEJB;
        svLogger.info("Expected: " + expectedInvSeq);
        svLogger.info("Actual: " + actualInvSeq);
        svLogger.info("Comparing expected and actual strings, if they match test passes.");
        assertEquals("Bean injectedRef --> Lookup using the ENC name derived from class level EJB injection " + "options was successsful.", expectedInvSeq, actualInvSeq);
    ...
    public void testXMLMDB02Bean() throws Exception {
        ...
        FATMDBHelper.putQueueMessage("CallRemoteInjEJB", qcfName, InterceptorMDB02ReqQueueName);
        actualInvSeq = (String) FATMDBHelper.getQueueMessage(qcfName, testResultQueueName);
        expectedInvSeq = EXPECTED_testMDB02Bean_CALLREMOTEINJEJB;
        svLogger.info("Expected: " + expectedInvSeq);
        svLogger.info("Actual: " + actualInvSeq);
        svLogger.info("Comparing expected and actual strings, if they match test passes.");
        assertEquals("Bean injectedRef --> Method call on injectedRef returned expected results.", expectedInvSeq, actualInvSeq);
