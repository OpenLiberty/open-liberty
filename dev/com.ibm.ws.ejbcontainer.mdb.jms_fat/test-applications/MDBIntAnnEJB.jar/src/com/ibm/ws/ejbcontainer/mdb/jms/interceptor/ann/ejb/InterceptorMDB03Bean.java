...
@MessageDriven(activationConfig = { @ActivationConfigProperty(propertyName = "destinationType", propertyValue = "javax.jms.Queue"),
                                    ...
                                    @ActivationConfigProperty(propertyName = "destination", propertyValue = "MDB03ReqQueue") })
@Interceptors({ Interceptor02.class, Interceptor01.class })
public class InterceptorMDB03Bean implements MessageListener {
    private static final String CLASS_NAME = InterceptorMDB03Bean.class.getName();
        ...
        try {
            ...
            if (rcvMsg.equals("AroundInvoke") || rcvMsg.equals("PostConstruct") || rcvMsg.equals("PreDestroy")) {
                ...
                try {
                    ...
                    SimpleSLLocal obj = (SimpleSLLocal) ctx.lookup("com.ibm.ws.ejbcontainer.mdb.jms.interceptor.ann.ejb.InterceptorMDB03Bean/simpleSL");

                    result += "Just completed lookup of com.ibm.ws.ejbcontainer.mdb.jms.interceptor.ann.ejb.InterceptorMDB03Bean/simpleSL." + newline;

                    // Call a method on the bean to ensure that the ref is valid
                    if (expected.equals(obj.getString())) {
                        // Test class will check for this value
                        result += "Successfully looked up the injected EJB via the default ENC created by injecting the EJB at the "
                                  + "method level and received expected message: " + obj.getString()
                                  + " from the method call.";
                    } else {
                        result += "Value from bean was not expected value. obj.getString() = " + obj.getString();
                    }
                } catch (Throwable t) {
                    svLogger.info("Unexpected exception: " + t.getMessage());
                    t.printStackTrace(System.out);
                    result += "Failed: unexpected exception (" + t.getClass().getSimpleName() + ") : " + t.getMessage();
                }
