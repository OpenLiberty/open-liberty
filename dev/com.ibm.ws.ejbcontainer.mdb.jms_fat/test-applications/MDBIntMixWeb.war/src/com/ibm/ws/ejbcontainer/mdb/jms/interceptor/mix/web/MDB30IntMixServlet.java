 ...
 *******************************************************************************/
package com.ibm.ws.ejbcontainer.mdb.jms.interceptor.mix.web;
...
import com.ibm.ws.ejbcontainer.mdb.jms.interceptor.mix.ejb.InterceptorMDB01Bean;
import com.ibm.ws.ejbcontainer.mdb.jms.interceptor.mix.ejb.InterceptorMDB02Bean;
import com.ibm.ws.ejbcontainer.mdb.jms.interceptor.mix.ejb.InterceptorMDB03Bean;
import com.ibm.ws.ejbcontainer.mdb.jms.interceptor.mix.ejb.InterceptorMDB04Bean;

import componenttest.app.FATServlet;

/**
 * <dl>
 * <dt><b>Test Name:</b> MDB30IntMixTest
 *
 ...
 * </dl>
 */
@WebServlet("/MDB30IntMixServlet")
@SuppressWarnings("serial")
public class MDB30IntMixServlet extends FATServlet {
    private final static String CLASS_NAME = MDB30IntMixServlet.class.getName();
    ...
    /**
     * Expected injection string for testMDB01Bean.
     */
    private static final String[] EXPECTED_testMDB01Bean_INJECTION = new String[] { "Interceptor01 = 42" };
    ...
    private static final String EXPECTED_testMDB02Bean_CALLINJEJB = "Successfully invoked " + "the getString() on the injected bean and it returned: success from the "
                                                                    + "method call." + newline
                                                                    + "Just completed lookup of com.ibm.ws.ejbcontainer.mdb.jms.interceptor.mix.ejb"
    ...
    private static final String EXPECTED_testMDB03Bean_CALLINJEJB = "Successfully invoked " + "the getString() on the injected bean and it returned: success from the method call."
                                                                    + newline
                                                                    + "Just completed lookup of com.ibm.ws.ejbcontainer.mdb.jms.interceptor.mix.ejb.InterceptorMDB03Bean/simpleSL."
                                                                    + newline
    ...
    public void testMixMDB01Bean() throws Exception {
        ...
        FATMDBHelper.putQueueMessage("InjectInterceptor", qcfName, InterceptorMDBReqQueueName);
        actualInvSeq = (String) FATMDBHelper.getQueueMessage(qcfName, testResultQueueName);
        expectedInvSeq = Arrays.toString(EXPECTED_testMDB01Bean_INJECTION);
        svLogger.info("Expected: " + expectedInvSeq);
        svLogger.info("Actual: " + actualInvSeq);
        svLogger.info("Comparing expected and actual strings, if they match test passes.");
        assertEquals("Interceptor Injection --> Context and env-entry injected successfully", expectedInvSeq, actualInvSeq);
        InterceptorMDB01Bean.svDestroyLatch.await(30, TimeUnit.SECONDS);
