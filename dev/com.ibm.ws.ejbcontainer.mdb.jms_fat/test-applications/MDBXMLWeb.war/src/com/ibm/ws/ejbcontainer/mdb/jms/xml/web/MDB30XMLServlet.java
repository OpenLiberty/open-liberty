 ...
 *******************************************************************************/
package com.ibm.ws.ejbcontainer.mdb.jms.xml.web;
...
import com.ibm.ws.ejbcontainer.mdb.jms.xml.ejb.BMTBeanIA;
import com.ibm.ws.ejbcontainer.mdb.jms.xml.ejb.BMTBeanNoCommit;
import com.ibm.ws.ejbcontainer.mdb.jms.xml.ejb.CMTBeanRequired;
import com.ibm.ws.ejbcontainer.mdb.jms.xml.ejb.SLRa;
import com.ibm.ws.ejbcontainer.mdb.jms.xml.ejb.SLRaHome;
...
@WebServlet("/MDB30XMLServlet")
public class MDB30XMLServlet extends FATServlet {
    private final static String CLASSNAME = MDB30XMLServlet.class.getName();
    ...
    String jndiSLRaHome = "java:global/MDBXMLApp/MDBXMLEJB/MDBSLR!com.ibm.ws.ejbcontainer.mdb.jms.xml.ejb.SLRaHome";
    ...
    public void testXMLCommonMDB() throws Exception {
        FATMDBHelper.emptyQueue(qcfName, testResultQueueName);
        SLRaHome slraHome = FATHelper.lookupRemoteBinding(jndiSLRaHome, SLRaHome.class);
        SLRa bean = slraHome.create();

        svLogger.info("-----> testCommonMDB -  trying to send message.");
        bean.sendMessage("Request test results");
        svLogger.info("-----> testCommonMDB -  message sent OK");
     ...
     */
    public void testXMLDurableTopic1() throws Exception {
        FATMDBHelper.emptyQueue(qcfName, testResultQueueName);
        FATMDBHelper.putTopicMessage("Request test results", tcfName, tsports);
    }

    public void testXMLDurableTopic2() throws Exception {
        String s = (String) FATMDBHelper.getQueueMessage(qcfName, testResultQueueName);
    ...
    public void testXMLCommonMDBTopic() throws Exception {
        ...
        svLogger.info("-----> testCommonMDBTopic -  trying to send message.");
        bean.putTopicMessage("Request test results", "MCM03");
        svLogger.info("-----> testCommonMDBTopic -  message sent OK");

        String s = (String) FATMDBHelper.getQueueMessage(qcfName, testResultQueueName);
        svLogger.info("Received message : " + s);
        assertEquals("Compare received message with expected message", "testCommonMDBTopic passed", s);
        FATMDBHelper.emptyQueue(qcfName, testResultQueueName);
    }
}