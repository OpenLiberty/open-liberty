...
public class AcmeFatUtils {
	 ...
	 */
	public static void deleteAcmeFiles(LibertyServer server) throws Exception {
		Log.info(AcmeFatUtils.class, "deleteAcmeFiles(LibertyServer)", "Deleting files generated by ACME feature.");
		server.deleteFileFromLibertyServerRoot("/resources/security/key.p12");
		server.deleteFileFromLibertyServerRoot("/resources/security/acmeAccountKey.pem");
		server.deleteFileFromLibertyServerRoot("/resources/security/acmeDomainKey.pem");
		...
		assertFalse("Acme file not removed, tests may fail, " + keystore, Files.exists(keystore.toPath(), LinkOption.NOFOLLOW_LINKS));
		assertFalse("Acme file not removed, tests may fail, " + accountKey, Files.exists(accountKey.toPath(), LinkOption.NOFOLLOW_LINKS));
		assertFalse("Acme file not removed, tests may fail, " + domainKey, Files.exists(domainKey.toPath(), LinkOption.NOFOLLOW_LINKS));
		
		Log.info(AcmeFatUtils.class, "deleteAcmeFiles(LibertyServer)", "Deleted files generated by ACME feature.");
	}
