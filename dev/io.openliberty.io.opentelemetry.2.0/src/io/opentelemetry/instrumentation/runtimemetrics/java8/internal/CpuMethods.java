...
public final class CpuMethods {
    ...
    static {
        ...
        Supplier<Long> processCpuTimeSupplier = methodInvoker(osBean, OS_BEAN_HOTSPOT, METHOD_PROCESS_CPU_TIME, Long.class);
        if (processCpuTimeSupplier == null) {
            // More users will be on hotspot than j9, so check for j9 second
            processCpuTimeSupplier = methodInvoker(osBean, OS_BEAN_J9, METHOD_PROCESS_CPU_TIME, Long.class);
        }
        processCpuTime = processCpuTimeSupplier;

        Supplier<Double> processCpuSupplier = methodInvoker(osBean, OS_BEAN_HOTSPOT, METHOD_PROCESS_CPU_LOAD, Double.class);
        ...
        Supplier<Double> systemCpuSupplier = methodInvoker(osBean, OS_BEAN_HOTSPOT, METHOD_CPU_LOAD, Double.class);
        if (systemCpuSupplier == null) {
            systemCpuSupplier = methodInvoker(osBean, OS_BEAN_HOTSPOT, METHOD_SYSTEM_CPU_LOAD, Double.class);
        }
        if (systemCpuSupplier == null) {
            // More users will be on hotspot than j9, so check for j9 second
    ...
    private static <T extends Number> Supplier<T> methodInvoker(
        ...
        try {
            ...
            return () -> {
                try {
                    Object r = method.invoke(osBean);

                    System.out.println("GREP1 " + osBean.toString() + " " + osBeanClassName + " " + methodName + " " + r.toString());

                    return returnType.cast(r);
                } catch (IllegalAccessException | InvocationTargetException e) {
                    System.out.println("GREP2 " + osBean.toString() + " " + osBeanClassName + " " + methodName + " null " + e);
                    return null;
                }
            };
        } catch (ClassNotFoundException | ClassCastException | NoSuchMethodException e) {
            System.out.println("GREP3r " + osBean.toString() + " " + osBeanClassName + " " + methodName + " null" + e);
            return null;
        }
    }
