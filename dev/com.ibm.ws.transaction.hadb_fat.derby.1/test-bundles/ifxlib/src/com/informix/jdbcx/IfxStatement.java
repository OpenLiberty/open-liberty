...
public class IfxStatement implements Statement {
    ...
    private void simQueryFailover(String sql) throws SQLException {
        boolean failOver = false;
        ...
        if (IfxConnection.isQueryFailoverEnabled() && IfxConnectionPoolDataSource.isTestingFailoverAtRuntime()) {
            String sqltag = IfxConnection.getControl().getSqltag();

            if (sqltag.length() > 0) {
                if (sql.endsWith(sqltag)) {
                    failOver = true;
                    IfxConnection.getControl().setSqltag("");
                }
