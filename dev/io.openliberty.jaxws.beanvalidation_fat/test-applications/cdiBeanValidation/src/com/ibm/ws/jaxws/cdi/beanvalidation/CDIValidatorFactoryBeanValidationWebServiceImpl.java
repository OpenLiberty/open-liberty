 ...
 */
@ApplicationScoped
@WebService(serviceName = "CDIValidatorFactoryBeanValidationWebService",
            portName = "CDIValidatorFactoryBeanValidationWebServicePort")
public class CDIValidatorFactoryBeanValidationWebServiceImpl {
    ...
    private final static Logger logger = Logger.getLogger(CDIValidatorFactoryBeanValidationWebServiceImpl.class.getName());
    ...
    public void oneWayWithValidation(String testString) {
        ...
        try {
            ...
            final Set<ConstraintViolation<String>> violations = injectValidatorFactory.getValidator().validate(testString);
    ...
    public void oneWayWithJAXBAnnotatedValidation(OneWayWithValidation request) {
        ...
        try {
            ...
            final Set<ConstraintViolation<OneWayWithValidation>> violations = injectValidatorFactory.getValidator().validate(request);
            if (!violations.isEmpty()) {
    ...
    public String twoWayWithValidation(String testString) {
        ...
        logger.info("Begining validation of testString: " + testString);
        try {
            final Set<ConstraintViolation<@NotNull @Size(min = 4, max = 10) String>> violations = injectValidatorFactory.getValidator().validate(testString);
            if (!violations.isEmpty()) {
                final ConstraintViolation<String> firstViolation = violations.iterator().next();
    ...
    public String twoWayWithValidationJAXBAnnotatedValidation(TwoWayWithValidation request) {
        ...
        try {
            ...
            logger.info("Begining validation of request: " + request);
            final Set<ConstraintViolation<TwoWayWithValidation>> violations = injectValidatorFactory.getValidator().validate(request);
            if (!violations.isEmpty()) {
