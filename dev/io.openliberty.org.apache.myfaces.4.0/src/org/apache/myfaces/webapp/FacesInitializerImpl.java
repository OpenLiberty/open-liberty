        ...
        try
        {
                    ...
                    MyfacesConfig.INITIALIZE_ALWAYS_STANDALONE, false))
            {
                ...
                } catch (UnsupportedOperationException use)
                {
                    ...
                    {
                        log.fine("An UnsupportedOperationException was caught while getting the ServletRegistrations from the ServletContext.");
                        log.fine(use.getMessage());
                        log.fine("Using the WebConfigProvider to look up FacesServlet mappings.");
                    }
                    // If the StartupServletContextListener was added programmatically an UnsupportedOperationException will be thrown
                    // by the ServletContext getServletRegistration method used by FacesServletMappingUtils. If an UnsupportedOperationException
                    // occurs we should fallback and try to read the web.xml directly by using the WebConfigProvider.
