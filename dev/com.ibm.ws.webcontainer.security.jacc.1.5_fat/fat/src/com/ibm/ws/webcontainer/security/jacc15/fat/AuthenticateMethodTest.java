 ...
 *
 * 4. Since useAuthenticationDataForUnprotectedResource is enabled by default,
 * any available authenticated data is used when accessing an unprotected resource.
 */
...
@Mode(TestMode.FULL)
public class AuthenticateMethodTest extends CommonAuthenticateMethodScenarios {

    private static final Class<?> myLogClass = AuthenticateMethodTest.class;
    private static LibertyServer myServer = LibertyServerFactory.getLibertyServer("com.ibm.ws.webcontainer.security.fat.loginmethod");
    private static CommonTestHelper myTestHelper = new CommonTestHelper();
    private static ServerTracesPasswordAsserter serverTracesPasswordAsserter;

    @BeforeClass
    public static void setUp() throws Exception {
        myServer.addInstalledAppForValidation("loginmethod");
        //LDAPUtils.addLDAPVariables(myServer);

        JACCFatUtils.installJaccUserFeature(myServer);
        JACCFatUtils.transformApps(myServer, "loginmethod.ear");
        ...
        serverTracesPasswordAsserter = new ServerTracesPasswordAsserter(myServer);
    }

    public AuthenticateMethodTest() {
        super(myServer, myLogClass, myTestHelper);
    }
     ...
     * <LI> 1) login() of user2 is called and should fail login since basic auth data was already used
    ...
    public void testAuthenticateMethodBA_UnprotectedServlet2() throws Exception {
        ...
        testHelper.verifyAuthenticationAlreadyHappenedException(test1);
    ...
    public void testAuthenticateMethodBA_ProtectedRedirectToUnprotected() throws Exception {
        ...
        // TEST1 - check values after authenticate - Since useAuthenticationDataForUnprotectedResource is enabled by default, authenticated data must be available.
        ...
        serverTracesPasswordAsserter.assertNoNewPasswords(validPassword, managerPassword);
    }
     ...
     * <OL>
     * <LI> No challenge or login required to access the unprotected resource
     * <LI> 1) login() of user2 is called and should fail login since form login data was already used in the second invocation
     * <LI> 2) logout() of user2 is called and should return null for the APIs
