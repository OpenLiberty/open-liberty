...
public final class LTCCallbacks {
     ...
     */
    private final ArrayList<UOWCallback> _callbacks = new ArrayList<UOWCallback>();
    ...
    public void registerCallback(UOWCallback callback) {
        ...
        if (!_callbacks.contains(callback)) {
            ...
            if (TraceComponent.isAnyTracingEnabled() && tc.isDebugEnabled())
                Tr.debug(tc, "Number of registered Callbacks: " + _callbacks.size());
        }
    ...
    public void contextChange(int typeOfChange) throws IllegalStateException {
        if (TraceComponent.isAnyTracingEnabled() && tc.isEntryEnabled())
            Tr.entry(tc, "contextChange", Util.printUOWStatusChangeType(typeOfChange));
        ...
        if (TraceComponent.isAnyTracingEnabled() && tc.isDebugEnabled())
            Tr.debug(tc, "Coordinator={0}", coord);
        ...
        for (int i = 0; i < _callbacks.size(); i++) {
            ...
            try {
                ...
                if (TraceComponent.isAnyTracingEnabled() && tc.isDebugEnabled())
                    Tr.debug(tc, "Exception caught during UOW callback at context change", ise);
                ex = ise;
            }
        }

        if (TraceComponent.isAnyTracingEnabled() && tc.isEntryEnabled())
            Tr.exit(tc, "contextChange", ex);
