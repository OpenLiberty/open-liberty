...
import io.openliberty.featureverifier.internal.FeatureSetGenerator;
...
public class FeatureSetGeneratorTest extends FeatureSetGenerator {
    ...
    private FeatureInfo createDebugFeature(String name, String version, String visibility, boolean isSingleton, String... nestedFeatures) {
        ...
        fi.setSingleton(isSingleton);
        HashMap<String, String> contentFeatures = new HashMap<String, String>();
        fi.setContentFeatures(contentFeatures);
        if (nestedFeatures != null) {
            for (String nested : nestedFeatures) {
                if (nested.indexOf('#') == -1) {
                    contentFeatures.put(nested, null);
                } else {
                    String parts[] = nested.split("#");
                    contentFeatures.put(parts[0], parts[1]);
                }
            }
        }
        return fi;
    ...
    public void testSingletonChoiceExpansion() {
        ...
        List<ConstrainedFeatureSet> testBuckets = createConstrainedFeatureList(
