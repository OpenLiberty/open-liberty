...
import io.openliberty.featureverifier.internal.GlobalConfig;
...
public class FeatureInfoTest {
    ...
    public void testCreatingFromManifest() throws IOException {
        ...
        File manifest = new File("test/feature/com.ibm.websphere.appserver.servlet-3.1" + getManifestFileExtension());
        assertTrue(manifest + "does not exist.", manifest.exists());
    ...
    public void testCreatingFromAnotherManifest() throws IOException {
        Map<VersionedEntity, String> devJars = new HashMap<VersionedEntity, String>();
        String installDir = null;
        File manifest = new File("test/feature/com.ibm.websphere.appserver.wsSecurity-1.1" + getManifestFileExtension());
        assertTrue(manifest + "does not exist.", manifest.exists());
        FeatureInfo i = createFromManifest(devJars, installDir, manifest);
        validateFeatureInfoHasSensibleValues(i);
    }
    ...
    public void testCreatingFromManifestWithSpecBundles() throws IOException {
        Map<VersionedEntity, String> devJars = new HashMap<VersionedEntity, String>();
        String installDir = null;
        File manifest = new File("test/feature/com.ibm.websphere.appserver.websocket-1.1" + getManifestFileExtension());
        assertTrue(manifest + "does not exist.", manifest.exists());
        FeatureInfo i = createFromManifest(devJars, installDir, manifest);
        validateFeatureInfoHasSensibleValues(i);
    ...
    public void testCreatingFromManifestWithJarEntry() throws IOException {
        Map<VersionedEntity, String> devJars = new HashMap<VersionedEntity, String>();
        String installDir = null;
        File manifest = new File("test/feature/com.ibm.websphere.appserver.jaxb-2.2" + getManifestFileExtension());
        assertTrue(manifest + "does not exist.", manifest.exists());
        FeatureInfo i = createFromManifest(devJars, installDir, manifest);
        validateFeatureInfoHasSensibleValues(i);
        Map<String, Set<VersionRange>> bundles = i.getContentBundles();
    ...
    public void testCreatingFromManifestWithDirectivesInFeatureContent() throws IOException {
        Map<VersionedEntity, String> devJars = new HashMap<VersionedEntity, String>();
        String installDir = null;
        File manifest = new File("test/feature/com.ibm.websphere.appserver.jpa-2.0" + getManifestFileExtension());
        assertTrue(manifest + "does not exist.", manifest.exists());
        FeatureInfo i = createFromManifest(devJars, installDir, manifest);
        validateFeatureInfoHasSensibleValues(i);
