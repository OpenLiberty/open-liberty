...
public class NewManifestProcessor {
    ...
    public static Manifest parseManifest(File file) {
        ...
        try {
            System.out.println("Parsing file: " + file.getCanonicalFile());
            ...
            /*
             * This is the main loop that contains all the flow logic. Basically the pattern is
             * this:
             * 1. Read a new line
             * 2. Skip empty lines
                ...
                try {
                    ...
                    if (fullAttribute.startsWith("-include=") || fullAttribute.startsWith("-include =")) {
                        ...
                        for (String includedManifest : splitAttribute[1].trim().split(",")) {
                            ...
                            File includedFile = new File(file.getAbsoluteFile().getParentFile(), includedManifestNoTilde.trim());
                            if (includedFile.exists()) {
                                ...
                                System.out.println("Could not process " + includedFile + " because the file could not be found.");
                            }
                        }
                    ...
                    System.out.println("Could not parse line: " + currentLine);
                    throw e;
                }
            }
    ...
    public static void replaceVariables(Manifest returnManifest) {
        ...
        for (Object attribute : mainAttributes.keySet()) {
            ...
            String resolved = resolveVariables(mainAttributes, attributeValue);
            if (!resolved.equals(attributeValue)) {
                mainAttributes.put(attribute, resolved);
            }
        }
    }

    private static String resolveVariables(Attributes mainAttributes, String originalString) {

        AttributesVariableResolver resolver = new AttributesVariableResolver(mainAttributes);
        String result = resolver.resolve(originalString);

        return result;

    }

    private static class AttributesVariableResolver extends AbstractVariableResolver {

        private final Attributes attributes;

        AttributesVariableResolver(Attributes attributes) {
            this.attributes = attributes;
        }

        @Override
        public String getValue(String name) {
            // In the Attributes all .'s are _'s
            String variableName = name.replace('.', '_');
            return attributes.getValue(variableName);
        }
