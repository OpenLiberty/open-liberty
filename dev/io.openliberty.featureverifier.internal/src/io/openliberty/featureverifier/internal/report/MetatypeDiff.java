    ...
    private void compareMetatype(File oldDir, File newDir, String metatypePath) {
        ...
        if (oldM.exists() && oldM.isFile() && newM.exists() && newM.isFile()) {
            ...
            if (f.hasDifferences()) {
                ...
                List<String> unified = UnifiedDiffUtils.generateUnifiedDiff(metatypePath, metatypePath, oldLines, patch, 5);
            ...
            if (oldProps.exists() && oldProps.isFile() && newProps.exists() && newProps.isFile()) {
                ...
                if (patch.getDeltas().size() > 0) {
                    ...
                    String why = "";
                    why += "   why: (unified diff)\n";
                    List<String> unified = UnifiedDiffUtils.generateUnifiedDiff(fPathAsString, fPathAsString, oldLines, patch, 5);
                    String unifiedString = "";
                    for (String s : unified) {
                        why += "      " + s + "\n";
                        unifiedString += s + "\n";
