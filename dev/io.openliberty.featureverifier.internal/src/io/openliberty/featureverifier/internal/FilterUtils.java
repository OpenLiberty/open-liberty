...
import io.openliberty.featureverifier.internal.FilterUtils.FilterOperation.NodeOperation;
...
public class FilterUtils {
    ...
    private static FilterNode parseFilterString(String filter) throws ParseError {
        if (filter.startsWith("(") && filter.endsWith(")")) {
            ...
            if (initialChar == '!') {
                throw new ParseError(new ErrorInfo(ReportType.ERROR, "[BAD_LDAP_FILTER '" + filter
                                                                     + "' ]", "Checker unable to parse filter fragment "
                                                                              + filter, "Checker was unable to parse the filter fragment as it makes use of ! when ! is not supported for use in the IBM-Provision-Capability header"));
            }
            ...
            throw new ParseError(new ErrorInfo(ReportType.ERROR, "[BAD_LDAP_FILTER " + filter
                                                                 + "]", "Checker unable to parse filter fragment "
                                                                        + filter, "Checker was unable to parse the filter fragment as it did not have a starting and terminating round bracket"));
        }
    }
