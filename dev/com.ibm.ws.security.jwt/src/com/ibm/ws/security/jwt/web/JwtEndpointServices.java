...
public class JwtEndpointServices {
    private static TraceComponent tc = Tr.register(JwtEndpointServices.class, TraceConstants.TRACE_GROUP, TraceConstants.MESSAGE_BUNDLE);
    ...
    private final ConcurrentServiceReferenceMap<String, JwtConfig> jwtConfigRef = new ConcurrentServiceReferenceMap<String, JwtConfig>(KEY_JWT_CONFIG);
    ...
    /**
     * Extracts the {@value WebConstants#JWT_REQUEST_ATTR} attribute from the provided request. That particular attribute is
     * supposed to be created and set by the filter directing the request.
     *
     * @param request
     * @param response
     * @return
    ...
    private JwtRequest getJwtRequest(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        if (jwtRequest == null) {
            String errorMsg = Tr.formatMessage(tc, "JWT_REQUEST_ATTRIBUTE_MISSING", new Object[] { request.getRequestURI(), WebConstants.JWT_REQUEST_ATTR });
     ...
     * @throws IOException
     */
    protected void handleJwtRequest(HttpServletRequest request, HttpServletResponse response, ServletContext servletContext, JwtConfig jwtConfig, EndpointType endpointType) throws IOException {
        if (jwtConfig == null) {
            if (tc.isDebugEnabled()) {
        ...
        case token:
            try {
                if (!isTransportSecure(request)) {
                    ...
                    response.sendError(HttpServletResponse.SC_NOT_FOUND, Tr.formatMessage(tc, "SECURITY.JWT.ERROR.WRONG.HTTP.SCHEME", new Object[] { url }));
                    return;
                }
    ...
    /**
     * determine if transport is secure. Either the protocol must be https or we must see a forwarding header that indicates it
     * was https upstream of a proxy. Use of a configuration property to allow plain http was rejected in review.
     *
     * @param req
     * @return
     */
    ...
    /**
     * produces a JWT token based upon the jwt Configuration, and the security credentials of the authenticated user that called
     * this method. Returns the token as JSON in the response.
     *
     * @param response
     * @param jwtConfig
     * @throws IOException
    ...
    /**
     * Obtains the JWK string that is active in the specified config and prints it in JSON format in the response. If a JWK is not
     * found, the response will be empty.
     *
     * @param response
     * @param jwtConfig
     * @throws IOException
