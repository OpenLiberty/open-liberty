...
public class JwtComponent implements JwtConfig {
	...
	private boolean tokenEndpointEnabled = false;
	...
	private void process(Map<String, Object> props) {
		...
		tokenEndpointEnabled = (Boolean) props.get(JwtUtils.CFG_KEY_TOKEN_ENDPOINT_ENABLED);
	...
	/**
	 * If the given host is "*", try to resolve this to a hostname or ip address by
	 * first checking the configured ${defaultHostName}. If ${defaultHostName} is
	 * "*", "localhost", or not specified, try obtaining the local ip address via
	 * InetAddress.
	 *
