...
public class ConcurBMTException implements MessageListener {
    @Resource
    ...
    public void ejbCreate() {
        System.out.println("--ConcurBMTException: ejbCreate");
    }
    ...
    public void onMessage(Message message) {
        if (AdapterUtil.inGlobalTransaction()) {
            System.out.println("ConcurBMTException is in a global transaction");
        } else {
            System.out.println("ConcurBMTException is in a local transaction");
        }
        System.out.println("--onMessage: ConcurBMTException got the message :" + message);
    }
    ...
    public void onStringMessage(String message) {
        ...
        System.out.println("--ConcurBMTException (" + message + "): entering onStringMessage");
        ...
        if (mdbInvokedTheFirstTime) {
            ...
            synchronized (syncObject) {
                ...
                while (concurrentInfo.getConcurrentMsgNumber() > 0) {
                    System.out.println("--ConcurBMTException (" + message + "): in the while block");
                    try {
                        Thread.sleep(100);
        ...
        System.out.println("--ConcurBMTException (" + message + "): exiting onStringMessage.");
    }
