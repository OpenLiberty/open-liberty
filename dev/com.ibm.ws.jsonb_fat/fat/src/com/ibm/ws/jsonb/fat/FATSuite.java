...
public class FATSuite {
     ...
     * JakartaEE9 - required modification - usr:testFeatureUsingJsonb-2.0
     * JakartaEE10 - required modification - usr:testFeatureUsingJsonb-3.0
     * JakartaEE11 - required modification - usr:testFeatureUsingJsonb-3.0
     */
    @ClassRule
    ...
    public static void configureImpls(LibertyServer server) {
        ...
        if (JakartaEEAction.isEE10OrLaterActive()) {
            ...
            //TODO replace with a third party implementation when one is developed
            server.addEnvVar("JSONB_PATH", "yasson/3.0.5/");
            server.addEnvVar("JSONB_JAR", "yasson.jar");
            server.addEnvVar("JSONB_ALT_JAR", "yasson.jar");

            server.addEnvVar("YASSON_PATH", "yasson/3.0.5/");
    ...
    private static void addFakeProvider(LibertyServer server) throws Exception {
        if (JakartaEEAction.isEE10OrLaterActive()) {
            RemoteFile parsson = server.getFileFromLibertySharedDir("resources/parsson/1.1.9/jakarta/parsson.jar");
