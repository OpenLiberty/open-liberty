 ...
 * interceptor methods in this class are private in order to test whether EJB
 * container can invoke private methods on interceptor class as required by EJB
 * 3 specification. Note, the intent is that none of the methods are overridden
 * by a subclass so that we can show that interceptor methods are invoked in
 * both the interceptor class and its super class.
 */
