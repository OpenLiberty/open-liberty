...
/**
 * Super class of a class level interceptor class for FVT. All of the interceptor methods
 * in this class are package protected in order to test whether EJB container can invoke
 * package protected methods on interceptor class as required by EJB 3 specification.
 * Note, the intent is that none of the methods are overridden by a subclass so that we
 * can show that interceptor methods are invoked in both the interceptor class and its
 * super class.
 */
public abstract class SuperMLInterceptor2 {
    /** Name of this class */
    private static final String CLASS_NAME = "SuperMLInterceptor2";
    ...
    @AroundInvoke
    Object superAroundInvoke(InvocationContext inv) throws Exception {
    ...
    @PostActivate
    void superPostActivate(InvocationContext inv) {
        try {
            ResultsLocal results = ResultsLocalBean.getSFBean();
    ...
    @PrePassivate
    void superPrePassivate(InvocationContext inv) {
        try {
            ResultsLocal results = ResultsLocalBean.getSFBean();
    ...
    @PreDestroy
    void superPreDestroy(InvocationContext inv) {
        try {
            ResultsLocal results = ResultsLocalBean.getSFBean();
