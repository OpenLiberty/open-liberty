...
*/
		...
		if (theWsdl.matches("UsrTokenPWDigestNonceCreatedSvc")) {
			...
			serviceName = new QName(SERVICE_NS,
					"SOAPServicePWDigestNonceCreated");
		...
		if (theWsdl.matches("UsrTokenPWDigestNonceCreatedSvcSSL")) {
			...
			serviceName = new QName(SERVICE_NS,
					"SOAPServicePWDigestNonceCreatedWithSSL");
		...
		SOAPServicePWDigest servicePWDigest = new SOAPServicePWDigest(wsdlURL,
				serviceName);

		try {

			reqMsg = new StringReader(
					"<soapenv:Envelope xmlns:soapenv=\"http://schemas.xmlsoap.org/soap/envelope/\"><soapenv:Header></soapenv:Header><soapenv:Body xmlns=\"http://wssec.pwdigest.cxf.fats/types\"><invoke>WSSECFVT CXF Service Client</invoke></soapenv:Body></soapenv:Envelope>");
			...
			rsp
					.print("<html><head><title>CXF Password Digest Service Cleint</title></head><body>");
