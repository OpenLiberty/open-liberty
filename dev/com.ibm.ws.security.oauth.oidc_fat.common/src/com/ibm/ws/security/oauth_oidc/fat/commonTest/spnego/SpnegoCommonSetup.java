...
public class SpnegoCommonSetup extends InitClass {
	...
	public static ExternalResource testRule = new ExternalResource() {
		/**
		 * Creates the SPN and keytab file to be used in any ensuing tests. Test classes
		 * can elect to create their own SPN and keytab file if needed. A common SPNEGO
		 * token is also created which can be used by all test classes. The common
		 * SPNEGO token should be refreshed at a certain interval, specified by the
		...
		protected void before() throws Exception {
			...
			if (!"127.0.0.1".equals(ip)) {
				Log.info(c, thisMethod, "The localhost ip address is " + ip
						+ " (not default value 127.0.0.1), localhost ip address tests will not be run");
				LOCALHOST_DEFAULT_IP_ADDRESS = false;
			}

			try {
				SpnegoOIDCCommonTest.commonSpnegoConfigSetup(null, "com.ibm.ws.security.openidconnect.fat.spnego.setup",
						null, SPNEGOConstants.NO_APPS, SPNEGOConstants.NO_PROPS, SPNEGOConstants.DONT_CREATE_SSL_CLIENT,
						SPNEGOConstants.CREATE_SPN_AND_KEYTAB, SPNEGOConstants.DEFAULT_REALM,
						SPNEGOConstants.CREATE_SPNEGO_TOKEN, SPNEGOConstants.SET_AS_COMMON_TOKEN,
						SPNEGOConstants.USE_CANONICAL_NAME, SPNEGOConstants.DONT_USE_COMMON_KEYTAB,
			...
			// The keytab file needs to be copied into the /tmp folder so we can include it
			// in the other servers
			SpnegoOIDCCommonTest.getMyServer().copyFileToTempDir(
					SPNEGOConstants.KRB_RESOURCE_LOCATION.substring(1) + SPNEGOConstants.KRB5_KEYTAB_FILE,
		...
		private boolean isSupportJDK() throws IOException {
			...
			JavaInfoFATUtils javaInfo = JavaInfoFATUtils.forServer(
					LibertyServerFactory.getLibertyServer("com.ibm.ws.security.openidconnect.fat.spnego.setup"));
			...
			Log.info(c, thisMethod, "The JDK used on this system is version: " + javaInfo.majorVersion()
					+ " and vendor: " + javaInfo.vendor());
		...
		protected void after() {
			try {
				if (RUN_TESTS) {
					...
					SpnegoOIDCCommonTest.getKdcHelper().deleteRemoteFileFromRemoteMachine(
							SpnegoOIDCCommonTest.getKdcHelper().getKdcMachine(), SPNEGOConstants.KRB5_KEYTAB_FILE);
					SpnegoOIDCCommonTest.getKdcHelper().deleteRemoteFileFromRemoteMachine(
							SpnegoOIDCCommonTest.getKdcHelper().getKdcMachine(),
							SPNEGOConstants.KRB5_KEYTAB_TEMP_SUFFIX);
				}
			} catch (Exception e) {
				Log.info(c, "after",
						"Exception thrown while deleting SPN: " + CommonTest.maskHostnameAndPassword(e.getMessage()),
						e);
			}
		};
	};
