...
public class CommonTest extends com.ibm.ws.security.fat.common.CommonTest {
	...
	public static String MONGO_PROPS_FILE = "mongoDB.props"; // this name needs to match the one used in
																// CustomStoreSample

	/**
	 * Install new user features, to be overridden by whoever needs to install user
	 * feature.
	 *
	 * @param aServer
	 * @throws Exception
	...
	/**
	 * Uninstall user features, to be overridden by whoever needs to uninstall user
	 * feature.
	 *
	 * @throws Exception
	 */
	 ...
	 * Waits for the test apps to start - It sets up the trust manager. - Defines
	 * some global variables for use by the follow on tests...
	 */
	public static TestServer commonSetUp(String requestedServer, String serverXML, String testType,
			List<String> addtlApps, boolean useDerby, List<String> addtlMessages) throws Exception {

		return commonSetUp(requestedServer, serverXML, testType, addtlApps, useDerby, addtlMessages, null, null, true,
				true);
	}
	 ...
	 */
	public static TestServer commonSetUp(String requestedServer, String serverXML, String testType,
			List<String> addtlApps, boolean useDerby, List<String> addtlMessages, boolean secStartMsg, boolean sslCheck)
			throws Exception {

		return commonSetUp(requestedServer, serverXML, testType, addtlApps, useDerby, addtlMessages, null, null,
				secStartMsg, sslCheck);
	}
	...
	public static TestServer commonSetUp(String requestedServer, String serverXML, String testType,
		...
		return commonSetUp(requestedServer, serverXML, testType, addtlApps, useDerby, addtlMessages, targetUrl,
				providerType, true, true);
	}
	...
	public static TestServer commonSetUp(String requestedServer, String serverXML, String testType,
		...
		return commonSetUp(requestedServer, serverXML, testType, addtlApps, useDerby, addtlMessages, targetUrl,
				providerType, secStartMsg, sslCheck, Constants.ACCESS_TOKEN_KEY, Constants.X509_CERT);
	}

	public static TestServer commonSetUp(String requestedServer, String serverXML, String testType,
			List<String> addtlApps, boolean useDerby, List<String> addtlMessages, String targetUrl, String providerType,
			boolean secStartMsg, boolean sslCheck, String tokenType, String certType) throws Exception {

		return commonSetUp(requestedServer, serverXML, testType, addtlApps, useDerby, addtlMessages, targetUrl,
				providerType, secStartMsg, sslCheck, tokenType, certType, Constants.JUNIT_REPORTING);
	}

	public static TestServer commonSetUp(String requestedServer, String serverXML, String testType,
			List<String> addtlApps, boolean useDerby, List<String> addtlMessages, String targetUrl, String providerType,
			boolean secStartMsg, boolean sslCheck, String tokenType, String certType, boolean reportViaJunit)
			throws Exception {

		return commonSetUp(requestedServer, serverXML, testType, addtlApps, useDerby, false, addtlMessages, targetUrl,
				providerType, secStartMsg, sslCheck, tokenType, certType, reportViaJunit);
	...
	public static TestServer commonSetUp(String requestedServer, String serverXML, String testType,
		...
		try {
			...
			if (!testType.equals(Constants.OIDC_RP)) {
				if (LibertyFileManager.libertyFileExists(aTestServer.getServer().getMachine(),
						aTestServer.getServer().pathToAutoFVTTestFiles
								+ "/internalFeatures/securitylibertyinternals-1.0.mf")) {
					aTestServer.getServer().copyFileToLibertyInstallRoot("lib/features",
							"internalFeatures/securitylibertyinternals-1.0.mf");
				}
			}
			if (!secStartMsg) {
			...
			// set properties in bootstrap.properties on every setup - not only when we're
			// switching/altering the token type
			if (testType.equals(Constants.GENERIC_SERVER) || testType.equals(Constants.OIDC_RP)) {
				aTestServer.addValidationEndpointProps(providerType, tokenType, certType, jwkValidationMap,
						classOverrideValidationEndpointValue);
			} else {
			...
			if (testType.equals(Constants.IDP_SERVER_TYPE)) {
				// we're having an issue with the in memory LDAP server on z/OS, added a method
				// to see if it can accept requests,
				// if NOT, we'll use a "external" LDAP server (Shibboleth allows for failover to
				// additional LDAP servers, but,
				// it doesn't allow different bindDN, bindPassword, ...)
				// this method will add properties to bootstrap.properties that will point to a
				// hopefully working LDAP server
			...
			// if test class is using LDAP, or we're configuring an IDP server, we'll need
			// the ldap vars in bootstrap.properties
			...
			if (skipServerStart) {
				Log.info(thisClass, "setupBeforeTest",
			...
			if (!useDerby && useMongo && JavaInfoFATUtils.forCurrentVM().majorVersion() == 7) { // Added Java 7 back in,
																								// need to do this to
																								// connect to Consul for
																								// MongoDB
			...
			/*
			 * reconfigExpectations = new FATDataHelpers().addExpectation(null, null,
			 * Constants.MESSAGES_LOG, null, null, Constants.MSG_OPENID_CONFIG_MODIFIED);
			 */
			...
			// String currentHttpsPort =
			// Integer.toString(aTestServer.getHttpDefaultSecurePort());
			// if (!validationTools.isInList(validPortNumbers, currentHttpsPort)) {
			// testHelpers.logDebugInfo();
			// throw new Exception("These tests use external IDP servers. Those external
			// servers have entries that include the https port used by the SP server on
			// this test machine. Therefore, these tests require the use of ports " +
			// Arrays.toString(validPortNumbers) + " ONLY. This execution is using port: " +
			// currentHttpsPort);
			// }
			// }
							...
							if (testType.equals(Constants.GENERIC_SERVER)) {
								...
								if (testType.equals(Constants.IDP_SERVER_TYPE)) {
									...
									throw new RuntimeException(
											"Unknown testType:" + testType + " Can not determine defaults to set");
								}
							}
						}
					}
			...
			if (useDerby && useMongo) { // while not ideal, setting two variables seemed to have the lowest code churn
										// when the MongoDB option was added.
				Log.info(thisClass, "setupBeforeTest",
						"Both useDerby and useMongo were set to true. Derby will be used.");
			}
		} catch (Exception e) {
			...
			// erroring out...
			try {
				// don't update the allowed timeout count for retrys - we're failing and exiting
		...
		if (tokenType.equals(Constants.JWT_TOKEN) || tokenType.equals(Constants.MP_JWT_TOKEN)
				|| tokenType.equals(Constants.BUILT_JWT_TOKEN)) {
	...
	private static void setupSSLClient(boolean overrideForConsul) {
		...
		try {
			...
			if (ksPath != null && ksPath.length() > 0) {
				...
				Log.info(thisClass, "setup Keymanager",
	 ...
	 *
	 * @param providerType - The type of provider that this test instance will use
	 */
	private static void initHttpUnitSettings() {
		 ...
		 * httpunit try to parse the java script embedded in the forms - that doesn't
	 ...
	 * value set to false, the clientCertificate is ignored. And the test results
	 * are the same as before in theory. 2) Once the ssl in the client initialized
	 * in a single test class, we got into troubles to update its settings again
	 */
	...
	/**
	 * Set up the local ACTION variables based on the server type The defaults are
	 * the steps/actions used with oidc/oauth, but some tests can run with those
	 * servers, or an ISAM server and for ISAM, the steps/actions will be different
	 * - this method lets us override the defaults
	 *
	 */
	...
	/**
	 * Generic steps to invoke methods (in the correct ordering) to test with the OP
	 * server
	 *
	 * @param testcase
	 * @param wc
	...
	public WebResponse genericOP(String testcase, WebConversation wc, TestSettings settings, String[] testActions,
	...
	public WebResponse genericOP(String testcase, WebConversation wc, TestSettings settings, String[] testActions,
			List<validationData> expectations, WebResponse response, String idToken) throws Exception {
		...
		try {
			...
			if (validationTools.isInList(testActions, Constants.SUBMIT_TO_AUTH_SERVER)) {
				response = helpers.submitToAuthServer(testcase, wc, response, settings, expectations,
			...
			if (validationTools.isInList(testActions, Constants.INVOKE_AUTH_ENDPOINT_WITH_BASIC_AUTH)) {
				response = invokeAuthorizationEndpoint(testcase, wc, response, settings, expectations,
						Constants.INVOKE_AUTH_SERVER_WITH_BASIC_AUTH);
			}
			...
			if (validationTools.isInList(testActions, Constants.SUBMIT_TO_AUTH_SERVER_WITH_BASIC_AUTH)) {
				response = helpers.submitToAuthServer(testcase, wc, response, settings, expectations,
						Constants.SUBMIT_TO_AUTH_SERVER_WITH_BASIC_AUTH);
			}

			if (validationTools.isInList(testActions, Constants.INVOKE_AUTH_SERVER)) {
				response = helpers.invokeAuthServer(testcase, wc, response, settings, expectations,
						Constants.INVOKE_AUTH_SERVER);
			}
			...
			if (validationTools.isInList(testActions, Constants.INVOKE_RS_PROTECTED_RESOURCE)) {
				response = helpers.invokeRsProtectedResource(testcase, wc, beforeResourceResponse, settings,
						expectations);
			}
	...
	public Object genericOP(String testcase, WebClient webClient, TestSettings settings, String[] testActions,
			List<validationData> expectations) throws Exception {
		return genericOP(testcase, webClient, settings, testActions, expectations, null, null);
	}

	public Object genericOP(String testcase, WebClient webClient, TestSettings settings, String[] testActions,
			List<validationData> expectations, Object response, String idToken) throws Exception {

		// return genericOP(testcase, webClient, settings, testActions, expectations,
		// null, null);
		//
		// }
		//
		// public WebResponse genericOP(String testcase, WebClient webClient,
		// TestSettings settings, String[] testActions, List<validationData>
		// expectations, WebResponse response, String idToken) throws Exception {
		...
		try {
			...
			if (validationTools.isInList(testActions, Constants.SUBMIT_TO_AUTH_SERVER)) {
				thePage = helpers.submitToAuthServer(testcase, webClient, thePage, settings, expectations,
						Constants.SUBMIT_TO_AUTH_SERVER);
			}
			...
			// if (validationTools.isInList(testActions,
			// Constants.INVOKE_AUTH_ENDPOINT_WITH_BASIC_AUTH)) {
			// response = invokeAuthorizationEndpoint(testcase, wc, response, settings,
			// expectations, Constants.INVOKE_AUTH_SERVER_WITH_BASIC_AUTH);
			// }
			//
			// if (validationTools.isInList(testActions,
			// Constants.SUBMIT_TO_AUTH_SERVER_FOR_TOKEN)) {
			// response = helpers.submitToAuthServerForToken(testcase, wc, response,
			// settings, expectations);
			// }
			//
			// if (validationTools.isInList(testActions,
			// Constants.SUBMIT_TO_AUTH_SERVER_WITH_BASIC_AUTH)) {
			// response = helpers.submitToAuthServer(testcase, wc, response, settings,
			// expectations, Constants.SUBMIT_TO_AUTH_SERVER_WITH_BASIC_AUTH);
			// }
			//
			// if (validationTools.isInList(testActions, Constants.INVOKE_AUTH_SERVER)) {
			// response = helpers.invokeAuthServer(testcase, wc, response, settings,
			// expectations, Constants.INVOKE_AUTH_SERVER);
			// }
			// if (validationTools.isInList(testActions,
			// Constants.INVOKE_AUTH_SERVER_WITH_BASIC_AUTH)) {
			// response = helpers.invokeAuthServer(testcase, wc, response, settings,
			...
			// if (validationTools.isInList(testActions, Constants.PERFORM_ISAM_LOGIN)) {
			// response = helpers.performISAMLogin(testcase, wc, response, settings,
			// expectations);
			// }
			...
			// if (validationTools.isInList(testActions,
			// Constants.INVOKE_TOKEN_ENDPOINT_CL_CRED)) {
			// response = invokeTokenEndpoint_clientCredentials(testcase, wc, settings,
			// expectations);
			// }
			//
			// if (validationTools.isInList(testActions,
			// Constants.INVOKE_TOKEN_ENDPOINT_PASSWORD)) {
			// response = invokeTokenEndpoint_password(testcase, wc, settings,
			// expectations);
			// }
			//
			...
			// response = helpers.invokeRsProtectedResource(testcase, wc,
			// beforeResourceResponse, settings, expectations);
			// }
			//
			if (validationTools.isInList(testActions, Constants.LOGOUT)) {
	 ...
	 * @throws Exception
	 */
	public WebResponse genericRP(String testcase, WebConversation wc, TestSettings settings, String[] testActions,
			List<validationData> expectations) throws Exception {
		...
		try {
			...
			if (validationTools.isInList(testActions, Constants.LOGOUT)) {
				response = helpers.processEndSession(testcase, wc, validationTools.getIDToken(settings, response),
						settings, expectations);
			}
	...
	public Object genericRP(String testcase, WebClient webClient, TestSettings settings, String[] testActions,
		...
		try {
			...
			if (validationTools.isInList(testActions, Constants.LOGIN_USER)) {
				thePage = helpers.processProviderLoginForm(testcase, webClient, (HtmlPage) thePage,
						Constants.LOGIN_USER, settings, expectations);
			}

			if (validationTools.isInList(testActions, Constants.PERFORM_IDP_LOGIN)) {
				thePage = helpers.performIDPLogin(testcase, webClient, (HtmlPage) thePage, settings, expectations);
			}
			// with java script enabled for negative samesite tests, we run into an OOM
			// issue with htmlunit
			...
			if (validationTools.isInList(testActions, Constants.LOGOUT)) {
				thePage = helpers.processLogout(testcase, webClient, validationTools.getIDToken(settings, thePage),
						settings, expectations);
			}
	 ...
	 * @param skipTestCase - optional
	 * @return
	 * @throws Exception
	 */
	/*
	 * test settings and skiptestcase not specified - pass in the defaults of null
	 * and false
	 */
	public WebResponse genericInvokeEndpoint(String testcase, WebConversation wc, WebResponse inResponse, String url,
			String method, String action, List<endpointSettings> parms, List<endpointSettings> headers,
			List<validationData> expectations) throws Exception {

		return genericInvokeEndpoint(testcase, wc, inResponse, url, method, action, parms, headers, expectations, null,
				false);
	}

	/*
	 * test settings passed in, but skiptestcase not specified - pass in the default
	 * of false
	 */
	public WebResponse genericInvokeEndpoint(String testcase, WebConversation wc, WebResponse inResponse, String url,
			String method, String action, List<endpointSettings> parms, List<endpointSettings> headers,
			List<validationData> expectations, TestSettings settings) throws Exception {

		return genericInvokeEndpoint(testcase, wc, inResponse, url, method, action, parms, headers, expectations,
				settings, false);
	}
	...
	public WebResponse genericInvokeEndpoint(String testcase, WebConversation wc, WebResponse inResponse, String url,
			String method, String action, List<endpointSettings> parms, List<endpointSettings> headers,
			List<validationData> expectations, boolean skipTestCase) throws Exception {
	...
	public WebResponse genericInvokeEndpoint(String testcase, WebConversation wc, WebResponse inResponse, String url,
			String method, String action, List<endpointSettings> parms, List<endpointSettings> headers,
			List<validationData> expectations, TestSettings settings, boolean skipTestCase) throws Exception {
		...
		try {
				...
				for (endpointSettings parm : parms) {
					if (parm.value != null) {
						Log.info(thisClass, thisMethod,
								"Setting request parameter:  key: " + parm.key + " value: " + parm.value);
			...
			if (headers != null) {
				for (endpointSettings header : headers) {
					Log.info(thisClass, thisMethod,
							"Setting header field:  key: " + header.key + " value: " + header.value);
	...
	public Object genericInvokeEndpoint(String testcase, WebClient webClient, Object startPage, String url,
			String method, String action, List<endpointSettings> parms, List<endpointSettings> headers,
			List<validationData> expectations, TestSettings settings) throws Exception {

		return genericInvokeEndpoint(testcase, webClient, startPage, url, method, action, parms, headers, expectations,
				settings, false);
	}
	...
	public Object genericInvokeEndpoint(String testcase, WebClient webClient, Object startPage, String inUrl,
			String method, String action, List<endpointSettings> parms, List<endpointSettings> headers,
			List<validationData> expectations, TestSettings settings, boolean skipTestCase) throws Exception {
	 ...
	 * @throws Exception
	 */
	public WebResponse invokeEndpointWithBody(String testcase, WebConversation wc, WebResponse inResponse, String url,
			String method, String action, List<endpointSettings> parms, List<endpointSettings> headers,
			List<validationData> expectations, InputStream source, String contentType) throws Exception {
	 ...
	 * @throws Exception
	 */
	public WebResponse genericInvokeForm(String testcase, WebConversation wc, WebResponse inResponse, String url,
			String method, String action, List<endpointSettings> parms, List<validationData> expectations)
			throws Exception {
		return genericInvokeForm(testcase, wc, inResponse, null, url, method, action, parms, expectations);

	}

	public WebResponse genericInvokeForm(String testcase, WebConversation wc, WebResponse inResponse,
			TestSettings settings, String url, String method, String action, List<endpointSettings> parms,
			List<validationData> expectations) throws Exception {
	...
	public Object genericInvokeForm(String testcase, WebClient webClient, Object inResponse, String url, String method,
			String action, List<endpointSettings> parms, List<validationData> expectations) throws Exception {
		return genericInvokeForm(testcase, webClient, inResponse, null, url, method, action, parms, expectations);

	}

	public Object genericInvokeForm(String testcase, WebClient webClient, Object inResponse, TestSettings settings,
			String inUrl, String method, String action, List<endpointSettings> parms, List<validationData> expectations)
			throws Exception {
	 ...
	 * refreshed or if the common token is null. This is intended to be called by
	 * a @Before method in test classes that wish to use this functionality. This
	 * allows individual tests that simply need a propagation token to invoke the
	 * protected resource to use a valid token that has already obtained. Individual
	 * tests then do not need to go through the entire genericOP flow.
	 */
	public void createCommonPropagationToken() {
		...
		if (shouldCommonTokenBeRefreshed() || commonPropagationToken == null) {
			Log.info(thisClass, method, "Obtaining a new common propagation token"
					+ ((commonPropagationToken == null) ? " (common token was null)" : ""));

			try {
				WebResponse response = genericOP(_testName, new WebConversation(), testSettings,
						Constants.BASIC_AUTHENTICATION_ACTIONS, vData.addSuccessStatusCodes());
				...
				Log.info(thisClass, method,
						"Common propagation token (" + commonPropagationToken + ") created at "
								+ formatter.format(new Date(propagationTokenCreationDate))
								+ " and will be refreshed in " + testPropagationTokenLifetimeSeconds + " second(s).");

			} catch (Exception e) {
				Log.error(thisClass, method, e,
						"Failed to obtain a common propagation token. Tests using the common token may fail.");
			}
		}
	}

	/**
	 * Returns true if the difference between the current time and the common token
	 * creation time exceeds the lifetime we specify in
	...
	private boolean shouldCommonTokenBeRefreshed() {
		...
		if (((currentTime - propagationTokenCreationDate) / 1000) > testPropagationTokenLifetimeSeconds) {
			Log.info(thisClass, method,
					"Common token lifetime has exceeded allowed time; recommend a new token should be created.");
			return true;
		}
		return false;
	}
	...
	public static void tearDown() throws Exception {
		...
		for (TestServer server : serverRefList) {
			...
			try {
				// update the allowedTimeout count to account for msgs issued during start
				// retries
				...
				// Typically caused by an unexpected error message in the logs; should re-throw
				// the exception
				e.printStackTrace(System.out);
				String prevExceptionMsg = (cumulativeException == null) ? "" : cumulativeException.getMessage();
				cumulativeException = new Exception(
						prevExceptionMsg + " [Exception #" + (++exceptionNum) + "]: " + e.getMessage() + "\n<br>");
			}
		}
	...
	/**
	 * Remove the mongoDB props file used by the CustomStoreSample and stop the
	 * locally started MongoDB server.
	 *
	 * @param server
	...
	private static boolean mongoDBTeardownCleanup(TestServer server, Exception cumulativeException, int exceptionNum) {
		try {
			try {
				...
				Log.info(thisClass, "mongoDBTeardownCleanup",
						"Exception removing MONGO_PROPS_FILE. If this is a Derby test, ignore this message." + e);
			}
	 ...
	 *                   null, testSettings.getUserinfoEndpt(),
	 *                   Constants.POSTMETHOD, Constants.INVOKE_USERINFO_ENDPOINT,
	 *                   parms, null, expectations);
	 **/
	public void genericInvokeEndpointWithHttpUrlConn(String testcase, String response, String url, String method,
			String action, List<endpointSettings> parms, List<endpointSettings> headers,
			List<validationData> expectations) throws Exception {
		...
		try {
			...
			// Delete doesn't support output - have to build the url with the query string
			// included
	...
	/**
	 * Invoke the authorization endpoint directly instead of using client.jsp First,
	 * we need to build up the list of parm needed for the endpoint - those come
	 * from our settings
	 *
	 * @param testCase     - the testcase name for logging purposes
	 * @param wc           - the web converation
	 * @param inResponse   - the response from the previous frame
	 * @param settings     - the current test settings - used to set the parms
	 * @param expectations - the expectations to be used for validation
	 * @return - the response from the endpoint invocation
	 * @throws Exception
	 */
	public WebResponse invokeAuthorizationEndpoint(String testCase, WebConversation wc, WebResponse inResponse,
			TestSettings settings, List<validationData> expectations) throws Exception {

		return invokeAuthorizationEndpoint(testCase, wc, inResponse, settings, expectations,
				Constants.INVOKE_AUTH_SERVER);

	}

	public WebResponse invokeAuthorizationEndpoint(String testCase, WebConversation wc, WebResponse inResponse,
			TestSettings settings, List<validationData> expectations, String basicAuth) throws Exception {
		...
		if (basicAuth.equals(Constants.INVOKE_AUTH_SERVER_WITH_BASIC_AUTH)) {
			Log.info(thisClass, thisMethod, "Building basic auth with user: " + settings.getAdminUser()
					+ " and password: " + settings.getAdminPswd());
			headers = eSettings.addEndpointSettings(null, "Authorization",
					cttools.buildBasicAuthCred(settings.getAdminUser(), settings.getAdminPswd()));
		}
		...
		WebResponse response = genericInvokeEndpoint(testCase, wc, inResponse, settings.getAuthorizeEndpt(),
				Constants.GETMETHOD, Constants.INVOKE_AUTH_ENDPOINT, parms, headers, expectations, settings);
		;
		return response;
	}

	public Object invokeAuthorizationEndpoint(String testCase, WebClient webClient, Object startPage,
			TestSettings settings, List<validationData> expectations) throws Exception {

		return invokeAuthorizationEndpoint(testCase, webClient, startPage, settings, expectations,
				Constants.INVOKE_AUTH_SERVER);
	...
	public Object invokeAuthorizationEndpoint(String testCase, WebClient webClient, Object startPage,
		...
		Log.info(thisClass, thisMethod,
				"WebClient isJavaScriptEnabled: " + webClient.getOptions().isJavaScriptEnabled());

		Object response = genericInvokeEndpoint(testCase, webClient, startPage, settings.getAuthorizeEndpt(),
				Constants.GETMETHOD, Constants.INVOKE_AUTH_ENDPOINT, parms, headers, expectations, settings);

		Set<Cookie> cookies = webClient.getCookieManager().getCookies();
		for (Cookie cookie : cookies) {
			if (cookie.getName().startsWith("WASSamlReq_")) {
				// prepend the url to the hacked string, then encode it (finally remove the
				// original and add the new value
	...
	/**
	 * Invoke the token endpoint directly instead of using client.jsp First, we need
	 * to build up the list of parm needed for the endpoint - those come from our
	 * settings
	 *
	 * @param testCase     - the testcase name for logging purposes
	 * @param wc           - the web converation
	 ...
	 * @throws Exception
	 */
	public WebResponse invokeTokenEndpoint(String testCase, WebConversation wc, WebResponse inResponse,
			TestSettings settings, List<validationData> expectations) throws Exception {
		...
		parms = eSettings.addEndpointSettings(parms, "code",
				validationTools.getValueFromResponseFull(inResponse, "Received authorization code: "));
		if (settings.getCodeVerifier() != null) {
			parms = eSettings.addEndpointSettings(parms, "code_verifier", settings.getCodeVerifier());
		}
		WebResponse response = genericInvokeEndpoint(_testName, wc, inResponse, settings.getTokenEndpt(),
				Constants.POSTMETHOD, Constants.INVOKE_TOKEN_ENDPOINT, parms, headers, expectations, settings);

		return response;
	}

	public Object invokeTokenEndpoint(String testCase, WebClient webClient, Object startPage, TestSettings settings,
			List<validationData> expectations) throws Exception {
		...
		parms = eSettings.addEndpointSettings(parms, "grant_type", "authorization_code");
		parms = eSettings.addEndpointSettings(parms, "code",
				validationTools.getValueFromResponseFull(startPage, "Received authorization code: "));
		if (settings.getCodeVerifier() != null) {
			parms = eSettings.addEndpointSettings(parms, "code_verifier", settings.getCodeVerifier());
		}
		Object response = genericInvokeEndpoint(_testName, webClient, startPage, settings.getTokenEndpt(),
				Constants.POSTMETHOD, Constants.INVOKE_TOKEN_ENDPOINT, parms, headers, expectations, settings);
	 ...
	 * @throws Exception
	 */
	public WebResponse invokeTokenEndpoint_clientCredentials(String testCase, WebConversation wc, TestSettings settings,
			List<validationData> expectations) throws Exception {
		...
		WebResponse response = genericInvokeEndpoint(_testName, wc, null, settings.getTokenEndpt(),
				Constants.POSTMETHOD, Constants.INVOKE_TOKEN_ENDPOINT, parms, headers, expectations, settings);
	 ...
	 * @throws Exception
	 */
	public WebResponse invokeTokenEndpoint_password(String testCase, WebConversation wc, TestSettings settings,
			List<validationData> expectations) throws Exception {
		...
		List<endpointSettings> headers = null;
		Log.info(thisClass, thisMethod, "Building basic auth with user: " + settings.getClientID() + " and password: "
				+ settings.getClientSecret());
		headers = eSettings.addEndpointSettings(null, "Authorization",
				cttools.buildBasicAuthCred(settings.getClientID(), settings.getClientSecret()));

		Log.info(thisClass, thisMethod,
				"Setting resource owner " + settings.getAdminUser() + " and password: " + settings.getAdminPswd());
	...
	/**
	 * Invoke the app-passwords endpoint method to create an app-password This
	 * method requests that the clientId/clientSecret be passed in the header
	 *
	 * @param testCase     - the testCase name
	 * @param wc           - the conversation
	 * @param settings     - current testSettings
	 * @param accessToken  - the access_token to use to generate the app-password
	 * @param appName      - the app name to register the app-password with
	 * @param usedBy       - the client_d that will be allowed to use the tokens
	 *                     created from the app-password created
	 * @param expectations - the expected results/output from invoking the endpoint
	 * @return - returns the response from the invocation (could contain the
	 *         app-password, or error/failure information
	 * @throws Exception - this method should NOT result in an exception - if one is
	 *                   thrown, the calling test case should report an error
	 */
	public WebResponse invokeAppPasswordsEndpoint_create(String testCase, WebConversation wc, TestSettings settings,
			String accessToken, String appName, String usedBy, List<validationData> expectations) throws Exception {
		return invokeAppPasswordsEndpoint_create(testCase, wc, settings, accessToken, appName, usedBy, Constants.HEADER,
				expectations);
	}
	 ...
	 * @param usedBy         - the client_d that will be allowed to use the tokens
	 *                       created from the app-password created
	 * @param clientLocation - flag indicating if clientid/clientSecret should be
	 *                       included in the header or as a parameter
	 * @param expectations   - the expected results/output from invoking the
	 *                       endpoint
	 * @return - returns the response from the invocation (could contain the
	 *         app-password, or error/failure information
	 * @throws Exception - this method should NOT result in an exception - if one is
	 ...
	 */
	public WebResponse invokeAppPasswordsEndpoint_create(String testCase, WebConversation wc, TestSettings settings,
			String accessToken, String appName, String usedBy, String clientLocation, List<validationData> expectations)
			throws Exception {
		String thisMethod = "invokeAppPasswordsEndpoint_create";
		msgUtils.printMethodName(thisMethod);

		Log.info(thisClass, thisMethod,
				"Generating app-password for access_token: [" + accessToken + "] and app_name: [" + appName
						+ "] using clientId: [" + settings.getClientID() + "] and clientSecret: ["
						+ settings.getClientSecret() + "]");
		...
		if (clientLocation.equals(Constants.HEADER)) {
			headers = eSettings.addEndpointSettings(headers, "Authorization",
					cttools.buildBasicAuthCred(settings.getClientID(), settings.getClientSecret()));
		} else {
			parms = eSettings.addEndpointSettings(parms, "client_id", settings.getClientID());
			parms = eSettings.addEndpointSettings(parms, "client_secret", settings.getClientSecret());
		...
		WebResponse response = genericInvokeEndpoint(_testName, wc, null, settings.getAppPasswordsEndpt(),
				Constants.POSTMETHOD, Constants.INVOKE_APP_PASSWORDS_ENDPOINT_CREATE, parms, headers, expectations,
				settings);
	 ...
	 * @param wc           - the conversation
	 * @param settings     - current testSettings
	 * @param accessToken  - the access_token to a) provide authorization and 2)
	 *                     possibly the user_id to list
	 * @param userId       - the user id to list app-passwords for
	 * @param expectations - the expected results/output from invoking the endpoint
	 * @return - returns the response from the invocation (could contain the list of
	 *         app-password info, or error/failure information
	 * @throws Exception - this method should NOT result in an exception - if one is
	 *                   thrown, the calling test case should report an error
	 */
	public WebResponse invokeAppPasswordsEndpoint_list(String testCase, WebConversation wc, TestSettings settings,
			String accessToken, String userId, List<validationData> expectations) throws Exception {
		return invokeAppPasswordsEndpoint_list(testCase, wc, settings, accessToken, userId, Constants.HEADER,
				expectations);
	}
	 ...
	 */
	public WebResponse invokeAppPasswordsEndpoint_list(String testCase, WebConversation wc, TestSettings settings,
			String accessToken, String userId, String clientLocation, List<validationData> expectations)
			throws Exception {
		String thisMethod = "invokeAppPasswordsEndpoint_list";
		msgUtils.printMethodName(thisMethod);

		Log.info(thisClass, thisMethod,
				"Listing app-passwords for access_token: [" + accessToken + "] and user_id: [" + userId
						+ "] using clientId: [" + settings.getClientID() + "] and clientSecret: ["
						+ settings.getClientSecret() + "]");
		...
		// pass the name of the user to perform the action on - if omitted, the list
		// will be for the user in the access_token
		...
		WebResponse response = genericInvokeEndpoint(_testName, wc, null, settings.getAppPasswordsEndpt(),
				Constants.GETMETHOD, Constants.INVOKE_APP_PASSWORDS_ENDPOINT_LIST, parms, headers, expectations,
				settings);
	 ...
	 * @param settings     - current testSettings
	 * @param accessToken  - the access_token to a) provide authorization and 2)
	 *                     possibly the user_id to revoke
	 * @param userId       - the user id to revoke app-passwords for
	 * @param appId        - the specific app_id to revoke
	 * @param expectations - the expected results/output from invoking the endpoint
	 * @throws Exception - this method should NOT result in an exception - if one is
	 *                   thrown, the calling test case should report an error
	 */
	public void invokeAppPasswordsEndpoint_revoke(String testCase, WebConversation wc, TestSettings settings,
			String accessToken, String userId, String appId, List<validationData> expectations) throws Exception {
		invokeAppPasswordsEndpoint_revoke(testCase, wc, settings, accessToken, userId, appId, Constants.HEADER,
				expectations);
	}
	 ...
	 */
	public void invokeAppPasswordsEndpoint_revoke(String testCase, WebConversation wc, TestSettings settings,
			String accessToken, String userId, String appId, String clientLocation, List<validationData> expectations)
			throws Exception {
		String thisMethod = "invokeAppPasswordsEndpoint_revoke";
		msgUtils.printMethodName(thisMethod);

		Log.info(thisClass, thisMethod,
				"Revoking app-passwords for access_token: [" + accessToken + "], user_id: [" + userId
						+ "] and app_id: [" + appId + "] using clientId: [" + settings.getClientID()
		...
		headers = eSettings.addEndpointSettingsIfNotNull(headers, Constants.ACCESS_TOKEN_KEY, accessToken);
		if (clientLocation.equals(Constants.HEADER)) {
			headers = eSettings.addEndpointSettingsIfNotNull(headers, "Authorization",
					cttools.buildBasicAuthCred(settings.getClientID(), settings.getClientSecret()));
		} else {
		...
		// RTC 274642 using htmlunit is resulting in an OOM - even with javascript
		// disabled - it's failing in:
		// java.lang.OutOfMemoryError: Failed to create a thread: retVal -1073741830,
		// errno 132 (0x84), errno2 -1055784930 (0xffffffffc112001e)
		// at java.lang.Thread.start(Thread.java:982)
		// at
		// com.gargoylesoftware.htmlunit.javascript.background.DefaultJavaScriptExecutor.startThreadIfNeeded(DefaultJavaScriptExecutor.java:63)
		// at
		// com.gargoylesoftware.htmlunit.javascript.background.DefaultJavaScriptExecutor.addWindow(DefaultJavaScriptExecutor.java:191)
		// at
		// com.gargoylesoftware.htmlunit.javascript.JavaScriptEngine.registerWindowAndMaybeStartEventLoop(JavaScriptEngine.java:629)
		...
		// at
		// com.ibm.ws.security.oauth_oidc.fat.commonTest.CommonTest.genericInvokeEndpoint(CommonTest.java:1280)
		// WebClient webClient = CommonTestHelpers.getWebClient(true);
		// webClient.getOptions().setJavaScriptEnabled(false);
		// genericInvokeEndpoint(testCase, webClient, null, urlString,
		// Constants.DELETEMETHOD, Constants.INVOKE_APP_PASSWORDS_ENDPOINT_REVOKE,
		// parms, headers, expectations, settings, false);
		genericInvokeEndpointWithHttpUrlConn(testCase, null, urlString, Constants.DELETEMETHOD,
				Constants.INVOKE_APP_PASSWORDS_ENDPOINT_REVOKE, parms, headers, expectations);
	 ...
	 * @param wc             - the conversation
	 * @param settings       - current testSettings
	 * @param accessToken    - the access_token to use to generate the app-token
	 * @param appName        - the app name to register the app-token with
	 * @param usedBy         - used_by value to include in the request. The request
	 *                       parameter will be omitted if this value is null.
	 * @param clientLocation - flag indicating if clientid/clientSecret should be
	 *                       included in the header or as a parameter
	 * @param expectations   - the expected results/output from invoking the
	 *                       endpoint
	 * @return - returns the response from the invocation (could contain the
	 *         app-token, or error/failure information
	 * @throws Exception - this method should NOT result in an exception - if one is
	 *                   thrown, the calling test case should report an error
	 */
	public WebResponse invokeAppTokensEndpoint_create(String testCase, WebConversation wc, TestSettings settings,
			String accessToken, String appName, String usedBy, String clientLocation, List<validationData> expectations)
			throws Exception {
		...
		Log.info(thisClass, thisMethod,
				"Generating app-token for access_token: [" + accessToken + "] and app_name: [" + appName
						+ "] using clientId: [" + settings.getClientID() + "] and clientSecret: ["
						+ settings.getClientSecret() + "]");
		...
		WebResponse response = genericInvokeEndpoint(_testName, wc, null, settings.getAppTokensEndpt(),
				Constants.POSTMETHOD, Constants.INVOKE_APP_TOKENS_ENDPOINT_CREATE, parms, headers, expectations,
				settings);
	 ...
	 * @param accessToken  - the access_token to a) provide authorization and 2)
	 *                     possibly the user_id to list
	 * @param userId       - the user id to list app-tokens for
	 * @param expectations - the expected results/output from invoking the endpoint
	 * @return - returns the response from the invocation (could contain the list of
	 *         app-token info, or error/failure information
	 * @throws Exception - this method should NOT result in an exception - if one is
	 *                   thrown, the calling test case should report an error
	 */
	public WebResponse invokeAppTokensEndpoint_list(String testCase, WebConversation wc, TestSettings settings,
			String accessToken, String userId, List<validationData> expectations) throws Exception {
		return invokeAppTokensEndpoint_list(testCase, wc, settings, accessToken, userId, Constants.HEADER,
				expectations);
	}
	...
	public WebResponse invokeAppTokensEndpoint_list(String testCase, WebConversation wc, TestSettings settings,
		...
		Log.info(thisClass, thisMethod,
				"Listing app-tokens for access_token: [" + accessToken + "] and user_id: [" + userId
						+ "] using clientId: [" + settings.getClientID() + "] and clientSecret: ["
						+ settings.getClientSecret() + "]");
		...
		HttpUnitOptions.setScriptingEnabled(false);
		WebResponse response = genericInvokeEndpoint(_testName, wc, null, settings.getAppTokensEndpt(),
				Constants.GETMETHOD, Constants.INVOKE_APP_TOKENS_ENDPOINT_LIST, parms, headers, expectations, settings);
		HttpUnitOptions.setScriptingEnabled(origScripting);
	 ...
	 * @param accessToken  - the access_token to a) provide authorization and 2)
	 *                     possibly the user_id to revoke
	 * @param userId       - the user id to revoke app-tokens for
	 * @param appId        - the specific app_id to revoke
	 * @param expectations - the expected results/output from invoking the endpoint
	 * @return - returns the response from the invocation (could be an empty
	 *         response, or error/failure information
	 * @throws Exception - this method should NOT result in an exception - if one is
	 *                   thrown, the calling test case should report an error
	 */
	public void invokeAppTokensEndpoint_revoke(String testCase, WebConversation wc, TestSettings settings,
			String accessToken, String userId, String tokenId, List<validationData> expectations) throws Exception {
		invokeAppTokensEndpoint_revoke(testCase, wc, settings, accessToken, userId, tokenId, Constants.HEADER,
				expectations);
	}
	 ...
	 */
	public void invokeAppTokensEndpoint_revoke(String testCase, WebConversation wc, TestSettings settings,
			String accessToken, String userId, String tokenId, String clientLocation, List<validationData> expectations)
			throws Exception {
		String thisMethod = "invokeAppTokensEndpoint_revoke";
		msgUtils.printMethodName(thisMethod);

		Log.info(thisClass, thisMethod,
				"Revoking app-tokens for access_token: [" + accessToken + "], user_id: [" + userId + "] and token_id: ["
						+ tokenId + "] using clientId: [" + settings.getClientID() + "] and clientSecret: ["
						+ settings.getClientSecret() + "]");
		...
		// webClient.getOptions().setJavaScriptEnabled(false);
		// genericInvokeEndpoint(testCase, webClient, null, urlString,
		// Constants.DELETEMETHOD, Constants.INVOKE_APP_TOKENS_ENDPOINT_REVOKE, parms,
		// headers, expectations, settings, false);
		genericInvokeEndpointWithHttpUrlConn(testCase, null, urlString, Constants.DELETEMETHOD,
				Constants.INVOKE_APP_TOKENS_ENDPOINT_REVOKE, parms, headers, expectations);
	 ...
	 * @throws Exception
	 */
	public WebResponse invokeGenericForm_refreshToken(String testCase, WebConversation wc, TestSettings settings,
			String originalRefreshToken, List<validationData> expectations) throws Exception {
		...
		WebResponse response = genericInvokeForm(_testName, wc, null, settings, settings.getRefreshTokUrl(),
				Constants.GETMETHOD, Constants.INVOKE_REFRESH_ENDPOINT, parms, expectations);

		return response;
	}

	public List<endpointSettings> setRefreshTokenParms(TestSettings settings, String originalRefreshToken)
			throws Exception {
	...
	public Object invokeGenericForm_refreshToken(String testCase, WebClient webClient, TestSettings settings,
			String originalRefreshToken, List<validationData> expectations) throws Exception {
		...
		// Constants.REFRESH_TOKEN_GRANT_TYPE);
		Object response = genericInvokeForm(_testName, webClient, null, settings, settings.getRefreshTokUrl(),
				Constants.POSTMETHOD, Constants.INVOKE_REFRESH_ENDPOINT, parms, expectations);

		return response;
	}

	public WebResponse invokeGenericForm_revokeToken(String testCase, WebConversation wc, TestSettings settings,
			String accessToken, List<validationData> expectations) throws Exception {

		String thisMethod = "invokeGenericForm_revokeToken";
		msgUtils.printMethodName(thisMethod);

		List<endpointSettings> headers = eSettings.addEndpointSettings(null, "Authorization",
				cttools.buildBasicAuthCred(settings.getClientID(), settings.getClientSecret()));
		List<endpointSettings> parms = eSettings.addEndpointSettings(null, "token", accessToken);
		return genericInvokeEndpoint(_testName, wc, null, settings.getRevocationEndpt(), Constants.POSTMETHOD,
				Constants.INVOKE_REVOKE_ENDPOINT, parms, headers, expectations);
	...
	/**
	 * Determines if the remote server is up and whether a request for the specified
	 ...
	 * @param isSecure
	 */
	public static boolean isRemoteServerAndUrlAvailable(String hostName, int port, String urlContextPath,
			boolean isSecure) {
		String method = "isRemoteServerAndUrlAvailable";

		return true;
		// try {
		// Log.info(thisClass, method, "Creating socket for host " + hostName + " and
		// port " + port);
		// Socket s = new Socket(hostName, port);
		// if (s.isConnected()) {
		// try {
		// WebConversation wc = new WebConversation();
		// String url = hostName + ":" + port + (urlContextPath == null ? "" :
		// urlContextPath);
		// if (isSecure) {
		// url = "https://" + url;
		// } else {
		...
		// Log.info(thisClass, method, "Remote server " + hostName + " and URL " +
		// urlContextPath + " is up and responding to requests.");
		// return true;
		// }
		// Log.info(thisClass, method, "Remote server " + hostName + " is up, but did
		// not receive a 200 status code for URL " + urlContextPath + ". Instead got " +
		// response.getResponseCode());
		// return false;
		// } catch (SAXException e) {
		// throw new IOException(e);
		// } catch (HttpNotFoundException e) {
		// Log.info(thisClass, method, "Remote server " + hostName + " considered not
		// available because of exception: " + e);
		// }
		// }
		// } catch (IOException e) {
		// Log.info(thisClass, method, "Caught exception attempting to contact " +
		// hostName + " and URL " + urlContextPath + ": " + e);
		// }
	...
	private void logTestCaseInServerSideLog(String action, TestServer server) throws Exception {
		...
		try {
			if (server != null) {
				...
				WebConversation wc = new WebConversation();
				WebRequest request = new GetMethodWebRequest(server.getServerHttpString() + "/"
						+ AppConstants.TESTMARKER_CONTEXT_ROOT + "/" + AppConstants.TESTMARKER_PATH);
	...
	public WebResponse buildPostSolicitedSPInitiatedRequest(String testcase, WebConversation wc, TestSettings settings,
			List<validationData> expectations) throws Exception {
	...
	public Object buildPostSolicitedSPInitiatedRequest(String testcase, WebClient webClient, TestSettings settings,
			List<validationData> expectations) throws Exception {
		...
		try {
			...
			com.gargoylesoftware.htmlunit.WebRequest request = new com.gargoylesoftware.htmlunit.WebRequest(url,
					HttpMethod.GET);
	...
	/**
	 * Takes an array of server:port strings and randomly chooses one - checks if it
	 * is listening on the port specified. If it is, it returns that information. If
	 * not, it creates a new list containing all OTHER server and calls itself
	 * again. Hopefully randomly finding a server that is active. If no servers are
	 * active then it throws an exception - which should cause the test class to
	 * fail.
	 *
	 * @param serverList
	 * @return
		...
		for (int i = 0; i < numServers; i++) {
			if (div != i) {
				...
				if (serverList[i] == null || serverList[i].length() == 0) {
					Log.info(thisClass, thisMethod,
							"The server selected is not set - skipping.  (position " + i + " in the server array)");
				} else {
	...
	public static String[] selectIDPServer() throws Exception {
		...
		while ((chosen_IDP_server == null) && (filteredIDPServerList != null)) {
			...
			if (isRemoteServerAndUrlAvailable(chosen_IDP_server[0], Integer.valueOf(chosen_IDP_server[1]), idpTfimApp,
					false)) {
				break;
			} else {
		...
		if (chosen_IDP_server == null) {
			// throw new Exception("No TFIM server appears to be active - Test class is
			// terminating");
	...
	public static String[] removeChosenFromFilteredIDPServerList(String[] origList, String[] chosen_IDP_server)
			throws Exception {
	...
	private static void setupMongoDBConfig(TestServer aTestServer, String httpString, Integer defaultPort) {
		...
		try {
			mongoTableUid = "_" + InetAddress.getLocalHost().getHostName() + "_"
	...
	static protected void setForClientSecretHash() {
		testSettings.setHttpString(testOPServer.getHttpString()); // need this information to talk to the Derby or
																	// MongoDB servlet
