...
<%
	...
	String savedParms = null ;
	Cookie[] cookies = request.getCookies();
	if (cookies != null) {
	    for (Cookie cookie : cookies) {
	        System.out.println("cookie: name: " + cookie.getName() + " value: " + cookie.getValue()) ;
	        if (cookie.getName().startsWith("WASSamlReq_")) {
	            savedParms = cookie.getValue();
	        }
	    }
	}
	
	String decodedCookie = URLDecoder.decode(savedParms, "UTF-8") ;
	System.out.println("decodedCookie: " + decodedCookie) ;
	Map<String, String> attrMap = new HashMap<String, String>() ;
	if (decodedCookie != null) {
	    String[] parts = decodedCookie.split("\\?")[1] .split("\\&") ;
	...
	if (MODE_AUTHORIZE.equals(mode)) {
		...
		testName = attrMap.get("test_name");
		username = attrMap.get("user_name");
		clientId = attrMap.get("client_id") ;
		clientSecret = attrMap.get("client_secret");
		redirectUri = attrMap.get("redirect_uri");
		authorizeEndpoint = attrMap.get("authorize_endpoint");
		tokenEndpoint = attrMap.get("token_endpoint");
		resourceEndpoint = attrMap.get("resource_endpoint");
		state = attrMap.get("state");
		scope = attrMap.get("scope");
		autoConsent = attrMap.get("autoauthz");
		if (nonce != null && !nonce.isEmpty()) {
		...
		System.out.println("authorize_endpoint: " + authorizeEndpoint) ;
		System.out.println("TokenEndpoint: " + tokenEndpoint) ;
		System.out.println("resource_endpoint: " + resourceEndpoint) ;
		System.out.println("state: " + state) ;
		System.out.println("scope: " + scope) ;
		System.out.println("autoauthz: " + autoConsent) ;
		*/
	}
