...
<%
    ...
    String tokenEndpoint = urlBase + "/oidc/endpoint/OidcConfigSample/token";
    String jwt_scope = "openid profile";
    String jwt_scope2 = "openid profile";
    String extraParams = "";

    
    if ("true".equals(request.getParameter("auto"))) {
        System.out.println("jwtDupScopes.jsp: in auto");
        submit = true;
        testName = request.getParameter("testCaseName");
        clientId = request.getParameter("client_id");
        clientSecret = request.getParameter("client_secret");
        jwtBearerToken = request.getParameter("jwt_bearer_token");
        tokenEndpoint = request.getParameter("token_endpoint");
        System.out.println("jwtDupScopes.jsp: client_id: " + clientId);
        System.out.println("jwtDupScopes.jsp: client_secret: " + clientSecret);
        System.out.println("jwtDupScopes.jsp: jwt_bearer_token: " + jwtBearerToken);
        System.out.println("jwtDupScopes.jsp: token_endpoint: " + tokenEndpoint);
        jwt_scope = request.getParameter("scope"); 
        System.out.println("jwtDupScopes.jsp: scope: " + jwt_scope);
        if( jwt_scope != null && (!jwt_scope.isEmpty()) ){
            extraParams = extraParams + "&scope=" + jwt_scope;  
        }
        jwt_scope2 = request.getParameter("scope2"); 
        System.out.println("jwtDupScopes.jsp: scope2: " + jwt_scope2);
        if( jwt_scope2 != null && (!jwt_scope2.isEmpty()) ){
            extraParams = extraParams + "&scope=" + jwt_scope2;  
        }
    }
%>
<html>
...
<form name="tokform" method="POST" action="jwtDupScopes.jsp">
<input type="hidden" name="auto" value="true" />
<table width=800>
<tr><td>Test Name</td><td><input type="text" name="testCaseName" value="<%=testName%>" /></td></tr>
...
<tr><td>JWT-Bearer Token</td><td><input type="text" name="jwt_bearer_token" value="<%=jwtBearerToken%>" size="360" /></td></tr>
...
<tr><td>JWT Scopes</td><td><input type="text" name="scope2" value="<%=jwt_scope2%>" size="60" /></td></tr>
...
if (submit) {
        ...
        System.out.println("jwtDupScopes.jsp: Start Test Case:  " + testName + "===============================");
        System.out.println("jwtDupScopes.jsp: opened the connection to " + urlToken + ": " + connToken);
        ...
        // read response
        connToken.connect();
        System.out.println("jwtDupScopes.jsp string buffer: " + sb.toString());
        System.out.println("jwtDupScopes.jsp: connected");
        InputStream streamToken = null;
        int responseCodeToken = connToken.getResponseCode();
        System.out.println("jwtDupScopes.jsp: got response code: " + responseCodeToken);
...
System.out.println("jwtDupScopes.jsp: End Test Case:  " + testName + "===============================");
        }   
} // end submit     
