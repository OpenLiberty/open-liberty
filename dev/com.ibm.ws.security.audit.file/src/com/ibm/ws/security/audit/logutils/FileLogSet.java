...
public class FileLogSet {
    ...
    private static final TraceComponent tc = Tr.register(FileLogSet.class);
     ...
     *
     * @see FileLogUtils#compileLogFileRegex
     */
    ...
    public boolean update(File directory, String fileName, String fileExtension, int maxFiles) {
        ...
        if (updateLocation) {
            ...
            filePattern = FileLogUtils.compileLogFileRegex(fileName, fileExtension);
        ...
        if (maxFiles <= 0) {
            ...
            if (files == null) {
                ...
                String[] existing = FileLogUtils.safelyFindFiles(directory, filePattern);
                if (existing != null) {
    ...
    public File createNewFile() throws IOException {
        if (FileLogUtils.validateDirectory(directory) == null) {
            return null;
        }
    ...
    private File rollFile() throws IOException {
        ...
        if (file.isFile()) {
            ...
            if (maxFiles == 1) {
                if (!FileLogUtils.deleteFile(file)) {
        ...
        if (!file.createNewFile()) {
            Tr.error(tc, "UNABLE_TO_DELETE_RESOURCE_NOEX", new Object[] { file });
        }
    ...
    protected String getDateString() {
        synchronized (FileLogUtils.FILE_DATE) {
            return FileLogUtils.FILE_DATE.format(new Date());
        }
    }
    ...
    private boolean copyFileTo(File srcFile, File destFile) throws IOException {
        ...
        try {
            ...
            FileLogUtils.tryToClose(target);
            FileLogUtils.tryToClose(source);

            if (!success) {
                FileLogUtils.deleteFile(destFile);
                return false;
            }
        }
    ...
    private void removeFile(int index) {
        ...
        FileLogUtils.deleteFile(directory, file);
    }
}
