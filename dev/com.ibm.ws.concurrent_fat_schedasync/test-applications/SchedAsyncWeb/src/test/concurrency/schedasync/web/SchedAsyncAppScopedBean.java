...
public class SchedAsyncAppScopedBean {
     ...
     * @return null to continue with more executions; a completed future with the remaining count to stop.
     */
    ...
    CompletableFuture<Integer> everyFiveSeconds(AtomicInteger countdown) {
        System.out.println("> everyFiveSeconds " + countdown);

        CompletableFuture<Integer> result;
        if (countdown.decrementAndGet() == 0)
            result = Asynchronous.Result.complete(countdown.get());
        else
            result = null;
