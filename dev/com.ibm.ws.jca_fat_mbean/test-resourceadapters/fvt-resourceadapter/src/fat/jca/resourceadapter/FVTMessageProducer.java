...
public class FVTMessageProducer implements MessageProducer {
    ...
    private static final long TIMEOUT = 5000; // maximum amount of time we will wait for an async operation to complete
    ...
    private void notifyEndpoints(Destination destination, Message message, int deliveryMode) throws Exception {
        ...
        try {
            ...
            for (Map.Entry<ActivationSpec, MessageEndpointFactory> entry : session.con.mc.mcf.adapter.endpointFactories.entrySet()) {
                ...
                boolean match;
                match = destination instanceof Queue ? Queue.class.getName().equals(asDestType)
                                                       && ((Queue) destination).getQueueName().equals(asDestName) : Topic.class.getName().equals(asDestType)
                                                                                                                    && ((Topic) destination).getTopicName().equals(asDestName);
                if (match) {
