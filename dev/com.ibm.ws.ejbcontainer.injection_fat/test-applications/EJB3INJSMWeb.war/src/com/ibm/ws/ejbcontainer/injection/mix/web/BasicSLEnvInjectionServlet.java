...
// 1.2 REGR/ws/code/ejbcontainer.test/src/com/ibm/ws/ejbcontainer/injection/mix/BasicSLEnvInjectionTest.java, WAS.ejbcontainer.fvt, WAS855.REGR, cf141822.02 5/3/10 11:12:44
//
...
// d403414   EJB3      20061108 tkb      : remove use of lookup hack
...
// d613417   WAS70     20100430 tkb      : added field override when methods present
// --------- --------- -------- --------- -----------------------------------------
 ...
 * <li>testSLLEnvFldWithMthdInjection - Business Interface: Object and Field Injection
 * </ul>
...
public class BasicSLEnvInjectionServlet extends FATServlet {
    private static final String CLASS_NAME = BasicSLEnvInjectionServlet.class.getName();
    ...
    private static final String BasicFldWithMthdBean = "BasicSLEnvInjectFldWithMthd";
    ...
    public void testSLLEnvPrimFldInjection() throws Exception {
        ...
        assertEquals("EJB method did not return expected results", PASSED,
                     bean.verifyEnvInjection(testpoint++));
     ...
     * Test Field injection of simple environment entries on an EJB 3.0
     * Stateless Session EJB, with Business Interfaces only, for fields
     * that are the Object primitives (String, Integer, Float, etc.)
     * and primitives (int, float, etc.) and set methods are present
     * but the annotation is on the fields. <p>
     *
     * Also test that the values injected into the fields may be looked
     * up through both the global namespace and the session context. <p>
     *
     ...
     */
    @Test
    public void testSLLEnvFldWithMthdInjection() throws Exception {
        // --------------------------------------------------------------------
        // Locate SL Local Home/Factory and execute the test
        // --------------------------------------------------------------------
        //446507 update lookup for new bindings
        EnvInjectionLocal bean = (EnvInjectionLocal) FATHelper.lookupLocalBinding(BasicFldWithMthdBean);
        assertNotNull("1 ---> SLLSB accessed successfully.", bean);
