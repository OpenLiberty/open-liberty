                    ...
                    if (!handled) {
                        ...
                        if (h2Handler != null && httpInboundConnection != null && request instanceof HttpServletRequest) {
                            ...
                            if (h2Handler.isH2Request(httpInboundConnection, request)) {
                                ...
                                //Pulling POST data into WC buffer before H2 handleRequest
                                if (httpRequest.getMethod().equalsIgnoreCase("POST")) {
                                    if (com.ibm.ejs.ras.TraceComponent.isAnyTracingEnabled() && logger.isLoggable(Level.FINE)) {
                                        logger.logp(Level.FINE, CLASS_NAME, "invokeTarget", "H2 POST request; prep data before starting H2 handleRequest");
                                    }

                                    //H2 is the last in filter chain.  application's filter may have already getInputStream/getReader.
                                    //If that is the case, IllegalStateException can occur, catch and allow the H2 handleRequest to continue.
                                    try { 
                                        java.io.InputStream in = httpRequest.getInputStream();               
                                        if (!WCCustomProperties.ENABLE_MULTI_READ_OF_POST_DATA) {
                                            ...
                                            //dynamically enable the multi read ONLY in this request; reset at the next getInputStream/getReader.
