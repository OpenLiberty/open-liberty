                ...
                if (retryCount > 0) {
                    ...
                    if (unresolvedAddress) {
                        ...
                        Tr.error(tc, TCPMessageConstants.LOCAL_HOST_UNRESOLVED,
                               ...
                               && !reuseAddrRetry) {
                        // The SO_REUSEADDR=false bind failed.  This can mean either:
                        //   (a) a real conflict — another process is actively listening, or
                        //   (b) a TIME_WAIT socket from a previous server run is still lingering.
                        // Distinguish the two by probing the port with a test connection,
                        // mirroring CHFW's TCPPort.finishInitServerSocket() logic:
                        //   - probe succeeds (connection accepted) → real conflict → log CWWKO0221E
                        //   - probe fails (connection refused)     → TIME_WAIT only →
                        //       retry the bind with SO_REUSEADDR=true so the restart succeeds.
                        ...
                        if (!probeAddr.isUnresolved()) {
                            try (java.net.Socket probe = new java.net.Socket()) {
                                ...
                                // Connection succeeded — someone is actively listening.
                                realConflict = true;
                                if (TraceComponent.isAnyTracingEnabled() && tc.isDebugEnabled()) {
                                ...
                                // Connection refused — port is in TIME_WAIT only, retry with SO_REUSEADDR=true.
                                if (TraceComponent.isAnyTracingEnabled() && tc.isDebugEnabled()) {
