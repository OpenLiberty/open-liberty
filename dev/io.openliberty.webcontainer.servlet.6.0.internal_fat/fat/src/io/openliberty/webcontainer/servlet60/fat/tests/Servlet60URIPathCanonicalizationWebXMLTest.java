...
/**
 * Test web.xml setting to skip the encoded character verification in the request URI for a servlet
 *
 * <context-param>
...
@Mode(TestMode.FULL)
public class Servlet60URIPathCanonicalizationWebXMLTest {
    private static final Logger LOG = Logger.getLogger(Servlet60URIPathCanonicalizationWebXMLTest.class.getName());
    private static final String TEST_APP_NAME = "URIPathCanonicalWebXmlTest";
    private static final String WAR_NAME = TEST_APP_NAME + ".war";

    @Server("servlet60_uriPathCanonicalWebXMLTest")
    public static LibertyServer server;

    @BeforeClass
    public static void setUp() throws Exception {
        ShrinkHelper.defaultDropinApp(server, WAR_NAME, "uripathwebxml.servlets");

        server.startServer(Servlet60URIPathCanonicalizationWebXMLTest.class.getSimpleName() + ".log");
    }
     ...
     * servletPath = DecodeNormalizeURI_2
     * pathInfo = null
     */
    @Test
    public void test_URI_0() throws Exception {
        String expected = "pathInfo [null] , servletPath [/DecodeNormalizeURI_2] , contextPath [/URIPathCanonicalWebXmlTest]";

        String url = "http://" + server.getHostname() + ":" + server.getHttpDefaultPort() + "/" + TEST_APP_NAME
                     + "/DecodeNormalizeURI_2";
