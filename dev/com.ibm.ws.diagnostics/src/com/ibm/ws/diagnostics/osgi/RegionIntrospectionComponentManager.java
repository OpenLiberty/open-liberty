 ...
 *
 */
public class RegionIntrospectionComponentManager implements StaticComponentManager {

    @Override
    public ReturnValue activate(Object instance, ComponentContext componentContext) {
        ((RegionIntrospection) instance).activate(componentContext.getBundleContext());
    ...
    public ReturnValue bind(Object instance, String name, Parameters parameters) {
        if ("setDigraph".equals(name)) {
            Object[] params = parameters.getParameters(RegionDigraph.class);
            ((RegionIntrospection) instance).setDigraph((RegionDigraph) params[0]);
        }
        return ReturnValue.VOID;
    }
