...
public class TestUnmanagedJaxWsTransportSecurityServlet extends HttpServlet {
    ...
    private <T> T getAndConfigClient(HttpServletRequest req, Class<T> clazz) throws MalformedURLException {
        ...
        if ("pojo".equals(serviceType)) {
            pojoService = new SayHelloPojoService(new URL("http://" + host + ":" + req.getLocalPort() + PROVIDER_CONTEXT_ROOT
                                                          + "/unauthorized/employPojoService?wsdl"), new QName("http://ibm.com/ws/jaxws/transport/security/", "SayHelloPojoService"));
            ...
            statelessService = new SayHelloStatelessService(new URL("http://" + host + ":" + req.getLocalPort() + PROVIDER_CONTEXT_ROOT
                                                                    + "/unauthorized/employStatelessService?wsdl"), new QName("http://ibm.com/ws/jaxws/transport/security/", "SayHelloStatelessService"));
            ...
            singletonService = new SayHelloSingletonService(new URL("http://" + host + ":" + req.getLocalPort() + PROVIDER_CONTEXT_ROOT
                                                                    + "/unauthorized/employSingletonService?wsdl"), new QName("http://ibm.com/ws/jaxws/transport/security/", "SayHelloSingletonService"));
        ...
        StringBuilder sBuilder = new StringBuilder(schema).append("://").append(host).append(":").append(port).append(PROVIDER_CONTEXT_ROOT).append(requestPath);
