 ...
 *
 */
public class PrometheusMetricWriter21 extends PrometheusMetricWriter implements OutputWriter {

    private static final TraceComponent tc = Tr.register(PrometheusMetricWriter21.class);

    public PrometheusMetricWriter21(Writer writer, Locale locale) {
        super(writer,locale);
    }
    ...
    protected void writeMetricMapAsPrometheus(StringBuilder builder, String registryName, Map<MetricID, Metric> metricMap, Map<String, Metadata> metricMetadataMap) {
        for (Entry<MetricID, Metric> entry : metricMap.entrySet()) {
            ...
            if (Counter.class.isInstance(metric)) {
                PrometheusBuilder21.buildCounter(builder, metricNamePrometheus, (Counter) metric, description, tags);
            } else if (ConcurrentGauge.class.isInstance(metric)) {
                PrometheusBuilder21.buildConcurrentGauge(builder, metricNamePrometheus, (ConcurrentGauge) metric, description, tags);
            } else if (Gauge.class.isInstance(metric)) {
                PrometheusBuilder21.buildGauge21(builder, metricNamePrometheus, (Gauge) metric, description, conversionFactor, tags, appendUnit);
            } else if (Timer.class.isInstance(metric)) {
                PrometheusBuilder21.buildTimer(builder, metricNamePrometheus, (Timer) metric, description, tags);
            } else if (Histogram.class.isInstance(metric)) {
                PrometheusBuilder21.buildHistogram(builder, metricNamePrometheus, (Histogram) metric, description, conversionFactor, tags, appendUnit);
            } else if (Meter.class.isInstance(metric)) {
                PrometheusBuilder21.buildMeter(builder, metricNamePrometheus, (Meter) metric, description, tags);
            } else {
