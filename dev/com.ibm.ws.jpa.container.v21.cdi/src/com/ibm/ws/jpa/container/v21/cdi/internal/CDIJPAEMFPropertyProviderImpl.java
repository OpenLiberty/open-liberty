...
public class CDIJPAEMFPropertyProviderImpl implements JPAEMFPropertyProvider, HibernateNotifier {
    ...
    public void updateProperties(Map<String, Object> props, ClassLoader applicationClassLoader) {
        if (cdiService != null) {
            List<Class<?>> beanManagerInterfaces = new ArrayList<Class<?>>();
            ...
            if (hibernateEnabled) {
                ...
                try {
                    ...
                    beanManagerInterfaces.add(extendedBeanManagerInterface);
                    try {
                        ...
                        beanManagerInterfaces.add(depreciatedExtendedBeanManagerInterface);
                    } catch (ClassNotFoundException e) {
                        //Do nothing, if we can't find the deprecated interface we'll continue with just the main interface.
                    }
                    beanManagerInterfaces.add(BeanManager.class);
            ...
            Class<?>[] beanManagerInterfacesArray = beanManagerInterfaces.toArray(new Class<?>[beanManagerInterfaces.size()]);
            Object beanManager = Proxy.newProxyInstance(classLoader,
                                                        beanManagerInterfacesArray,
                                                        invocationHandler);
