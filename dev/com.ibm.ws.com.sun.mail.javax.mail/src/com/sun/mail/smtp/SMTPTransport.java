...
public class SMTPTransport extends Transport {
	...
	//if (logger.isLoggable(Level.FINE))
        ...
        try {
        validateCommand(cmdBytes); // Liberty change
	    serverOutput.write(cmdBytes);
	    serverOutput.write(CRLF);
	    serverOutput.flush();
	} catch (IOException | RuntimeException ex) { // Liberty change - Added RuntimeException
	    throw new MessagingException("Can't send command to SMTP host", ex);
	}
    }

    // Liberty Change start - Does not allow to add CR and LF in the command.
