...
public class ConcurrentCDI4_1Test extends FATServletClient {
    ...
    @Server("concurrent_fat_cdi4_1")
    @TestServlets({
                    @TestServlet(servlet = ConcurrentCDI4_1Servlet.class,
                                 contextRoot = APP_NAME, minJavaLevel = 17),
                    @TestServlet(servlet = ConcurrentCDI4Servlet.class,
                                 contextRoot = APP_NAME_EE10),
                    @TestServlet(servlet = ConcurrentCDIAdditionalServlet.class,
                                 contextRoot = WEBAPP_NAME_EE10)
    })
    ...
    public static void setUp() throws Exception {
        ...
        // Adding "concurrent.cu3.web" to the following would cause conflict with app-defined ManagedExecutorService.
        // There is a spec proposal to detect conflict and avoid automatically adding the bean.
        ShrinkHelper.defaultDropinApp(server, APP_NAME_EE10,
                                      "concurrent.cdi4.web");

        // Create concurrentCDIWeb2.war
        ShrinkHelper.defaultDropinApp(server, WEBAPP_NAME_EE10,
                                      "concurrent.cdi4.webapp");
        ...
        server.startServer();
        runTest(server,
                APP_NAME_EE10 + '/' + ConcurrentCDI4Servlet.class.getSimpleName(),
