...
public class CVEServiceClient {
	...
	public JSONObject retrieveCVEData(Map<String, String> data, String urlLink) throws IOException {
		...
		if (!urlLink.startsWith("https")) {
			throw new MalformedURLException("Invalid Protocol, expected \"https\': " + urlLink);
		}
		...
		JSONObject json = JSONObject.parse(getResponse(connection));
		return json;
	}
			...
			if (key == "features" || key == "iFixes") {
				...
				if (data.get(key).length() > 0) {
					...
					for (String tempString : tempArray) {
						jsonData.append("\"").append(tempString).append("\", ");
					}
