...
final class LTPADigSignatureLegacy {
    ...
    public LTPADigSignatureLegacy() {
        super();
    }

    static void generateRSAKeys(byte[][] rsaPubKey, byte[][] rsaPrivKey) {
        byte[][] rsaKey = LTPACryptoLegacy.rsaKey(128, true, true); //64 is 512, 128 is 1024
    ...
    static byte[] sign(byte[] mesg, LTPAPrivateKey privKey, boolean useJCE) throws Exception {
        ...
        LTPACryptoLegacy.setRSAKey(rsaPrivKey);
        byte[] signature = LTPACryptoLegacy.signISO9796(rsaPrivKey, data, 0, data.length);
        return signature;
    }
    ...
    static boolean verify(byte[] mesg, byte[] signature, LTPAPublicKey pubKey, boolean useJCE) throws Exception {
        ...
        return LTPACryptoLegacy.verifyISO9796(rsaPubKey, data, 0, data.length, signature, 0, signature.length);
    }
