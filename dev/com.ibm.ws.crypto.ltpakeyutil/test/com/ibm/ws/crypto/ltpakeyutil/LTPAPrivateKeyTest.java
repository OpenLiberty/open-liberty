...
public class LTPAPrivateKeyTest {
	...
	public void testRawKeyIsEncodedWithRightContents() {
		...
		try {
			...
			assertEquals("The raw key must be properly encoded with right contents.", new String(expectedEncodedKey),
					new String(actualEncodedKey));
		} catch (Throwable t) {
			outputMgr.failWithThrowable(methodName, t);
		}
	}
	...
	public void testEncodedKeyIsDecodedWithRightContents() {
		...
		try {
			...
			assertEquals("The private exponent must have the right contents.", new String(expectedPrivateExponent),
					new String(actualRawKey[1]));
			assertEquals("The public exponent must have the right contents.", new String(expectedPublicExponent),
					new String(actualRawKey[2]));
			assertEquals("The prime p must have the right contents.", new String(expectedPrimeP),
					new String(actualRawKey[3]));
			assertEquals("The prime q must have the right contents.", new String(expectedPrimeQ),
					new String(actualRawKey[4]));
		} catch (Throwable t) {
			outputMgr.failWithThrowable(methodName, t);
		}
	}
	...
	public void testGetAlgorithm() {
		...
		try {
			...
			// issue 25138
			// assertEquals("The actual algorithm must be RSA/SHA-1", expectedAlgorithm,
			// actualAlgorithm);
	...
	public void testGetFormat() {
		...
		try {
			...
			// issue 25138
			// assertEquals("The actual algorithm must be RSA/SHA-1", expectedFormat,
			// actualFormat);
	...
	private byte[] getExpectedEncodedKeyFromRawKey(byte[][] rawKey) {
		...
		int encodedKeyLength = PRIVATE_EXPONENT_LENGTH_FIELD_LENGTH + privateExponent.length + publicExponent.length
				+ primeP.length + primeQ.length;
		...
		System.arraycopy(privateExponent, 0, expectedEncodedKey, PRIVATE_EXPONENT_LENGTH_FIELD_LENGTH,
				privateExponent.length);
		System.arraycopy(publicExponent, 0, expectedEncodedKey,
				PRIVATE_EXPONENT_LENGTH_FIELD_LENGTH + privateExponent.length, publicExponent.length);
		System.arraycopy(primeP, 0, expectedEncodedKey,
				PRIVATE_EXPONENT_LENGTH_FIELD_LENGTH + privateExponent.length + publicExponent.length, primeP.length);
		System.arraycopy(primeQ, 0, expectedEncodedKey,
				PRIVATE_EXPONENT_LENGTH_FIELD_LENGTH + privateExponent.length + publicExponent.length + primeP.length,
				primeQ.length);
	...
	private byte[] getExpectedPrivateExponentFromEncodedKey(byte[] encodedKey) {
		...
		System.arraycopy(encodedKey, PRIVATE_EXPONENT_LENGTH_FIELD_LENGTH, expectedPrivateExponent, 0,
	...
	private byte[] getExpectedPrimeQFromEncodedKey(byte[] encodedKey) {
		...
		int privateExponentLength = getPrivateExponentLength(encodedKey);
		int primeQOffset = PRIVATE_EXPONENT_LENGTH_FIELD_LENGTH + privateExponentLength + EXPONENT_LENGTH
				+ PRIME_P_LENGTH;
