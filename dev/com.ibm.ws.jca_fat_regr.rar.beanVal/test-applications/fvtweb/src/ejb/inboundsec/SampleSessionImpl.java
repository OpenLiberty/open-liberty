...
public class SampleSessionImpl implements SampleSessionLocal {
    ...
    public MessageEndpointTestResultsImpl sendMessage(String deliveryId,
        ...
        try {
            ...
            testResults = (MessageEndpointTestResultsImpl) messageProvider.getTestResult(deliveryId);
        } catch (Exception e) {
            throw e;
        }
    ...
    public MessageEndpointTestResultsImpl[] sendNestedMessage(
        ...
        try {
            ...
            testResults[0] = (MessageEndpointTestResultsImpl) messageProvider.getTestResult(deliveryId);
            testResults[1] = (MessageEndpointTestResultsImpl) messageProvider.getTestResult(childDeliveryId);
        } catch (Exception e) {
            throw e;
        }
        Exception ex = messageProvider.getWork(deliveryId).getWorkException();
        Exception childEx = messageProvider.getWork(childDeliveryId).getWorkException();
        if (ex != null) {
            throw ex;
        }
        if (childEx != null) {
