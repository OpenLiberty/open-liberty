...
public class CxfX509AsyncSvcClient extends HttpServlet {
    ...
    //2/26/2021
    private static String cbhVersion = null;
    ...
    protected void doWorker(HttpServletRequest request,
        ...
        try {
            ...
            //2/26/2021
            cbhVersion = request.getParameter("CallBackhandlerVersion");
            System.out.println("In CxfX509AsyncSvcClient, after request.getParameter, CBHVersion: " + cbhVersion);
        ...
        try {
            ...
            if (rawServiceName.equals("X509AsyncService")) {
                ...
                X509AsyncService service = new X509AsyncService(wsdlURL, serviceName);
            ...
            //6/2021
            if (cbhVersion != null) {
                if (cbhVersion.equals("EE7")) {
                    requestContext.put("ws-security.callback-handler",
                                       "com.ibm.ws.wssecurity.example.cbh.CommonPasswordCallback");
                }
                if (cbhVersion.equals("EE8")) {
                    requestContext.put("ws-security.callback-handler",
                                       "com.ibm.ws.wssecurity.example.cbhwss4j.CommonPasswordCallbackWss4j");
                }
            ...
            if (testName.contains("Block")) {
                ...
                if (request.getParameter("testName").contains("testCxfAsyncInvokeNonBlocking")) {
                    System.out.println("In CxfX509AsyncSvcClient, Non-Blocking Case");
                    int waitCount = 0;
                    while (!responseRef.isDone() && waitCount < 15) {
                        //2/2021
                        System.out.println("In CxfX509AsyncSvcClient, thread sleep ... and waitCount: " + waitCount);
                        Thread.sleep(1000);
                        waitCount++;
                    }
                    if (responseRef.isDone() && waitCount < 15) {
                        //2/2201
                        System.out.println("In CxfX509AsyncSvcClient, response is done and waitCount: " + waitCount);
                        soapResp = responseRef.get();
                        System.out.println("after responseRef.get ");
                    } else {
