...
package com.ibm.ws.wssecurity.fat.derivekeyclient;
...
import test.wssecfvt.derivekey.DeriveKeyService1;
import test.wssecfvt.derivekey.DeriveKeyService2;
import test.wssecfvt.derivekey.DeriveKeyService3;
import test.wssecfvt.derivekey.DeriveKeyService4;
import test.wssecfvt.derivekey.DeriveKeyService5;
import test.wssecfvt.derivekey.DeriveKeyService6;
import test.wssecfvt.derivekey.DeriveKeyService7;

/**
 * Servlet implementation class CxfDeriveKeySvcClient
 */
@WebServlet("/CxfDeriveKeySvcClient")
public class CxfDeriveKeySvcClient extends HttpServlet {

    static String strJksLocation = "sslServerTrust.jks";
    private static final long serialVersionUID = 1L;
    private static final String SERVICE_NS = "http://derivekey.wssecfvt.test";
    ...
    static String simpleSoapBody = "<soapenv:Envelope xmlns:soapenv=\"http://schemas.xmlsoap.org/soap/envelope/\">"
                                   + "<soapenv:Header/>"
                                   + "<soapenv:Body xmlns=\"http://derivekey.wssecfvt.test/types\">"
                                   + "<invoke>WSSECFVT CXF DeriveKeyClient Simple Soap Msg</invoke>"
                                   + "</soapenv:Body>"
                                   + "</soapenv:Envelope>";
 ...
 */
        ...
        wsdlLocation = theType + "://localhost:" + thePort + "/derivekey/"
                       + rawServiceName + "?wsdl";
        ...
        try {
            ...
            String endPtUrl = theType + "://localhost:" + thePort + "/derivekey/" + rawServiceName;
            // setup to handle multiple services later
            if (rawServiceName.equals("DeriveKeyService1")) {
                System.out.println("In DeriveKeyService1 case");
                DeriveKeyService1 service = new DeriveKeyService1(wsdlURL, serviceName);
                service.addPort(servicePort, SOAPBinding.SOAP11HTTP_BINDING, endPtUrl);
                dispSOAPMsg = service.createDispatch(servicePort,
                                                     SOAPMessage.class, Mode.MESSAGE);
            }
            if (rawServiceName.equals("DeriveKeyService2")) {
                System.out.println("In DeriveKeyService2 case");
                DeriveKeyService2 service = new DeriveKeyService2(wsdlURL, serviceName);
                service.addPort(servicePort, SOAPBinding.SOAP11HTTP_BINDING, endPtUrl);
                dispSOAPMsg = service.createDispatch(servicePort,
                                                     SOAPMessage.class, Mode.MESSAGE);
            }
            if (rawServiceName.equals("DeriveKeyService3")) {
                System.out.println("In DeriveKeyService3 case");
                DeriveKeyService3 service = new DeriveKeyService3(wsdlURL, serviceName);
                service.addPort(servicePort, SOAPBinding.SOAP11HTTP_BINDING, endPtUrl);
                dispSOAPMsg = service.createDispatch(servicePort,
                                                     SOAPMessage.class, Mode.MESSAGE);
            }
            if (rawServiceName.equals("DeriveKeyService4")) {
                System.out.println("In DeriveKeyService4 case");
                DeriveKeyService4 service = new DeriveKeyService4(wsdlURL, serviceName);
                service.addPort(servicePort, SOAPBinding.SOAP11HTTP_BINDING, endPtUrl);
                dispSOAPMsg = service.createDispatch(servicePort,
                                                     SOAPMessage.class, Mode.MESSAGE);
            }
            if (rawServiceName.equals("DeriveKeyService5")) {
                System.out.println("In DeriveKeyService5 case");
                DeriveKeyService5 service = new DeriveKeyService5(wsdlURL, serviceName);
                service.addPort(servicePort, SOAPBinding.SOAP11HTTP_BINDING, endPtUrl);
                dispSOAPMsg = service.createDispatch(servicePort,
                                                     SOAPMessage.class, Mode.MESSAGE);
            }
            if (rawServiceName.equals("DeriveKeyService6")) {
                System.out.println("In DeriveKeyService6 case");
                DeriveKeyService6 service = new DeriveKeyService6(wsdlURL, serviceName);
                service.addPort(servicePort, SOAPBinding.SOAP11HTTP_BINDING, endPtUrl);
                dispSOAPMsg = service.createDispatch(servicePort,
                                                     SOAPMessage.class, Mode.MESSAGE);
            }
            if (rawServiceName.equals("DeriveKeyService7")) {
                System.out.println("In DeriveKeyService7 case");
                DeriveKeyService7 service = new DeriveKeyService7(wsdlURL, serviceName);
                service.addPort(servicePort, SOAPBinding.SOAP11HTTP_BINDING, endPtUrl);
                dispSOAPMsg = service.createDispatch(servicePort,
                                                     SOAPMessage.class, Mode.MESSAGE);
            }
            ...
            rsp.print("<html><head><title>CXF Key Derivation Service Cleint</title></head><body>");
            rsp.print("<p>Request: CXF Key Derivation Service Client</p>");
