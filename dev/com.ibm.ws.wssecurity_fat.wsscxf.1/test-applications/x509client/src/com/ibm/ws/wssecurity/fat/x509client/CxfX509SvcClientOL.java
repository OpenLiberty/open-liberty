 ...
 */
//2/2021 created
@WebServlet("/CxfX509SvcClientOL")
public class CxfX509SvcClientOL extends HttpServlet {
    ...
    protected void doWorker(HttpServletRequest request,
        ...
        try {
            ...
            featureSet = request.getParameter("featureSet");
        ...
        endPoint = httpProtocal + "//localhost:"
                   ...
                   + rawServiceName;
        try {
            if (thisMethod.equals("testCxfX509OLCallback")) {
                testCxfX509OLCallback(request, response);
            }
     ...
     *      response)
     */
    protected void testCxfX509OLCallback(HttpServletRequest request,
                                         HttpServletResponse response) throws ServletException, IOException {
 ...
 */
            ...
            PrintWriter rsp = response.getWriter();
            if (testMode == null || testMode.equals("positive")) {
                ex.printStackTrace();
                rsp.print("<html><head/><body>");
    ...
    void setCryptoObjectsOnRequestContext(Map<String, Object> requestContext) throws Exception {
        ...
        requestContext.put("ws-security.callback-handler", "com.ibm.ws.wssecurity.example.cbhol.CommonPasswordCallbackOL");
        String strServerDir = System.getProperty("server.config.dir").replace('\\', '/');
        ...
        if (featureSet == "EmptyAction") { // For Jaxws-2.2 we are using different crypto class
            org.apache.ws.security.components.crypto.Crypto crypto = org.apache.ws.security.components.crypto.CryptoFactory.getInstance(getProperties(strX509JksSignLocation,
                                                                                                                                                      strServerDir));
            org.apache.ws.security.components.crypto.Crypto crypto2 = org.apache.ws.security.components.crypto.CryptoFactory.getInstance(getProperties(strX509JksEncrLocation,
                                                                                                                                                       strServerDir));
            requestContext.put("ws-security.signature.crypto", crypto);
            requestContext.put("ws-security.encryption.crypto", crypto2);
        } else {
            org.apache.wss4j.common.crypto.Crypto crypto = org.apache.wss4j.common.crypto.CryptoFactory.getInstance(getProperties(strX509JksSignLocation, strServerDir));
            org.apache.wss4j.common.crypto.Crypto crypto2 = org.apache.wss4j.common.crypto.CryptoFactory.getInstance(getProperties(strX509JksEncrLocation, strServerDir));
            requestContext.put("ws-security.signature.crypto", crypto);
            requestContext.put("ws-security.encryption.crypto", crypto2);
        }
    ...
    void setClientX509KeyProps(Map<String, Object> requestContext) throws Exception {
        ...
        requestContext.put("ws-security.callback-handler", "com.ibm.ws.wssecurity.example.cbhol.CommonPasswordCallbackOL"); // The callback
