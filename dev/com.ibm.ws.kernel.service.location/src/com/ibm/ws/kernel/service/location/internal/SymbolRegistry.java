...
public class SymbolRegistry {
	...
	/** HashMap of key (prefixes) and value (SymbolResolvers) */
	private Map<String, SymbolResolver> symbolMap = new ConcurrentHashMap<String, SymbolResolver>();
	...
	protected void addSymbolResolver(SymbolResolver symbolResolverIN) {
		List<String> prefixList = symbolResolverIN.getSupportedPrefixes();
		for (String prefix : prefixList){
			symbolMap.put(prefix, symbolResolverIN);
		}
	}
		...
		if (m.find()) {
			...
			if (entry != null) {
				...
				if (value != null) {
					...
					// Finds SymbolResolver in map that can handle resolving symbol
					SymbolResolver symbolResolver = findResolverInMap(symbol);
					
					// Resolve symbol if correct SymbolResolver is found 
					if (symbolResolver != null) {
						// Remove prefix and resolve symbol
						value = symbolResolver.resolveSymbol(symbol.substring(4));
					}
					
					// Replace symbol in path with resolved value
					if (value != null) {
						return replaceAndResolveString(originalPath, path, includeRoots, recurse, m, value, pathNormalize);
					}
				}
	...
	/**
	 * Helper method for resolving string. Iterates through symbol map and finds prefix that matches symbol
	 * @param symbol
	 * @return SymbolResolver object
	 */
	public SymbolResolver findResolverInMap(String symbol) {
		SymbolResolver resolver = null;
		for (String key : symbolMap.keySet()) {
			if (symbol.startsWith(key)){
				resolver = symbolMap.get(key);
				break;
			}
		}
		return resolver;
