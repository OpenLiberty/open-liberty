...
import com.ibm.ws.security.authorization.jacc.sl.role.FileRoleMapping;
...
public class JaccProvider {
            ...
            if (roles != null) {
                for (String role : requiredRoleList) {
                    if (roles.contains(role)) {
                        System.out.println("Everyone is granted access.");
                        return true;
                    }
                }
            }
    ...
    public boolean checkRolePerm(WSPolicyConfigurationImpl pc, Permission p, String contextId) {
        ...
        for (String role : requiredRoleList) {
            if (ALL_AUTHENTICATED_ROLE.equals(role)) {
                System.out.println("granted access to the authenticated user (**).");
                return true;
            }
        }
        ...
        if (roles != null && !roles.isEmpty()) {
            for (String role : requiredRoleList) {
                if (ALL_AUTHENTICATED_ROLE.equals(role) || roles.contains(role)) {
                    System.out.println("granted access for the user. Role matches.");
                    return true;
                }
            }
        }
        ...
        if (groupIds != null && !groupIds.isEmpty()) {
            for (String groupId : groupIds) {
                ...
                if (roles != null && !roles.isEmpty()) {
                    for (String role : requiredRoleList) {
                        if (ALL_AUTHENTICATED_ROLE.equals(role) || roles.contains(role)) {
                            System.out.println("granted access for the group. Role matches.");
                            return true;
                        }
                    }
                }
            ...
            for (Entry<String, List<Permission>> e : roleToPermMap.entrySet()) {
                ...
                for (Permission perm : rList) {
                    if (perm.implies(p)) {
                        ...
                        System.out.println("Added role: " + role + " to the requiredRoleList for Permission: " + perm);
                    }
                }
            }
        }
        System.out.println("getRequiredRoleList : " + requiredRoleList);
    ...
    private String getAccessId(WSCredential cred) {
        ...
        System.out.println("getAccessId ; " + accessId);
        return accessId;
    }
