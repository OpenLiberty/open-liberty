...
package com.ibm.ws.security.authorization.jacc.sl.role;
...
public class FileRoleMapping {
    ...
    public static FileRoleMapping getInstance() {
        if (instance == null) {
            roleMappingFile = System.getProperty("roleMappingFile");
            System.out.println("roleMappingFile : " + roleMappingFile);
            instance = new FileRoleMapping();
        }
        return instance;
    }
    ...
    private List<String> getRoles(String applicationName, String accessId, Type type) {
                ...
                if (!(s.startsWith("#") || s.trim().length() <= 0)) {
                    ...
                    if (list.length == 4) {
                        if (list[0].equals(applicationName) && list[2].equals(getTypeString(type)) && list[3].equals(accessId)) {
                            ...
                            output.add(list[1]);
                            System.out.println("getRoles : " + list[1]);
                        }
                    }
                }
            }
        ...
        System.out.println("getRoles return :" + output);
        return output;
    }
