...
import io.openliberty.microprofile.config.internal_fat.apps.varExpansion.VarExpansionServlet;

/**
 * FAT test class for variable expansion in MicroProfile Config 2.0+.
 */
...
public class VarExpansionTest extends FATServletClient {
    ...
    @ClassRule
    public static RepeatTests r = ConfigRepeatActions.repeatDefault20Up(SERVER_NAME);

    @BeforeClass
    public static void setUp() throws Exception {
        DeployOptions[] options = { DeployOptions.SERVER_ONLY };
        ShrinkHelper.defaultApp(server, APP_NAME, options,
                                new String[] { "io.openliberty.microprofile.config.internal_fat.apps.varExpansion" });
        server.startServer();
    }
