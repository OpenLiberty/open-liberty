...
class RLSSuspendTokenManager implements AlarmListener {
    ...
    private RLSSuspendTokenManager() {
        ...
        _tokenMap = new HashMap<RLSSuspendToken, Alarm>();
    ...
    RLSSuspendToken registerSuspend(int timeout) {
        if (tc.isEntryEnabled())
            Tr.entry(tc, "registerSuspend", timeout);
        ...
        if (timeout > 0) {
            ...
            alarm = Configuration.getAlarmManager().scheduleAlarm(timeout * 1000L, this, token);
        ...
        if (tc.isEntryEnabled())
            Tr.exit(tc, "registerSuspend", token);
    ...
    void registerResume(RLSSuspendToken token) throws RLSInvalidSuspendTokenException {
        if (tc.isEntryEnabled())
            Tr.entry(tc, "registerResume", token);
        ...
        if (token != null && _tokenMap.containsKey(token)) {
            ...
            synchronized (_tokenMap) {
                ...
                Alarm alarm = _tokenMap.remove(token /* tokenImpl */);
