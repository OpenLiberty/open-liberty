...
public class Lock {
    ...
    public Lock(String ownerIdentifier) {
        if (tc.isEntryEnabled())
            Tr.entry(tc, "Lock", ownerIdentifier);

        _sharedThreads = new HashMap<Thread, Integer>();
            ...
            if ((!_sharedThreads.containsKey(currentThread)) &&
                ...
                while ((_threadHoldingExclusiveLock != null) || (_threadRequestingExclusiveLock != null)) {
                    if (tc.isDebugEnabled())
                        Tr.debug(tc, "Thread " + Integer.toHexString(currentThread.hashCode()) + " is waiting for the exclusive lock to be released");
                    try {
                        if (tc.isDebugEnabled())
                            Tr.debug(tc, "Thread " + Integer.toHexString(currentThread.hashCode()) + " waiting..");
                        this.wait();
                ...
                if (tc.isDebugEnabled())
                    Tr.debug(tc, "Thread " + Integer.toHexString(currentThread.hashCode()) + " is has detected that the exclusive lock has been released");
            }

            count = _sharedThreads.get(currentThread);
    ...
    public void releaseSharedLock(int requestId) throws NoSharedLockException {
        ...
        synchronized (this) {
            Integer count = _sharedThreads.get(currentThread);
    ...
    public boolean attemptExclusiveLock() throws HoldingExclusiveLockException {
        ...
        synchronized (this) {
            if ((_threadHoldingExclusiveLock != null) && (_threadHoldingExclusiveLock.equals(currentThread))) {
                ...
                if ((_threadHoldingExclusiveLock != null) || (_threadRequestingExclusiveLock != null)) {
                    ...
                    Integer suspendedSharedLockCount = _sharedThreads.get(currentThread);
                    _sharedThreads.remove(currentThread);
                    this.notifyAll();
                    ...
                    if (tc.isDebugEnabled())
                        Tr.debug(tc, "Thread " + Integer.toHexString(currentThread.hashCode()) + " has detected that the exclusive lock has been released");

                    // Resume the shared locks that were suspened prior to the wait
                    if (suspendedSharedLockCount != null)
                        _sharedThreads.put(currentThread, suspendedSharedLockCount); //  PK96860
                    ...
                    if (tc.isDebugEnabled())
                        Tr.debug(tc, "Thread " + Integer.toHexString(currentThread.hashCode()) + " is waiting for the shared locks to quiesce");
                    ...
                    if (tc.isDebugEnabled())
                        Tr.debug(tc, "Thread " + Integer.toHexString(currentThread.hashCode()) + " has detected that the shared locks have quiesced");
        ...
        if (tc.isEntryEnabled())
            Tr.exit(tc, "attemptExclusiveLock", "" + obtained);
