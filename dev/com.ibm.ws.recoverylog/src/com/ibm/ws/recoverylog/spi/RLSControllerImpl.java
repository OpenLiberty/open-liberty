...
public class RLSControllerImpl implements RLSController {
    ...
    RLSControllerImpl() {
        if (tc.isDebugEnabled())
            Tr.debug(tc, "RLSControllerImpl", this);
    }
    ...
    private static void resumeRLS(RLSSuspendToken token) throws RLSInvalidSuspendTokenException {
        ...
        synchronized (SUSPEND_LOCK) {
            try {
                ...
                if (tc.isEventEnabled())
                    Tr.event(tc, "Rethrow RLSInvalidSuspendTokenException - attempt to resume an invalid token", token);
                if (tc.isEntryEnabled())
                    Tr.exit(tc, "resumeRLS", exc);
                throw exc;
            }
    ...
    static boolean isSuspended() {
        if (tc.isDebugEnabled())
            Tr.debug(tc, "isSuspended {0}", _isSuspended);
