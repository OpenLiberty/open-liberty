...
public class LogCursorImpl implements LogCursor {
     ...
     * is supported.
     */
    private final boolean _removeSupported;
    ...
    public LogCursorImpl(Lock controlLock, Object object) {
        if (tc.isEntryEnabled())
            Tr.entry(tc, "LogCursorImpl", controlLock, object);
        ...
        if (tc.isEntryEnabled())
            Tr.exit(tc, "LogCursorImpl", new Object[] { _initialSize, this });
    }
    ...
    public LogCursorImpl(Lock controlLock, Collection collection, boolean removeSupported, LogCursorCallback callback) {
        ...
        if (tc.isDebugEnabled())
            Tr.debug(tc, "LogCursorImpl", controlLock, collection, removeSupported, callback, this);
    }
    ...
    public LogCursorImpl(Lock controlLock, Collection collection1, Collection collection2, boolean removeSupported, LogCursorCallback callback) {
        ...
        if (collection1 != null) {
            ...
            if (tc.isDebugEnabled())
                Tr.debug(tc, "Collection 1 size: " + collection1Size);
        ...
        if (collection2 != null) {
            ...
            if (tc.isDebugEnabled())
                Tr.debug(tc, "Collection 2 size: " + collection2Size);
    ...
    public Object next() {
        ...
        if (tc.isEntryEnabled())
            Tr.exit(tc, "next", preprocessedResult);
    ...
    private Object preprocessResult(Object result) {
        if (tc.isEntryEnabled())
            Tr.entry(tc, "preprocessResult", new Object[] { result, this });
        ...
        if (tc.isEntryEnabled())
            Tr.exit(tc, "preprocessResult", processedResult);
