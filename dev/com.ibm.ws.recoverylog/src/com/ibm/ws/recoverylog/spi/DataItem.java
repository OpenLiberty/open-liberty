...
public class DataItem {
    private static final TraceComponent tc = Tr.register(DataItem.class, TraceConstants.TRACE_GROUP, TranConstants.NLS_FILE);
    ...
    public DataItem(int storageMode, byte[] data, RecoverableUnitSectionImpl rus) {
        if (tc.isEntryEnabled())
            Tr.entry(tc, "DataItem", storageMode, RLSUtils.toHexString(data, RLSUtils.MAX_DISPLAY_BYTES), rus);
    ...
    public DataItem(int storageMode, ReadableLogRecord logRecord, RecoverableUnitSectionImpl rus) throws InternalLogException {
        if (tc.isEntryEnabled())
            Tr.entry(tc, "DataItem", storageMode, logRecord, rus);
        ...
        try {
            if (tc.isDebugEnabled())
                Tr.debug(tc, "Reading data size field @ position " + logRecord.position());

            _dataSize = logRecord.getInt();

            if (tc.isDebugEnabled())
                Tr.debug(tc, "This data item contains " + _dataSize + " bytes of data");
            ...
            if (storageMode == MultiScopeRecoveryLog.MEMORY_BACKED) {
                if (tc.isDebugEnabled())
                    Tr.debug(tc, "Reading " + _dataSize + "bytes of data @ position " + logRecord.position());
                _data = new byte[_dataSize];
                _logRecord.get(_data);
            } else {
                if (tc.isDebugEnabled())
                    Tr.debug(tc, "Tracking " + _dataSize + "bytes of data @ position " + logRecord.position());
    ...
    protected void write(WriteableLogRecord logRecord) {
        if (tc.isEntryEnabled())
            Tr.entry(tc, "write", new Object[] { logRecord, this });
        ...
        if (tc.isDebugEnabled())
            Tr.debug(tc, "Writing '" + data.length + "' bytes " + RLSUtils.toHexString(data, RLSUtils.MAX_DISPLAY_BYTES));
    ...
    protected byte[] getData() {
        ...
        if (data != null) {
            ...
            if (_storageMode == MultiScopeRecoveryLog.FILE_BACKED) {
                ...
                if (_filePosition != UNWRITTEN) {
                    if (tc.isDebugEnabled())
                        Tr.debug(tc, "Retrieving " + _dataSize + " bytes of data @ position " + _filePosition);
        ...
        if (tc.isEntryEnabled())
            Tr.exit(tc, "getData", new Object[] { data.length, RLSUtils.toHexString(data, RLSUtils.MAX_DISPLAY_BYTES) });
        return data;
    }
}
