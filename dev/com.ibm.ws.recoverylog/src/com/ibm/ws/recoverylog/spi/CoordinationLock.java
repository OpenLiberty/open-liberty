...
public class CoordinationLock {
    ...
    */
     ...
     */
    final static int LOCKRETRYCOUNT = 1;
    ...
    */
    ...
    public int lock() {
        ...
        if (tc.isEntryEnabled())
            Tr.exit(tc, "lock", new Integer(result));
        return result;
    }
    ...
    private int obtainLock(CoordinationLockHandle handle) {
        ...
        try {
            ...
            try {
                ...
                handle._raf = (RandomAccessFile) Configuration.getAccessController().doPrivileged(
...
//                    RandomAccessFile raf = new RandomAccessFile(file, "rw");
            ...
            while (!halt) {
                ...
                if (tc.isDebugEnabled())
                    Tr.debug(tc, "RLSHA: Lock attempt #" + lockAttempt + " on lock file " + handle);

                handle._fileLock = handle._channel.tryLock();

                if (handle._fileLock != null) {
                    if (tc.isDebugEnabled())
                        Tr.debug(tc, "RLSHA: Obtained an exclusive access lock on lock file " + handle);
                    halt = true;
                    result = LOCK_SUCCESS;
                } else {
                    if (tc.isDebugEnabled())
                    ...
                    if ((LOCKRETRYCOUNT <= 0) || (lockAttempt < LOCKRETRYCOUNT)) {
                        if (LOCKRETRYDELAY > 0) {
                            ...
                            try {
                                if (tc.isDebugEnabled())
                                    Tr.debug(tc, "Sleeping for " + LOCKRETRYDELAY + " ms before re-try");
                        ...
                        if (tc.isDebugEnabled())
                            Tr.debug(tc, "Retry count exceeded, aborting lock attempt on lock file " + handle);
                        halt = true;
                        try // @255605A
                        {
                            handle._raf.close();
            ...
            if (tc.isDebugEnabled())
                Tr.debug(tc, "An unexpected exception has occured when trying to obtain the exclusive lock " + exc);
        ...
        if (tc.isEntryEnabled())
            Tr.exit(tc, "obtainLock", new Integer(result));
        return result;
    }
    ...
    */
    public CoordinationLockHandle ensureFileExists(String lockFile) {
        if (tc.isEntryEnabled())
            Tr.entry(tc, "ensureFileExists", lockFile);
        ...
        if (tc.isEntryEnabled())
            Tr.exit(tc, "ensureFileExists", clh);
        return clh;
    }
