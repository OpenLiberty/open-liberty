...
public class SingleDataItem extends DataItem {
    private static final TraceComponent tc = Tr.register(SingleDataItem.class, TraceConstants.TRACE_GROUP, TraceConstants.NLS_FILE);
    ...
    public SingleDataItem(int storageMode, byte[] data, RecoverableUnitSectionImpl rus) {
        ...
        if (tc.isDebugEnabled())
            Tr.debug(tc, "SingleDataItem", storageMode, RLSUtils.toHexString(data, RLSUtils.MAX_DISPLAY_BYTES), rus, this);
    }
    ...
    public SingleDataItem(int storageMode, ReadableLogRecord logRecord, RecoverableUnitSectionImpl rus) throws InternalLogException {
        ...
        if (tc.isDebugEnabled())
            Tr.debug(tc, "SingleDataItem", storageMode, logRecord, rus, this);
    }
    ...
    protected void setData(byte[] data) {
        if (tc.isEntryEnabled())
            Tr.entry(tc, "setData", RLSUtils.toHexString(data, RLSUtils.MAX_DISPLAY_BYTES), this);
    ...
    protected void setData(ReadableLogRecord logRecord) /* @MD19840AA */
        ...
        if (tc.isEntryEnabled())
            Tr.entry(tc, "setData", this, logRecord);
