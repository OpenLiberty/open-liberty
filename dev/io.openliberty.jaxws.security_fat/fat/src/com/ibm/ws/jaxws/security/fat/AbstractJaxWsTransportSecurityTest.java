...
abstract public class AbstractJaxWsTransportSecurityTest {
	...
	protected void prepareForTest(String serverConfigFile, String providerWebXMLFile, String clientBindingFile)
			throws Exception {
		if (dynamicUpdate) {
		...
		if (null == newFile) {
			...
			try {
				...
				while (!(success = file.delete()) && count < 5) {
					Thread.sleep(1000);
					++count;
					Log.info(AbstractJaxWsTransportSecurityTest.class, "tryUpdateServerFile",
	...
	protected static void updateServerConfigFile(String newServerConfigFile, boolean checkAppUpdate) throws Exception {
		...
		try {
			...
			if (null == lastServerConfig) {// The first time to update server config file
				...
				if (SERVER_CONFIG_WITH_SSL.contains(newServerConfigFile)
						&& SERVER_CONFIG_WITHOUT_SSL.contains(lastServerConfig)) {
					Log.info(AbstractJaxWsTransportSecurityTest.class, "updateServerConfigFile",
							"Wait for ssl port open.");
					isFound = null != server.waitForStringInLogUsingMark("CWWKO0219I:.*-ssl");
				}
	 ...
	 * @throws IOException
	 */
	protected void runTest(RequestParams params, String expectedResp, List<String> expectedServerInfos)
			throws Exception, ProtocolException, MalformedURLException, IOException {
	 ...
	 * @throws IOException
	 */
	protected void runTest(RequestParams params, List<String> expectedResponses, List<String> expectedServerInfos)
			throws Exception, ProtocolException, MalformedURLException, IOException {
//        server.setMarkToEndOfLog();

		StringBuilder urlBuilder = new StringBuilder("http://").append(server.getHostname()).append(":")
				.append(server.getHttpDefaultPort()).append(SERVLET_PATH).append("?user=").append(params.userName)
				.append("&serviceType=").append(params.serviceType).append("&schema=").append(params.schema)
				.append("&port=").append(params.port).append("&path=").append(params.path).append("&testMethod=")
		...
		if (null != expectedResponses) {
			assertTrue(printExpectedResponses(expectedResponses, false),
					checkExpectedResponses(urlBuilder.toString(), expectedResponses, false));
		}
	...
	private boolean checkExpectedResponses(String servletUrl, List<String> expectedResponses, boolean exact)
			throws IOException {
