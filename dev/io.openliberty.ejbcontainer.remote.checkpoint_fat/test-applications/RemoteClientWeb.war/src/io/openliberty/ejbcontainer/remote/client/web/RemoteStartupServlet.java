 ...
 *******************************************************************************/
package io.openliberty.ejbcontainer.remote.client.web;
...
import io.openliberty.ejbcontainer.remote.server.shared.TxAttrEJB;
import io.openliberty.ejbcontainer.remote.server.shared.TxAttrEJBHome;
import io.openliberty.ejbcontainer.remote.server.shared.TxAttrRemote;
import test.TestRemoteInterface;

/**
 * Tests variations of looking up remote enterprise beans located on a different server
 * and whether the EJB container performs the correct action for resolving remote EJBs
 * at application startup. for each of the possible TX
 */

@WebServlet(urlPatterns = { "/RemoteStartupServlet" }, loadOnStartup = 1)
public class RemoteStartupServlet extends RemoteTxAttrServlet {
    ...
    /**
     * Attempt to resolve a remote EJB during checkpoint.
     */
    @Override
    public void init() {
        System.out.println("--- RemoteStartupServlet init starting ---");
        try {
            getTxAttrBean();
        } catch (Exception e) {
            System.out.println("--- RemoteStartupServlet init completed with exception: " + e + "---");
            e.printStackTrace(System.out);
            return;
        }
        System.out.println("--- RemoteStartupServlet init completed without exception ---");
    }
