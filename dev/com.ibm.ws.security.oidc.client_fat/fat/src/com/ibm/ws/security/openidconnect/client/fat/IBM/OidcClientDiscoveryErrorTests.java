...
/**
 * This is the test class that will run error scenarios with OpenID Connect RP tests with an RP configuration (using access token and X509 cert) which includes discovery of OP endpoints.
 ...
 *     
 * This test configures the sslRef as part of the openidConnectClient configuration so that the SSL processing is handled by the client rather than
...
@Mode(TestMode.FULL)
@AllowedFFDC({ "org.apache.http.NoHttpResponseException","java.security.cert.CertPathBuilderException", "sun.security.validator.ValidatorException", "javax.net.ssl.SSLHandshakeException", "com.ibm.websphere.ssl.SSLException" })
...
public class OidcClientDiscoveryErrorTests extends CommonTest {
     ...
     * <LI>CWWKS1524E: The OpenID Connect client failed to obtain OpenID Connect Provider information through the discovery endpoint...
     * </OL>
     */
    @Test
    ...
    public void OidcClientDiscoverySSLTest_NoRPTrustStore() throws Exception {
        ...
        List<String> rpWaitForMsgs = Arrays.asList("CWWKS1524E");

        testOPServer.reconfigServer("op_server_basic_ssl.xml", _testName, Constants.JUNIT_REPORTING, null);
        testRPServer.reconfigServer("rp_server_basic_ssl_no_truststore.xml", _testName, Constants.JUNIT_REPORTING, rpWaitForMsgs);
     ...
     */
    //@Test
    // TODO: This test encounters an error when run with other tests as SSL update timing differs. 
    @AllowedFFDC({ "java.security.cert.CertPathBuilderException", "sun.security.validator.ValidatorException", "javax.net.ssl.SSLHandshakeException", "com.ibm.websphere.ssl.SSLException"})
    public void OidcClientDiscoverySSLTest_BadRPTrustStore() throws Exception {
       ...
       testOPServer.reconfigServer("op_server_basic_ssl.xml", _testName, Constants.JUNIT_REPORTING, null);
       testRPServer.reconfigServer("rp_server_basic_ssl_bad_truststore.xml", _testName, Constants.JUNIT_REPORTING, rpWaitForMsgs);
     ...
     * Expected Results:
     * <OL>
     * <LI>The discovery should be attempted and should fail with 401 when the RP tries process the first authentication request and the following messages should be logged
    ...
    public void OidcClientDiscoverySSLTest_BadOPTrustStoreWithClientAuth() throws Exception {
        ...
        List<String> rpWaitForMsgs = Arrays.asList("CWWKS1525E","CWWKS1524E");

        testOPServer.reconfigServer("op_server_ssl_bad_truststore_clientAuth.xml", _testName, Constants.JUNIT_REPORTING, null);
        testRPServer.reconfigServer("rp_server_basic_ssl.xml", _testName, Constants.JUNIT_REPORTING, rpWaitForMsgs);
        ...
        testOPServer.addIgnoredServerExceptions("CWPKI0022E", "CWWKO0801E");
     ...
     * <LI>CWWKS1525E: A successful response was not returned from the URL [https://localhost:8947/oidc/endpoint/OidcConfigSample/.well-known/openid-configuration]. This is the [404] response status and the [CWOAU0073E: An error was encountered while authenticating a user. 
     *       Please try authenticating again, or contact the site administrator if the problem persists.] error from the discovery request.
     * <LI>CWWKS1524E: The OpenID Connect client [client01] failed to obtain Open ID Connect Provider endpoint information through the discovery endpoint URL [https://localhost:8947/oidc/endpoint/OidcConfigSample/.mal-formed/openid-configuration]. 
    ...
    public void OidcClientDiscoverySSLTest_NoOPTrustStore() throws Exception {
        ...
        testOPServer.reconfigServer("op_server_basic_ssl_no_truststore.xml", _testName, Constants.JUNIT_REPORTING, null);
        testRPServer.reconfigServer("rp_server_basic_ssl.xml", _testName, Constants.JUNIT_REPORTING, rpWaitForMsgs);
        ...
        testOPServer.addIgnoredServerExceptions("CWWKG0058E");
    ...
    public void OidcClientDiscoverySSLTest_DiscoveryEndpointNotHTTPS() throws Exception {
	    ...
	    testOPServer.reconfigServer("op_server_basic_ssl.xml", _testName, Constants.JUNIT_REPORTING, null);
	    testRPServer.reconfigServer("rp_server_basic_ssl_nohttps.xml", _testName, Constants.JUNIT_REPORTING, rpWaitForMsgs);
     ...
     * Test Purpose:
     * <OL>
     * <LI>This is an error scenario where the discovery endpoint is mal-formed and does not have the required syntax (.mal-formed instead of .well-known)
     ...
     * <OL>
     * <LI>The discovery should be attempted and should fail with 401 when the RP tries process the first authentication request and the following messages should be logged
     * <LI>CWWKS1525E: A successful response was not returned from the URL [https://localhost:8947/oidc/endpoint/OidcConfigSample/.mal-formed/openid-configuration]. This is the [404] response status and the [CWOAU0073E: An error was encountered while authenticating a user. 
     *       Please try authenticating again, or contact the site administrator if the problem persists.] error from the discovery request.
     * <LI>CWWKS1524E: The OpenID Connect client failed to obtain OpenID Connect Provider information through the discovery endpoint...
     * </OL>
     */
    ...
    public void OidcClientDiscoverySSLTest_DiscoveryEndpointMalFormed() throws Exception {
        ...
        // Reconfigure OP server with SSL settings
        // Reconfigure RP server with SSL settings
	     List<String> rpWaitForMsgs = Arrays.asList("CWWKS1524E","CWWKS1525E");

	    testOPServer.reconfigServer("op_server_basic_ssl.xml", _testName, Constants.JUNIT_REPORTING, null);
	    testRPServer.reconfigServer("rp_server_basic_ssl_malform.xml", _testName, Constants.JUNIT_REPORTING, rpWaitForMsgs);
     ...
     * Test Purpose:
     * <OL>
     * <LI>With discovery configured along with other endpoints, attempt to access a test servlet specifying both a discovery endpoint and authorization, token, userinfo and jwk endpoints in the RP config.
     * <LI>In this scenario, the consent form is disabled in OP by setting the oauthProvider attribute in server.xml as: autoAuthorize="true"
     * <LI>A message is logged to indicate that discovery is processed and the other configured endpoints will be ignored. 
     * <LI>
     * <LI>Test is showing a good main path flow between RP and OPusing discovered endpoints. In this scenario, a message is logged to indicate that the values
    ...
    public void OidcClientDiscoveryTestDiscoveredEndpointsOverrideConfiguredEndpoints() throws Exception {
        ...
        List<String> rpWaitForMsgs = Arrays.asList("CWWKS1521W");

        testOPServer.reconfigServer("op_server_orig.xml", _testName, Constants.JUNIT_REPORTING, null);
        testRPServer.reconfigServer("rp_server_discovery_conflict.xml", _testName, Constants.JUNIT_REPORTING, rpWaitForMsgs);
    	 ...
    	 * CWWKS1522W: The OpenId Connect client (Relying Party or RP) [client01] configuration specifies both the discovery endpoint URL [discoveryEndpointUrl] and the issuer identifier [issuerIdentifier]. 
    	...
    	public void OidcClientDiscoveryTestWithDiscoveredIssuerIdentifierFromOPConfigOverridesRPConfig() throws Exception {
    	    ...
    	    List<String> rpWaitForMsgs = Arrays.asList("CWWKS1522W");

    	    testOPServer.reconfigServer("op_server_badissuer_ssl.xml", _testName, Constants.JUNIT_REPORTING, null);
    	    testRPServer.reconfigServer("rp_server_basic_issident_ssl.xml", _testName, Constants.JUNIT_REPORTING, rpWaitForMsgs);
