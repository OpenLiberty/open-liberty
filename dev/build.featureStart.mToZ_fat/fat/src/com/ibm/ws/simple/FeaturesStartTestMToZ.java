...
public class FeaturesStartTestMToZ {
    ...
    public void cleanup() throws Exception {
        ...
        if (server.isStarted()) {
            // If the server is started at this point, force stop the server ignoring all messages
    ...
    private boolean skipFeature(String feature) throws Exception {
        ...
        if (!feature.startsWith("zosconnect-") && // z/OS Connect is NOT a z/OS only feature
            (feature.startsWith("zos") || feature.equalsIgnoreCase("batchSMFLogging-1.0"))) {
            Log.info(c, testName.getMethodName(), "Skipping feature " + feature + " because it is for z/OS only.");
            return true;
        }
        ...
        if (feature.equalsIgnoreCase("wmqJmsClient-1.1")) // wmqJmsClient.rar.location variable has not been set
            return true;
