 ...
 * Duplicate of com.ibm.ws.session.SessionData
 */
...
public class SessionData60 extends HttpSessionImpl60 {
    ...
    private final SessionContext _sessCtx;
    ...
    private static final String methodClassName = "SessionData60";
    ...
    public void invalidateAll(boolean remote) throws SecurityException {
        //save... in case invalidate clear them out
        String originalSessionId = getId();
        String appName = _sessCtx.getAppName();
        ...
        if (com.ibm.ejs.ras.TraceComponent.isAnyTracingEnabled() && LoggingUtil.SESSION_LOGGER_CORE.isLoggable(Level.FINER)) {
            String logString = "(" + remote + ") for app " + appName + " id " + sessionId;
        ...
        /*
         * Extract this portion of SessionData invalidate() from
         * SessionContextRegistry.getInstance().invalidateAll(getId(), _sessCtx.getAppName(), this, remote, false);
         * and do it here instead
         */

        if (sessionId != null && sessionId.startsWith(SessionContextRegistry.NO_BACKEND_UPDATE_FLAG)) { // special check to prevent each zOS servant from doing db query
            sessionId = sessionId.substring(1); // remove flag
            if (com.ibm.ejs.ras.TraceComponent.isAnyTracingEnabled() && LoggingUtil.SESSION_LOGGER_CORE.isLoggable(Level.FINE)) {
                LoggingUtil.SESSION_LOGGER_CORE.logp(Level.FINE, methodClassName, "invalidateAll", "backendUpdate - sessionId: " + sessionId);
            }
        }

        if ((this.getISession() != null) && (!((MemorySession) this.getISession()).isInvalInProgress())) {
            this.invalidate(); // take care of this session first
        }
        ...
        //Still need to call into new overload SessionContextRegistry.invalidateAll()
        SessionContextRegistry.getInstance().invalidateAll(originalSessionId, appName, remote, false); // false indicates request was not remotely initiated
