...
public class LocalTranCoordImpl implements LocalTransactionCoordinator, UOWCoordinator, UOWScope {
     ...
     *                                At Boundary
     */
    protected LocalTranCoordImpl(boolean boundaryIsAS, boolean unresActionIsCommit, boolean resolverIsCAB, LocalTranCurrentImpl current) throws IllegalStateException {
        if (tc.isEntryEnabled())
            Tr.entry(tc, "LocalTranCoordImpl",
    ...
    public void enlistForCleanup(OnePhaseXAResource resource) throws IllegalStateException {
        if (tc.isEntryEnabled())
            Tr.entry(tc, "enlistForCleanup", resource);
        ...
        if (LocalTranCurrentImpl.globalTranExists()) {
            ...
            if (tc.isEntryEnabled())
                Tr.exit(tc, "enlistForCleanup", ise);
            throw ise;
        }
    ...
    public void delistFromCleanup(OnePhaseXAResource resource) throws IllegalStateException {
        if (tc.isEntryEnabled())
            Tr.entry(tc, "delistFromCleanup", resource);
        ...
        if (LocalTranCurrentImpl.globalTranExists()) {
            ...
            if (tc.isEntryEnabled())
                Tr.exit(tc, "delistFromCleanup", ise);
            throw ise;
        }
    ...
    public void enlistSynchronization(javax.transaction.Synchronization sync) throws IllegalStateException {
        if (tc.isEntryEnabled())
            Tr.entry(tc, "enlistSynchronization", sync);
        ...
        if ((_state != Running) && (_state != Suspended)) {
            ...
            if (tc.isEntryEnabled())
                Tr.exit(tc, "enlistSynchronization", ise);
            throw ise;
        }
     ...
     */
    protected void complete(int endMode, boolean isCompleting) throws InconsistentLocalTranException, RolledbackException, IllegalStateException {
        if (tc.isEntryEnabled()) {
            ...
            if (_enlistedResources != null) {
                for (int i = 0; i < _enlistedResources.size(); i++) {
                    ...
                    try {
                        if (_outcomeRollback) {
                            if (tc.isDebugEnabled())
                                Tr.debug(tc, "Calling rollback on resource " + resource);
                            resource.rollback(null);
                        } else {
                            if (tc.isDebugEnabled())
                                Tr.debug(tc, "Calling commit on resource " + resource);
    ...
    public boolean getRollbackOnly() {
        if (tc.isEventEnabled())
            Tr.event(tc, "getRollbackOnly: " + _rollbackOnly);
    ...
    protected void informSynchronizations(boolean isCompleting) {
        if (tc.isEntryEnabled())
            Tr.entry(tc, "informSynchronizations: isCompleting=" + isCompleting);
        ...
        if (_containerSync != null) {
            if (_state == Running) {
                ...
                if (!_rollbackOnly) {
                    ...
                    { /* @PK08578A */
                        ...
                        if (tc.isDebugEnabled())
                            Tr.debug(tc, "ContainerSync threw an exception during beforeCompletion", t); /* @PK08578A */
            ...
            // if not really completing (mid-AS checkpoiint or reset) delay this signal
                ...
                { /* @PK08578A */
                    ...
                    if (tc.isDebugEnabled())
                        Tr.debug(tc, "ContainerSync threw an exception during afterCompletion", t); /* @PK08578A */
                } /* @PK08578A */
            }
        }
    ...
    protected boolean driveSynchronization(Synchronization sync) {
        ...
        if (_state == Running) {
            try {
                ...
                //
                if (tc.isDebugEnabled())
                    Tr.debug(tc, "An Error occurred in beforeCompletion. Setting RollbackOnly=true.", t);
            ...
            try {
                ...
                //
                if (tc.isDebugEnabled())
                    Tr.debug(tc, "An Error occurred in afterCompletion.", t);
            }
        }
    ...
    public boolean isASScoped() {
        if (tc.isEventEnabled())
            Tr.event(tc, "isASScoped: return=" + _boundaryIsAS);
    ...
    public boolean isContainerResolved() /* @292139C */
    {
        ...
        getComponentMetadataForLTC();
        if (tc.isEventEnabled())
            Tr.event(tc, "isContainerResolved: return=" + _resolverIsCAB);
    ...
    public Byte getConfiguredMetadata() {
        ...
        if (tc.isDebugEnabled())
            Tr.debug(tc, "getConfiguredMetadata", retval);
        return retval;
    }
    ...
    public long getLocalId() {
        ...
        if (tc.isEntryEnabled())
            Tr.exit(tc, "getLocalId", _localId);
        return _localId;
    }
    ...
    public boolean isShareable() {
        ...
        if (tc.isEntryEnabled())
            Tr.exit(tc, "isShareable", _shareable);
        return _shareable;
    }

    void setShareable(boolean shareable) {
        if (tc.isEntryEnabled())
            Tr.entry(tc, "setShareable", new Object[] { shareable, this });
